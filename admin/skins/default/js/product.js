...
function handleLease(checked){
	document.getElementById('lease_price').disabled = !checked;
	document.getElementById('lease_deposit').disabled = !checked;
	document.getElementById('unit').disabled = !checked;
}
...
function getAttrList(productId){
	...
	if(selProductsType != undefined){
		ProductsType = selProductsType.options[selProductsType.selectedIndex].value;
	}
...
function batch_operations(id){
	...
	var barch_subscription = document.getElementById("barch_subscription");
    ...
    if((barch_opration_select.value!='export_csv'&&barch_opration_select.value!='export_csv_by_category')&&checkboxes=="" ){
		alert(j_please_select+j_product);
		return;
	}
    if((barch_opration_select.value == "export_csv"||barch_opration_select.value=='export_csv_by_category')&&export_csv.value == "choice_export"){
	   if(checkboxes=="" ){
			alert(j_please_select+j_product);
			return;
	   }
    }
    if(barch_opration_select.value == 'batch_set_subscription' && barch_subscription.value == ''){
    	alert('请选择是否批量发送订阅');
    	return;
    }
	var strsel = barch_opration_select.options[barch_opration_select.selectedIndex].text;
	if(barch_opration_select.value == "export_csv"||barch_opration_select.value == "export_csv_by_category"){
		ProductExport();
	}else{
		if(confirm('Are you sure to '+strsel+"？")){
		   if(barch_opration_select.value=="product_cat"){
				...
				tempForm.submit();
			  	...
			  	if(barch_opration_select.value=='send_product_mail'){
			  		var producr_ids=checkboxes.join(',');
			  		window.location.href=admin_webroot+"mails/send_product_mail/"+producr_ids;
			  	}else if(barch_opration_select.value=="quotes_edit"){
					...
					postData= postData.substring(0,postData.length-1);
					window.location.href=admin_webroot+"quotes/view/?pid="+postData;
		   	 	}else{
		   	 		var list=[];
					for(var i=0;i<bratch_operat_check.length;i++){
						if(bratch_operat_check[i].checked){
					...
					if(barch_opration_select.value=="batch_set_subscription"){
						postData={'checkboxes[]':list,'subscription_send_flag':document.getElementById("barch_subscription").value};
					}
...
function changeprofile(){
	...
	var export_type_location_price=0;
	if(document.getElementById("export_type_location_price")){
		export_type_location_price=$("#export_type_location_price").val();
	}
	if(code==0){
		alert("请选择导出方式");
		return false;
	...
	if(confirm('Are you sure to '+strsel+"？")){
		if(barch_opration_select.value == "export_csv"&&export_csv.value == "search_result"){
		    ...
		    if(barch_opration_select.value=="export_csv"){
				 ...
				 sUrl+="?export_csv="+document.getElementById("export_csv").value+"&export_type="+document.getElementById("export_type").value+"&export_type_re="+document.getElementById("export_type_re").value+"&code="+code+"&export_type_location_price="+export_type_location_price;
			     tempForm.action=sUrl;
			     tempForm.method="post";
				　document.body.appendChild(tempForm);
...
function barch_opration_select_onchange(obj){
	...
	if(obj.value=="transfer_category"){
		...
		$(".export_type_location_price").parent().hide();
		$(".export_type_location_price input[type='checkbox']").uCheck('uncheck');
	}
	if(obj.value=="export_csv"||obj.value == "export_csv_by_category"){
		//$('#profilegroup').parent().show();
...
function  order_opration_select_onchange(obj){
	...
	if(obj.value==""){
		...
		$("label.export_type_location_price").parent().show();
...
function product_detail_checks(){
	...
	if(document.getElementById("shop_price")){
		var shop_price=document.getElementById("shop_price").value;
		if(document.getElementsByName("data[Product][purchase_price]")[0]){
			var purchase_price=document.getElementsByName("data[Product][purchase_price]")[0].value;
			shop_price=shop_price!=''?parseFloat(shop_price):0;
			purchase_price=purchase_price!=''?parseFloat(purchase_price):0;
			if(shop_price<purchase_price){
				alert(j_price_range_setting);
				return false;
			}
		}
	}
...
function loadExportProfile(){
	var profile_group=new Array('Product','ProductCategory');
	$.ajax({
		type: "POST",
		url:admin_webroot+'profiles/getdropdownlist',
		dataType: 'json',
		data: {group:profile_group},
		success: function (result) {
	    		if(result.flag == 1){
				...
				for(var i=0;i<result_content.length;i++){
					var option=document.createElement("option");
					profilegroup.appendChild(option);
					option.value=result_content[i]['Profile']['code'];
					option.text=result_content[i]['ProfileI18n']['name'];
				}
...
function ProductExport(){
	...
	var export_type_location_price=0;
	if(document.getElementById("export_type_location_price")){
		if($("label.export_type_location_price input[type='checkbox']").prop('checked')){
			export_type_location_price=$("#export_type_location_price").val();
		}
	}
	if(export_csv.value==''){
	...
	var strsel = barch_opration_select.options[barch_opration_select.selectedIndex].text;
	if(confirm('Are you sure to '+strsel+"？")){
		if((barch_opration_select.value == "export_csv"||barch_opration_select.value == "export_csv_by_category")&&export_csv.value == "search_result"){
			var export_act_flag='1';
			if(document.getElementById('brand_id')!=null){
				var brand_id=document.getElementById('brand_id').value;
			}else{
			...
			var ta = ck_checkbox();
			var chestr = '';
			chestr +="&"+"category_id=" +ta.substring(ta,ta.length-1);
			var attr=attr_checkbox();
			chestr +="&"+"attr_value=" +attr.substring(attr,attr.length-1);
			var url = "export_act_flag="+export_act_flag+"&brand_id="+brand_id+"&product_type_id="+product_type_id+"&forsale="+forsale+"&start_date_time="+start_date_time+"&end_date_time="+end_date_time+"&product_keywords="+product_keywords+"&is_recommond="+is_recommond+"&start_date="+start_date+"&end_date="+end_date+"&min_price="+min_price+"&max_price="+max_price+"&category_id="+chestr+"&export_type_location_price="+export_type_location_price+'&export_code='+barch_opration_select.value+"&product_cross_reference="+product_cross_reference;
			window.location.href = encodeURI(admin_webroot+"products?"+url);
		}else{
			document.getElementById('export_act_flag').value='';
			var postData = "";
			...
			var tempForm　=　document.createElement("form");
			var sUrl = admin_webroot+"products/batch_operations/"+barch_opration_select.value;//访问的URL地址
			sUrl+="?export_csv="+document.getElementById("export_csv").value+"&export_type="+document.getElementById("export_type").value+"&export_type_re="+document.getElementById("export_type_re").value+"&export_type_location_price="+export_type_location_price;
			tempForm.action=sUrl;
			tempForm.method="post";
			document.body.appendChild(tempForm);
			var　tempInput　=　document.createElement("input");

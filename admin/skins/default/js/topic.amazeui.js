			...
			if(data.flag=="1"){
					...
					if(data.content){
						...
						for(i=0;i<data.content.length;i++){
							selhtml+= "<div class='am-g' style='border-bottom:1px solid #ddd'><div class='am-checkbox am-margin-top-xs am-margin-bottom-xs' style='margin-left:10px; '><label><input type='checkbox' data-am-ucheck name='checkboxes[]'  value="+data.content[i]['Product'].id+" data-am-ucheck />"+data.content[i]['Product'].code+"--"+data.content[i]['ProductI18n'].name+"</label> </div></div>";
						}
			        ...
			        $("input[name='checkboxes[]']").uCheck('enable')
					return;
				}
				if(data.flag=="2"){
					alert(data.content);

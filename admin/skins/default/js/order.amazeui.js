...
function sendinfo(m) {
	...
	if(tmpselect=="1"){
		$("#order_address_info_table div.order_user_address_edit").hide();
	}else{
		$("#order_address_info_table div.order_user_address_edit").show();
	}
	if(m!=0){
		edit_order_address();
	}
...
function edit_order_address(){ 
  ...
  $("div.order_user_address_edit.address_save").css("display",'block');
...
//		data: { 'order_id': order_id,'order_user_id': order_user_id ,'type': "user",'order_shipping_id': order_shipping_id},
		...
		success: function(data){
			...
			$("#order_user_info select").selected({'noSelectedText':j_please_select,'maxHeight':150});
...
function order_address_data_save(){
	...
	if(order_email!=''){
		if(!/^[a-zA-Z0-9_-]+@[a-zA-Z0-9_-]+(\.[a-zA-Z0-9_-]+)+$/.test(order_email)){
			$("#order_address_data_save").attr("disabled",false);
			alert('邮箱格式有误！');return;
		}
	}
	...
	if(order_zipcode!=''){
		if(!/^[1-9][0-9]{5}$/.test(order_zipcode)){
			$("#order_address_data_save").attr("disabled",false);
			alert('邮编格式有误！');return;
		}
	}
	...
	var postData="";
	if(document.getElementById("opener_select_user_id"))
	 postData += "order_id="+order_id+"&order_user_id="+order_user_id;
	else
	 postData += "order_id="+order_id;
	//收货人信息
	postData+="&order_telephone="+encodeURI(order_telephone)+"&order_consignee="+encodeURI(order_consignee)+"&order_mobile="+encodeURI(order_mobile);
	postData+="&order_country="+encodeURI(order_country)+"&order_province="+encodeURI(order_province)+"&order_city="+encodeURI(order_city)+"&order_district="+encodeURI(order_district);
	postData+="&order_sign_building="+encodeURI(order_sign_building)+"&order_address="+encodeURI(order_address)+"&order_best_time="+encodeURI(order_best_time)+"&order_how_oos="+encodeURI(order_how_oos);
	postData+="&order_zipcode="+encodeURI(order_zipcode)+"&order_note="+encodeURI(order_note)+"&order_postscript="+encodeURI(order_postscript)+"&order_email="+encodeURI(order_email)+"&order_shipping_id="+order_shipping_id+"&sel_address="+encodeURI(sel_address)+"&user_address_id="+user_address_id;
...
function search_order_product(){
	...
	if(order_user_id==0||order_user_id==""){
		$("#my-popup .am-popup-bd").html('请先选择购货人');
		$("#my-popup").modal('open');
		return false;
	}
	var obj = document.getElementById("add_product_button");
	var lease_type = document.getElementById("lease_type").value;//订单类型
	var keywords=Trim(document.getElementById("order_product").value);
	if(keywords.replace(/([\u0391-\uFFE5])/ig,'11').length<3){
		$("#order_search #r_t").remove();
		$("#order_search p").remove();
		$("#order_search").append('<p style="margin-left:0.5em;">'+j_keyword_three+"</p>");
	...
	var postData = {'keywords': keywords,'brand':brand,'product_style':product_style,'product_type':product_type,'lease_type':lease_type};
		...
		success: function(data){
			 ...
			 if (data.message){
				if(data.message.length==0){
					$("#order_search #r_t").remove();
					$("#order_search p").remove();
					$("#order_search").append('<p style="margin-left:0.5em;">'+rebate_055+"</p>");
					 // alert(rebate_055);
					 // $("#my-popup .am-popup-bd").html(rebate_055);
					 // $("#my-popup").modal('open');
					return;
				}
				var table ="<div class='am-table am-g' id='r_t' style='background-color:#FFF;padding:10px 0px 0 20px;'>";
				table+="<div class='am-cf'><div style='border:none;' class='am-u-lg-3 am-u-md-3 am-u-sm-2'>图片</div><div style='border:none;' class='am-u-lg-3 am-u-md-3 am-u-sm-2'>商品</div><div style='border:none;' class='am-u-lg-3 am-u-md-3 am-u-sm-4'>价格</div><div style='border:none;' class='am-u-lg-3 am-u-md-3 am-u-sm-4'>操作</div></div>";
	            for (i = 0; i < data.message.length; i++ ){
	            		var product_image=data.message[i]['Product'].img_thumb;
	            		product_image=product_image.trim()==''?'/theme/default/images/default.png':product_image;
	                		table+="<tr><div class='am-u-lg-3 am-u-md-3 am-u-sm-2'><img width='40' height='40' src="+product_image+"></div><div class='am-u-lg-3 am-u-md-3 am-u-sm-2'><div>"+data.message[i]['Product'].code+"</div><div >"+data.message[i]['ProductI18n'].name+"</div></div>"
					table+="<div class='am-u-lg-3 am-u-md-3 am-u-sm-4'>￥"+data.message[i]['Product'].shop_price+"</div>";
					...
					table+="<div class='am-u-lg-3 am-u-md-3 am-u-sm-4'><button "+data_am_modal_txt+" onclick='add_order_product(\""+data.message[i]['Product'].code+"\",\""+data.message[i]['Product'].is_sku+"\",\""+data.message[i]['Product'].id+"\")' class='am-btn am-btn-success am-btn-sm'>添加</button></div></tr>";
	            }
				table+="</div>";
				// $("#my-popup .am-popup-bd").html(table);
				$("#order_search #r_t").remove();
				$("#order_search p").remove();
				$("#order_search").append(table);
				// $("#my-popup").modal('open');
	         };
	...
	$.ajax({ url: sUrl,
		...
		success: function(data){
			if(data.code==1){
				...
				order_reflash(data.hasproduct,data.total,data.need_pay,insure_fee,order_discount);
				$(".am-close").click();	
...
//订单编辑页刷新
function order_reflash(hasproduct,total,need_pay,insure_fee,order_discount){
	var order_id = document.getElementById("order_id").value;//订单ID
	var lease_type = document.getElementById("lease_type").value;//订单类型
	if(lease_type=="L"){
		var sUrl = admin_webroot+"orders/lease_edit/"+order_id+"/1/";//访问的URL地址
	}else{
	...
	$.ajax({ url: sUrl,
		...
		success: function(data){
			...
			if(total>0){
				var total_num = new Number(total);var need_pay_mum=new Number(need_pay);
				total=total_num.toFixed(2);need_pay=need_pay_mum.toFixed(2);
				document.getElementById('order_total').innerHTML = sprintf(js_config_price_format,total);
				document.getElementById('need_pay').innerHTML = sprintf(js_config_price_format,need_pay);
			}else{
				var total_num = new Number(total);var need_pay_mum=new Number(need_pay);
				total=total_num.toFixed(2);need_pay=need_pay_mum.toFixed(2);
				document.getElementById('order_total').innerHTML = sprintf(js_config_price_format,total);
			...
			if(typeof(insure_fee)!='undefined'){
				document.getElementById("order_insure_fee_span").innerHTML=sprintf(js_config_price_format,insure_fee);
				document.getElementById("order_insure_fee").value=insure_fee;
			}
			if(typeof(order_discount)!='undefined'){
				document.getElementById("order_discount_span").innerHTML=sprintf(js_config_price_format,order_discount);
				document.getElementById("order_discount").value=order_discount;
			}
...
function order_products_data_save(){
	...
	var order_product_expire_date= document.getElementsByName("order_product_expire_date[]");//订单商品到期时间
	var order_product_begin_date= document.getElementsByName("order_product_begin_date[]");//订单商品开始时间
	...
	var order_product_expire_date_arr=[];
	var order_product_begin_date_arr=[];
	for(var i=0;i<order_product_id.length;i++){
		...
		if(typeof(order_product_expire_date[i])!='undefined'){
			order_product_expire_date_arr[i]=order_product_expire_date[i].value;
		}
		if(typeof(order_product_begin_date[i])!='undefined'){
			order_product_begin_date_arr[i]=order_product_begin_date[i].value;
		}
	}
	
var postData ={ "order_id":order_id,"order_product_id":order_pid_arr,"order_product_code":order_pro_code_arr,"order_product_price":order_pro_price_arr,"order_product_quntity":order_pro_qty_arr,"order_product_discount":order_pro_discount_arr,"order_product_attr":order_pro_attr_arr,'order_product_expire_date':order_product_expire_date_arr,'order_product_begin_date':order_product_begin_date_arr};
	...
	$.ajax({ url: sUrl,
		...
		success: function(data){
			if(data.code==1){
				document.getElementById("last_order_subtotal").innerHTML=sprintf(js_config_price_format,data.subtotal);
				...
				if(data.money_paid!=null){
					document.getElementById("order_money_paid_span").innerHTML=sprintf(js_config_price_format,data.need_pay);
				}
				if(data.insure_fee!=null){
					document.getElementById("order_insure_fee_span").innerHTML=sprintf(js_config_price_format,data.insure_fee);
					document.getElementById("order_insure_fee").value=data.insure_fee;
				}
				if(document.getElementById("purchase_price_total")!=null){
					if(data.purchase_price_total!=null){
						document.getElementById("purchase_price_total").innerHTML=sprintf(js_config_price_format,data.purchase_price_total);
					}
				}
...
function order_status_select(code){
		if(code==""){
			...
			if(code=="order_delivery"||code=="order_payment_delivery"){//订单发货
			...
			if(code=="order_check"){
				$("#order_status_change").val('order_check');
			}
			if(code=="order_check_remove"){
				$("#order_status_change").val('order_check_remove');
			}
			$("#OrderProductTable tfoot").removeClass('order_status');
			
			var status_change_title=$("#OrderStatusChange input#"+code).val();
			$("#order_status_change_modal div.am-modal-title").html(status_change_title);
			$("#order_status_change_modal").modal({closeViaDimmer:false}).on('closed.modal.amui', function(){
					$("#order_status_change_modal textarea").val('');
			});
		}
}
...
function order_status_change(){
	...
	if(document.getElementById("order_check")){
		var order_check_id = '1';
	}else{
		var order_check_id = '0';
	}
	...
	// }
	//postData+="&check_status="+order_check_id;
...
function edit_order_user(){
	...
	if(order_user_id==0){
		...
		document.getElementById("create_user_info").style.display="block";
	}
	...
	$("#avatar_img01").css("display","none").removeAttr('disabled');
	$("#avatar_img02").css("display","none").removeAttr('disabled');
	$("#avatar_img03").css("display","none").removeAttr('disabled');
	$("#avatar_img04").css("display","none").removeAttr('disabled');
	$("#avatar_img05").css("display","none").removeAttr('disabled');
	$("#avatar_img06").css("display","none").removeAttr('disabled');
//	YUI().use('*', function(Y) {
...
function order_user_save(){
	...
	postdata+="&create_user_name="+encodeURI(create_user_name);
	postdata+="&create_user_mobile="+encodeURI(create_user_mobile);
...
function order_status_select_reload(){
	...
	$.ajax({ url: sUrl,
		...
		success: function(data){
			$("#status_change_td").html('');
			$.each(data,function(index,item){
				$("#status_change_td").append("<input type='button' id='"+index+"' class='am-btn am-btn-success am-radius am-btn-sm' name='order_status_change' onclick='order_status_select(this.id)' value='"+item+"' />");
			});
		}
	});
}
...
function orderproduct_expire_date(){
    var nowTemp = new Date();
    var nowDay = new Date(nowTemp.getFullYear(), nowTemp.getMonth(), nowTemp.getDate(), 0, 0, 0, 0).valueOf();
    var nowMoth = new Date(nowTemp.getFullYear(), nowTemp.getMonth(), 1, 0, 0, 0, 0).valueOf();
    var nowYear = new Date(nowTemp.getFullYear(), 0, 1, 0, 0, 0, 0).valueOf();
    $("div.orderproduct_expire_date input[type='text']").each(function(){
        var $dateinput = $(this);
        var checkin = $dateinput.datepicker({
            ...
            order_products_data_save();//订单商品信息保存
            checkin.close();
        }).data('amui.datepicker');
    });
}

function orderproduct_begin_date(){
    var nowTemp = new Date();
    var nowDay = new Date(nowTemp.getFullYear(), nowTemp.getMonth(), nowTemp.getDate(), 0, 0, 0, 0).valueOf();
    var nowMoth = new Date(nowTemp.getFullYear(), nowTemp.getMonth(), 1, 0, 0, 0, 0).valueOf();
    var nowYear = new Date(nowTemp.getFullYear(), 0, 1, 0, 0, 0, 0).valueOf();
    $("div.orderproduct_begin_date input[type='text']").each(function(){
        var $dateinput = $(this);
        var checkin = $dateinput.datepicker({
            theme: 'success',
            locale:backend_locale,

...
$(document).ready(function(){
	...
	$(".am-search button").click(function(){
		...
		if(keyword.length>0){
			window.location.href=web_base+"/searchs/index?keyword="+keyword;
		}
	});
...
//获取手机验证码
function mobile_code_send(input_str,send_btn){
	...
	$.ajax({
		...
		success: function(data){
			if(data.code=='1'){
				if(typeof(send_btn)!='undefined'){
					var countdown_second=60;
					var btn_default_txt=$(send_btn).text();
					var btn_default_val=$(send_btn).val();
					var countdown=function(btn){
						if (countdown_second == 0) { 
							btn.removeAttribute("disabled");
							$(btn).text(btn_default_txt).val(btn_default_val);
							countdown_second = 60;
						} else { 
...
//获取邮箱验证码
function email_code_send(input_str,send_btn){
...
function virtual_purchase_pay(type,type_id){
	if(typeof(js_login_user_data)=='undefined'||js_login_user_data==null)return;
	$.ajax({
		type: "POST",
		url:web_base+"/virtual_purchases/purchase_order",
		data:{'virtual_type':type,'virtual_type_id':type_id},
		dataType:"html",
		async: false,
		success: function(data) {

...
//    } 
    ...
    function home_timeline($page =-1 , $count = 20) 
    { 
    ...
    function update_avatar( $pic_path )
	{
	 ...
	 //新增 清除seesion
    function end_seesion()
    {
    	$this->oauth->post('http://api.t.sina.com.cn/account/end_session.json');
    }
    ...
    function shortUrl($url)
    {
    	...
    	$this->oauth->get('http://api.t.sina.com.cn/shortUrl.json' ,$param);
    }
    // ========================================= 
...
class WeiboOAuth { 
    ...
    function getRequestToken($oauth_callback = NULL) { 
        ...
        if(!isset($token['oauth_token'])||!isset($token['oauth_token']))
        	$token = OAuthUtil::parse_parameters($request); 
        $this->token = new OAuthConsumer($token['oauth_token'], $token['oauth_token']); 
        return $token; 
    } 

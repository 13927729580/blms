		...
		while ($i<$h_size && $head[$i]){
			...
			switch (strtoupper($summ)){
				...
				if ($left_tag_pos=strpos($content,"<")){
					...
					else if (ereg("[\"|\']([^\'\"]+)[\'|\"]",$this->from_name,$reg))
					$this->from_name=$reg[1];
				}else{
					$this->from_name=$content;
					$this->from_mail=$content;//没有发件人的邮件地址
				}
				break;
				...
				if ($left_tag_pos=strpos($content,"<")){
					...
					else  if (ereg("[\"|\']([^\'\"]+)[\'|\"]",$this->to_name,$reg))
					$this->to_name=$reg[1];
				}else{
					$this->to_name=$content;
					$this->to_mail=$content; //没有分开收件人的邮件地址
				}
				break;
				...
				if (ereg("<([^>]+)>",$content,$reg)) $this->reply_to=$reg[1];
				else $this->reply_to=$content;
				break;
				case "CONTENT-TYPE":// 整个邮件的 Content类型， 
				eregi("([^;]*);",$content,$reg);
				...
				break;
				case "CC"://抄送到。。
				if (ereg("<([^>]+)>",$content,$reg)) $this->cc_to=$reg[1];
				else $this->cc_to=$content;
				default:
				break;
			}// end switch
			$i++;
	...
	function decode_mult($type,$boundary,$begin_row){// 该方法用递归的方法实现　复合类型邮件正文的解码，邮件源文件取自于 body_temp 数组，调用时给出该复合类型的类型、分隔符及　在　body_temp 数组中的开始指针
		...
		while ($i<$lines){// 这是一个部分的结束标识；
			...
			if (eregi("multipart",$sub_type)){// 该子部分又是有多个部分的；
				...
				if ($name!="") $this->body[$this->tem_num]['name']=$name;
				else

...
class MSN
{
    ...
    var $membership_url = 'https://local-sn.contacts.msn.com/abservice/SharingService.asmx';
    var $membership_soap = 'http://www.msn.com/webservices/AddressBook/FindMembership';

    var $addmember_url = 'https://local-sn.contacts.msn.com/abservice/SharingService.asmx';
    var $addmember_soap = 'http://www.msn.com/webservices/AddressBook/AddMember';

    var $addcontact_url = 'https://local-sn.contacts.msn.com/abservice/abservice.asmx';
    var $addcontact_soap = 'http://www.msn.com/webservices/AddressBook/ABContactAdd';

    var $delmember_url = 'https://local-sn.contacts.msn.com/abservice/SharingService.asmx';
  ...
  <ps:RequestMultipleSecurityTokens xmlns:ps="http://schemas.microsoft.com/Passport/SoapServices/PPCRL" Id="RSTS">
    ...
    <wst:RequestSecurityToken Id="RST6">
      ...
      <wsse:PolicyReference URI="MBI"></wsse:PolicyReference>
        ...
        // RST3: local-sn.contacts.msn.com
        ...
        // matches[6]: ...
        // matches[7]: RST3 (local-sn.contacts.msn.com) ticket
        // matches[8]: ...
...
public function getAllFriendsList2(){
        //SOAP请求包
        ...
        $header_array = array(
            ...
            'Host: local-sn.contacts.msn.com'
        );

        //发送SOAP请求
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL,'http://local-sn.contacts.msn.com/abservice/abservice.asmx');
        curl_setopt($curl, CURLOPT_HTTPHEADER, $header_array);
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($curl, CURLOPT_FOLLOWLOCATION, 1);
        curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, 0);
...
//                    	print_r($ann);
//                    	echo "========<br>";
////                        if($ann->Name=='AB.displayName'){
////                            $name=$ann->Value;
////                            break;
////                        }
//                    }
//                }elseif($info->annotations->Annotation->Name=='AB.displayName'){
//                	echo 2;
//                    $name=$info->annotations->Annotation->Value;
//                }
//            }
			
			if(!$name)if((isset($info->firstName)) && (isset($info->lastName)))$name=$info->lastName.$info->firstName;
 
			
            if(!$name)if(isset($info->displayName))if($info->displayName!=$email)$name=$info->displayName;

//            if(!$name)if(isset($info->quickName))$name=$info->quickName;
           if(empty($name))if(isset($info->displayName))$name=$info->displayName;

//           if(empty($name) || （htmlspecialchars((string)$email,ENT_QUOTES) == htmlspecialchars((string)$name,ENT_QUOTES)）)
//    			$name=$info->quickName;
			//echo "<<<<<<<<<<<<<<<"."<br>";
            //加入联系人数组中
            $contacts[(string)$email]=array('email'=>htmlspecialchars((string)$email,ENT_QUOTES),'name'=>htmlspecialchars((string)$name,ENT_QUOTES),'group'=>htmlspecialchars((string)$group),ENT_QUOTES);
        }//print_r($contacts);
        //die();
        return $contacts;
    }

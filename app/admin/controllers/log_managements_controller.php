...
class LogManagementsController extends AppController
{
    ...
    public $uses = array('SystemResource','Config','ConfigI18n','LanguageDictionary','SmsSendHistory','MailStatistic','MailSendHistory');
    ...
    public function index()
    {
        ...
        $sms_success = $this->SmsSendHistory->find('count', array('conditions' => array('SmsSendHistory.flag' => 1, "SmsSendHistory.send_date>= '".$now."'")));//成功
        $sms_error = $this->SmsSendHistory->find('count', array('conditions' => array('SmsSendHistory.flag' => 0, "SmsSendHistory.send_date>= '".$now."'")));//失败
        ...
        if (constant('Product') == 'AllInOne') {
         ...
         //邮件日志
            ...
            $mail_count = $this->MailSendHistory->find('count', array('conditions' => ''));//邮件统计数量
            $mail_success = $this->MailSendHistory->find('count', array('conditions' => array('MailSendHistory.flag>=1', "MailSendHistory.created>= '".$now."'")));//发布成功
            $mail_error = $this->MailSendHistory->find('count', array('conditions' => array('MailSendHistory.flag' => 0, "MailSendHistory.created>= '".$now."'")));

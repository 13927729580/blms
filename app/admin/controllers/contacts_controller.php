...
class ContactsController extends AppController
{
    ...
    public $uses = array('Contact','InformationResource','InformationResourceI18n','OperatorLog','ContactConfig');
    ...
    public function index($page = 1)
    {
        ...
        if (isset($this->params['url']['contact_us_type']) && trim($this->params['url']['contact_us_type'])!= '') {
            $condition['and']['type'] = trim($this->params['url']['contact_us_type']);
    ...
    public function view($id)
    {
        ...
        if(empty($this->data['Contact'])){$this->redirect('index');}
        $this->set('this->data', $this->data);
        $this->navigations[] = array('name' => $this->data['Contact']['contact_name'],'url' => '');
        $Resource_info = $this->Resource->getformatcode(array('contact_us_type'), $this->backend_locale);
        $this->set('Resource_info', $Resource_info);
		...
		$condition=array();
		$condition['ContactConfig.type']=isset($this->data['Contact']['type'])?$this->data['Contact']['type']:'';
		$condition['ContactConfig.contact_id']=array(0,$id);
		$condition['ContactConfig.status']='1';
		$contact_config_info=$this->ContactConfig->find('all',array('conditions'=>$condition,'order'=>'ContactConfig.orderby'));
		$contact_config_data=array();
		$contact_config_value=array();
		$contact_config_option=array();
		foreach($contact_config_info as $v){
			if($v['ContactConfig']['contact_id']==0){
				$contact_config_data[$v['ContactConfig']['code']]=$v;
				$config_option=array();
				...
				if($config_option_txt!=""){
					...
					$contact_config_option[$v['ContactConfig']['code']]=$config_option;
				}
			}else{
				$contact_config_value[$v['ContactConfig']['code']]=$v['ContactConfig']['value'];
			}
		}
		$this->set('contact_config_data',$contact_config_data);
		$this->set('contact_config_value',$contact_config_value);
		$this->set('contact_config_option',$contact_config_option);
    }
    ...
    public function quick_reply(){
        	...
        	if(!empty($contact_data)&&!empty($quick_reply_content)){
        		$mailsendqueue = array(
	              ...
	              $mail_result=$this->Notify->send_email($mailsendqueue,$this->configs);
	              if($mail_result){
	              	$result['flag']='1';

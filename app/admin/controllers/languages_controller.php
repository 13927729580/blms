...
class LanguagesController extends AppController
{
    ...
    public function index()
    {
        ...
        if (isset($this->params['url']['system_code']) && $this->params['url']['system_code'] != '') {
	            $condition['Language.system_code'] = $this->params['url']['system_code'];
	            $this->set('system_code', $this->params['url']['system_code']);
        }
        if (isset($this->params['url']['module_code']) && $this->params['url']['module_code'] != '') {
	            $condition['Language.module_code'] = $this->params['url']['module_code'];
        ...
        $lost_lan1 = array();
        $lost_lan2 = array();
        $data = $this->Language->find('all', array('conditions'=>$condition,'order' => array('Language.is_default desc')));
	...
	$default_language_cond=array();
	$default_language_cond['id <>']=$id;
	$default_language_info=$this->Language->find('all',array('conditions'=>$default_language_cond));
        $default_language_list=array();
        foreach($default_language_info as $v){
        	if($v['Language']['front']=='1'){
        		$default_language_list['front']=$v['Language']['id'];
        	}
        	if($v['Language']['backend']=='1'){
        		$default_language_list['backend']=$v['Language']['id'];
        	}
        }
        $this->set('default_language_list',$default_language_list);
    }
    ...
    function system_modified(){
		...
		$language_id=isset($_POST['id'])?$_POST['id']:0;
		$system_code=isset($_POST['val'])?trim($_POST['val']):'';
		$this->Language->save(array('id'=>$language_id,'system_code'=>$system_code));
		$result['flag'] = 1;
      ...
      function module_modified(){
		...
		$this->Language->save(array('id'=>$language_id,'module_code'=>$module_code));
		$result['flag'] = 1;

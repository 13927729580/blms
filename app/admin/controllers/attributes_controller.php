...
class AttributesController extends AppController
{
    ...
    public function attribute_option($attribute_id = 0, $page = 1)
    {
        ...
        $this->Pagination->init($condition, $parameters, $options);
        $attr_option_list = $this->AttributeOption->find('all', array('page' => $page, 'limit' => $rownum ,'conditions' => $condition, 'order' => 'AttributeOption.orderby,AttributeOption.option_name'));
    ...
    public function upload_csv()
    {
        ...
        $fields_array = array($this->ld['code'],
          ...
          $this->ld['sort'],
           ...
           $this->ld['option_value'],
           $this->ld['option_name'],
           $this->ld['price'],
           $this->ld['status'],
           $this->ld['option_sort']
            );
        $optionfields = array('AttributeOption.option_value','AttributeOption.option_name','AttributeOption.price','AttributeOption.status','AttributeOption.orderby');
        $newdatas = array();
        $newdatas[] = $fields_array;
        $this->Attribute->set_locale($this->backend_locale);

        $Attributes_all = $this->Attribute->find('all', array('order' => 'Attribute.id ', 'limit' => 10));
        //pr($Attributes_all);exit();
        ...
        foreach ($Attributes_all as $k => $v) {
            ...
            //pr($v);
            if(isset($v['AttributeOption'])&& count($v['AttributeOption']) > 0){
                //pr($v['AttributeOption']);
                foreach ($v['AttributeOption'] as $kk => $vv) {

                    $optiontemp = array();
                    $optiontemp[] = $vv['option_value'];
                    $optiontemp[] = $vv['option_name'];
                    $optiontemp[] = $vv['price'];
                    $optiontemp[] = $vv['status'];
                    $optiontemp[] = $vv['orderby'];
                    $newdatas[] = array_merge($user_tmp,$optiontemp) ;
                }
            }else{
                $user_tmp[] = '';
                $user_tmp[] = '';
         ...
         //接收过来的文件不为空
            ...
            if ($_FILES['file']['error'] > 0) {
             ...
             $key_arr = array();
                ...
                //pr($key_arr);
                $key_arr[]= 'AttributeOption.option_value';
                $key_arr[]= 'AttributeOption.option_name';
                $key_arr[]= 'AttributeOption.price';
                $key_arr[]= 'AttributeOption.status';
                $key_arr[]= 'AttributeOption.orderby';
                //pr($key_arr);exit();
                ...
                while ($row = $this->fgetcsv_reg($handle, 10000, ',')) {
                    ...
                    //pr($row);exit();
                    foreach ($row as $k => $v) {
                        $temp[$key_arr[$k]] = trim($v)=='' ? '' : @iconv($csv_export_code, 'utf-8//IGNORE', $v);
                    }
      ...
      elseif (!empty($this->data)) {
        //pr($this->data);
          ...
          foreach ($this->data as $key => $v) {
              ...
              $AttributeOption_finds = array('id','option_value','attribute_id','option_name','price','status','orderby');
                //定义两个数组 分别存 两张表
                $Attribute_arr = array();//主表数组
                $AttributeI18n_arr = array();//次表数组
                $AttributeOption_arr = array();
                    foreach ($v as $ks => $vs) {
                    ...
                    //pr($fields_k);exit();
                        //pr($tr);
                       if(isset($tr[1])&&in_array($tr[1],$AttributeOption_finds)){
                            $AttributeOption_arr[$tr[1]] = $vvv;
                       }
                    }
                    //pr($AttributeOption_arr);
              ...
              $AttributeOption_arr['attribute_id'] = $this->Attribute->id;

              $i8n_arr = $this->AttributeI18n->find('first', array('conditions' => array('AttributeI18n.locale' => $AttributeI18n_arr['locale'], 'AttributeI18n.attribute_id' => $AttributeI18n_arr['attribute_id'])));
              ...
              //pr($AttributeOption_arr);
               ...
               $arr_option = $this->AttributeOption->find('first', array('conditions' => array('AttributeOption.locale' => $AttributeOption_arr['locale'], 'AttributeOption.attribute_id' => $AttributeOption_arr['attribute_id'],'AttributeOption.option_value' => $AttributeOption_arr['option_value'])));
               $AttributeOption_arr['id'] = isset($arr_option['AttributeOption']['id']) ? $arr_option['AttributeOption']['id'] : 0;
              //pr($AttributeOption_arr);

               $this->AttributeOption->save($AttributeOption_arr);
          }
          $this->redirect('/attributes');
      }

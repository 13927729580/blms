...
class MessagesController extends AppController
{
    ...
    public $components = array('Pagination','RequestHandler','Notify','Phpexcel');
    public $uses = array('NotifyTemplateType','MailTemplate','UserMessage','User','ProductType','Resource','Product','ProductI18n','OperatorLog');
    ...
    public function index($page = 1)
    {
        ...
        if (isset($this->params['url']['reply_status']) && $this->params['url']['reply_status'] != '') {
            ...
            if (!empty($replycount) && sizeof($replycount) > 0) {
                ...
                //pr($reply_ids);exit();
                if ($reply_status == 0) {
                    $condition['not']['UserMessage.id'] = $reply_ids;
                } else {
    ...
    public function view($id)
    {
        ...
        if ($this->RequestHandler->isPost()) {
            if ($this->data['UserMessage']['msg_content'] != '') {
                ...
                $restore = $this->UserMessage->find('first',array('conditions'=>array('UserMessage.parent_id'=>$id)));
                $sent_date = date('Y-m-d H:m:s');
                if(!empty($restore['UserMessage'])){
			...
			foreach($totify_template_info as $template_type=>$totify_template){
				if($template_type=="email"&&$usermessage['UserMessage']['user_email']!=""){
					$subject = $totify_template['NotifyTemplateTypeI18n']['title'];
						...
						'receiver_email' =>$usermessage['UserMessage']['user_email'],//接收人姓名;接收人地址
						'cc_email' => ';',
						'bcc_email' => ';',
						'title' => $subject,
						'html_body' => $html_body,

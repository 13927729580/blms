            ...
            if ($reply_type == 'picture') {
                ...
                if ($element_data = $this->_getElementMsg($reply_picture)) {
                    foreach ($element_data as $k => $v) {
                        ...
                        $reply_data['articles'][$k]['url'] = empty($v['OpenElement']['url']) ? $this->server_host.'/open_elements/preview/'.$v['OpenElement']['id'] : $v['OpenElement']['url'];
                    }
                ...
                if ($article_data = $this->_getArticleMsg($reply_article)) {
                    $reply_data['articles'][0]['title'] = $article_data['ArticleI18n']['title'];
    ...
    *************************************************************/
    ...
    public function api_user_action($id){
		...
		if (!empty($openModelInfo)) {
			...
			if ($openType == 'wechat') {
				$next_openid=isset($_REQUEST['next_openid'])?$_REQUEST['next_openid']:'';
				$url = 'https://api.weixin.qq.com/cgi-bin/user/get?access_token='.$openModelInfo['OpenModel']['token'].'&next_openid='.$next_openid;
			}
			if (isset($url)) {
				$api_result = file_get_contents($url);
				$api_result = json_decode($api_result, true);
				if (isset($api_result['data']['openid'])) {
					$result['code']='1';
					$result['message']='获取关注列表成功';
					$result['open_user_list']=$api_result;
					$result['open_type_id']=$openModelInfo['OpenModel']['open_type_id'];
					$result['accessToken']=$accessToken;
				} else {
					$result['message']=isset($api_result['errmsg'])?$api_result['errmsg']:'获取关注列表失败';
				}
			}
		}
		die(json_encode($result));
     ...
     *@version  $Id$
     */
    public function ajax_get_open_user_detail(){
		Configure::write('debug', 1);
		$this->layout = 'ajax';
		$this->operator_privilege('open_users_update');
		$result=array();
		$result['code']='0';
		$result['message']='获取关注用户失败';
		$accessToken=isset($_REQUEST['accessToken'])?$_REQUEST['accessToken']:'';
		$openId=isset($_REQUEST['openId'])?$_REQUEST['openId']:'';
		$openTypeId=isset($_REQUEST['openTypeId'])?$_REQUEST['openTypeId']:'';
		$openType=isset($_REQUEST['openType'])?$_REQUEST['openType']:'wechat';
		$openIds=array();
		if(is_array($openId)&&sizeof($openId)>0){
			$openIds=$openId;
//			foreach($openId as $v){
//				$this->_saveUser($accessToken,$v,$openTypeId,$openType);
//			}
		}else if(is_string($openId)&&$openId!=''){
			$openIds[]=$openId;
			//$this->_saveUser($accessToken,$openId,$openTypeId,$openType);
		}
		if(!empty($openIds)&&sizeof($openIds)>0&&sizeof($openIds)<=100){
			$this->_saveUser($accessToken,$openIds,$openTypeId,$openType);
			$result['code']='1';
			$result['message']='获取关注用户成功';
		}else if(!empty($openIds)&&sizeof($openIds)>100){
			$request_openIds=array();
			foreach($openIds as $k=>$v){
				$data_key=floor($k/100);
				$request_openIds[$data_key][]=$v;
			}
			foreach($request_openIds as $openId_list){
				$this->_saveUser($accessToken,$openId_list,$openTypeId,$openType);
			}
			$result['code']='1';
			$result['message']='获取关注用户成功';
		}
    ...
    private function _saveUser($accessToken, $openId, $openTypeId, $openType = 'wechat'){
		...
		$post_data=array();
		foreach($openId as $v){
			$post_data['user_list'][]=array('openid'=>$v);
		}
	       $post_data_str = $this->to_josn($post_data);
	        $result = $this->https_request($url,$post_data_str);
	        if (!empty($result) && isset($result['user_info_list'])) {
	        	$open_user_list=$this->OpenUser->find('list',array('fields'=>'OpenUser.openid,OpenUser.id','conditions'=>array('OpenUser.openid'=>$openId)));
			foreach($result['user_info_list'] as $user_result){
				if (!isset($open_user_list[$user_result['openid']])) {
					$userInfo['OpenUser']['id'] = 0;
				} else {
					$userInfo['OpenUser']['id'] =$open_user_list[$user_result['openid']];
					$userInfo['OpenUser']['subscribe'] = 1;
				}
				$userInfo['OpenUser']['open_type'] = $openType;
				$userInfo['OpenUser']['open_type_id'] = $openTypeId;
				$userInfo['OpenUser']['openid'] = $user_result['openid'];
				$userInfo['OpenUser']['nickname'] = urlencode($user_result['nickname']);
				$userInfo['OpenUser']['sex'] = $user_result['sex'];
				$userInfo['OpenUser']['language'] = $user_result['language'];
				$userInfo['OpenUser']['city'] = $user_result['city'];
				$userInfo['OpenUser']['province'] = $user_result['province'];
				$userInfo['OpenUser']['country'] = $user_result['country'];
				$userInfo['OpenUser']['headimgurl'] = $user_result['headimgurl'];
				$userInfo['OpenUser']['subscribe_time'] = $user_result['subscribe_time'];

...
class UploadFilesController extends AppController
{
    ...
    public function add()
    {
        ...
        if ($this->RequestHandler->isPost()) {
        	$file_type_list=explode(',',strtolower($file_types));
            $files = $_FILES; 
                ...
                foreach ($this->data as $k => $v) {
                    if (isset($v['file_url']) && is_array($v['file_url'])) {
                        if (isset($v['file_url']['tmp_name']) && !empty($v['file_url']['tmp_name'])) {
                            ...
                            $filename=isset($file_path['filename'])?strtolower($file_path['filename']):'';
                            $file_extension=isset($file_path['extension'])?strtolower($file_path['extension']):'';
                            if (!empty($file_extension)&&in_array($file_extension,$file_type_list)){
                                $Document = $this->Document->find('first', array('conditions' => array('Document.name' => $filename, 'Document.type' => $file_extension)));
                                if (!empty($Document)) {
    ...
    public function ajax_add()
    {
        ...
        if ($this->RequestHandler->isPost()) {
            ...
            if ($files_size > 10 * 1024 * 1024) {
				...
				$file_type_list=explode(',',strtolower(isset($this->configs['files_format']) ? $this->configs['files_format'] : 'txt,pdf,doc,docx,xls,xlsx,zip,rar'));
				...
				$file_extension=isset($file_path['extension'])?strtolower($file_path['extension']):'';
					...
					$result['msg'] = $this->ld['file_format_error'];
					$result['flag'] = 0;
					Configure::write('debug', 0);
					$this->layout = 'ajax';
					die(json_encode($result));

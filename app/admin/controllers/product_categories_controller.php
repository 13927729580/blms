...
//			}
    ...
    function ajax_category_search(){
		...
		$condition['CategoryProductI18n.locale'] = $this->backend_locale;
		if($category_keyword==""){
        		$condition['CategoryProduct.parent_id'] = '0';
        	}else{
        		$condition['CategoryProductI18n.name like'] = "%{$category_keyword}%";
        	}
		$category_info = $this->CategoryProduct->find('all', array('conditions' => $condition,'fields'=>'CategoryProduct.id,CategoryProductI18n.name'));
		if(!empty($category_info)){
			...
			$sub_condition=array();
			$sub_condition['CategoryProduct.status'] = '1';
			$sub_condition['CategoryProductI18n.locale'] = $this->backend_locale;
			$sub_condition['CategoryProduct.parent_id'] = $category_ids;
			$sub_category_info=$this->CategoryProduct->find('all', array('conditions' => $sub_condition,'fields'=>'CategoryProduct.id,CategoryProduct.parent_id,CategoryProductI18n.name'));
			if(!empty($sub_category_info)){
				...
				$child_condition['CategoryProduct.status'] = '1';
				$child_condition['CategoryProductI18n.locale'] = $this->backend_locale;
				$child_condition['CategoryProduct.parent_id'] = $sub_category_ids;
				$child_category_info=$this->CategoryProduct->find('all', array('conditions' => $child_condition,'fields'=>'CategoryProduct.id,CategoryProduct.parent_id,CategoryProductI18n.name'));
				...
				if(!empty($child_category_info)){
					foreach($child_category_info as $v){
						...
						$child_category_datas[$v['CategoryProduct']['parent_id']][]=$v;
					}
				}
				foreach($sub_category_info as $k=>$v){
					$v['SubCategory']=isset($child_category_datas[$v['CategoryProduct']['id']])?$child_category_datas[$v['CategoryProduct']['id']]:array();
					$sub_category_datas[$v['CategoryProduct']['parent_id']][]=$v;
				}
				foreach($category_info as $k=>$v){
					if(in_array($v['CategoryProduct']['id'],$sub_category_ids)||in_array($v['CategoryProduct']['id'],$child_category_ids)){
						unset($category_info[$k]);
						continue;
					}
					$category_info[$k]['SubCategory']=isset($sub_category_datas[$v['CategoryProduct']['id']])?$sub_category_datas[$v['CategoryProduct']['id']]:array();
				}
			}
			$result['flag'] = 1;

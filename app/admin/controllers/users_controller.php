...
class UsersController extends AppController
{
    ...
    public $uses = array('UserRelationship','Product','Brand','UserContact','ConfigI18n','Resource','User','UserAddress','UserBalanceLog','UserPointLog','Application','OperatorLog','UserChat','UserLike','UserFan','UserMessage','UserVisitors','Blog','UserAction','Comment','SynchroUser','UserRank','UserRankLog','Operator','ScoreLog','Profile','ProfileFiled','UserConfig','NotifyTemplateType','UserCategory','UserActionLog','UserShareLog','Order');
    ...
    public function index($page = 1)
    {
        ...
        if(isset($_REQUEST['category_id'])&&!empty($_REQUEST['category_id'])){
            $condition['User.category_id'] = $_REQUEST['category_id'];
            $this->set('category_id', $_REQUEST['category_id']);
        }
       if(isset($_REQUEST['rank'])&& $_REQUEST['rank'] !=""){
            $condition['User.rank'] = $_REQUEST['rank'];
            $this->set('rank', $_REQUEST['rank']);
        }

        if($this->operator_privilege('users_advanced',false)){
            if (isset($_REQUEST['user_manager']) && !empty($_REQUEST['user_manager'])) {
                //pr($_REQUEST['user_manager']);exit();
                $user_manager=$_REQUEST['user_manager'];
                $condition['and']['User.operator_id']=$user_manager;
                $this->set('user_manager',$user_manager);
            }
        }else{
            $condition['User.operator_id']=$this->admin['id'];
        }
        if (constant('Product') == 'AllInOne')$this->loadModel('OrderProduct');
        //商品品牌搜索
        $band_arr = array();
        if (constant('Product') == 'AllInOne'&&isset($_REQUEST['band_id']) && $_REQUEST['band_id'] != '') {
            $band_arr_id = explode(',', $_REQUEST['band_id']);
            if (in_array('-1', $band_arr_id)) {
                $this->set('unknow_brand',-1);
                ...
                $band_arr = $brand_ids_array;
            }else{
            	$band_arr = $band_arr_id;
            }
            $user_brand_list = $this->OrderProduct->find('all', array('fields' => array('Order.user_id'),'conditions' => array('Product.brand_id' => $band_arr_id)));
            $user_ids=array();
            if(!empty($user_brand_list)){
            	foreach($user_brand_list as $k=>$v){
	            	$user_ids[]=$v['Order']['user_id'];
	            }
            }
            $condition['User.id']=array_unique($user_ids);
        }
        $this->set('band_arr', $band_arr);
        if(constant('Product') == 'AllInOne'&&(isset($_REQUEST['order_price_min'])&&!empty($_REQUEST['order_price_min'])) || (isset($_REQUEST['order_price_max'])&&!empty($_REQUEST['order_price_max']))){
            $cons = array();
            if(!empty($_REQUEST['order_price_min'])){
                $cons['Order.total >='] = $_REQUEST['order_price_min'];
            }
            if(!empty($_REQUEST['order_price_max'])){
                $cons['Order.total <='] = $_REQUEST['order_price_max'];
            }
            $user_list = $this->OrderProduct->find('all', array('fields' => array('Order.user_id'),'conditions' => $cons));
            $user_ids=array();
            if(!empty($user_list)){
            ...
            $condition['User.id']=array_unique($user_ids);
            $this->set('order_price_min', $_REQUEST['order_price_min']);
            $this->set('order_price_max', $_REQUEST['order_price_max']);
        }
        if(constant('Product') == 'AllInOne'&&isset($_REQUEST['order_manager'])&&!empty($_REQUEST['order_manager'])){
            $user_list = $this->OrderProduct->find('all', array('fields' => array('Order.user_id'),'conditions' => array('Order.order_manager' => $_REQUEST['order_manager'])));
            $user_ids=array();
            if(!empty($user_list)){
            	foreach($user_list as $k=>$v){
	            	$user_ids[]=$v['Order']['user_id'];
        ...
        $UserRank_data=$this->UserRank->find('all',array('fields'=>"UserRank.id,UserRankI18n.name"));
        $this->set('UserRank_data',$UserRank_data);
     //   pr($UserRank_data);
    ...
    public function user_status()
    {
        ...
        if (!empty($user_info)) {
            ...
            $totify_template=$this->NotifyTemplateType->typeformat($mailtemplete_code);
    ...
    public function view($id=0)
    {
        ...
        $user_info = $this->User->find('first', array('conditions' => $condition));//会员基本信息
        if(empty($user_info)){
            $this->navigations[] = array('name' => $this->ld['add_user'],'url' => '');
            $this->set('title_for_layout', $this->ld['add_user'].'-'.$this->ld['users_search'].' - '.$this->configs['shop_name']);
        }else{
        ...
        $R_info = $this->Resource->getformatcode(array('order_product_status'), $this->backend_locale);
        $this->set('R_info', $R_info);
        ...
        if ($this->RequestHandler->isPost()) {
            ...
            if(trim($this->data['User']['name'])=='')$this->data['User']['name']=$this->data['User']['user_sn'];
            //操作员日志
            ...
            */
            ...
            if (!empty($_POST['balance']) && is_numeric($_POST['balance'])) {
                if ($_POST['balance_type']) {
                    ...
                    $this->notify_balance($id,$_POST['balance']);
                } else {
                    $BalanceLog['UserBalanceLog']['user_id'] = $id;
                    $BalanceLog['UserBalanceLog']['amount'] = '-'.$_POST['balance'];
                    $BalanceLog['UserBalanceLog']['admin_user'] = $this->admin['name'];
            ...
            /* 积分 */
            if (isset($this->configs['use_point'])&&$this->configs['use_point']=='1'&&!empty($_POST['point']) && is_numeric($_POST['point'])) {
                if ($_POST['point_type']) {
                    $PointLog['UserPointLog']['user_id'] = $id;
                    $PointLog['UserPointLog']['point'] = isset($user_info['User']['point'])?$user_info['User']['point']:0;
                    $PointLog['UserPointLog']['point_change'] = $_POST['point'];
        ...
        if ($this->System->checkmodule('B2C')) {
		$this->loadModel('Order');
		...
		$product_list = $this->OrderProduct->find('all',array('conditions'=>array('Order.user_id'=>$id,'Order.user_id >'=>0)));
        ...
        //pr($id);
        $user_id = $id;
        $my_share_logs = $this->UserShareLog->find('all', array('conditions'=>array('UserShareLog.user_id'=>$user_id),'order'=>'UserShareLog.created desc'));
        //pr($my_share_logs);
        ...
        */
        ...
        $this->loadModel('UserWork');$this->loadModel('UserExperience');$this->loadModel('UserEducation');$this->loadModel('UserTaskLog');
        $this->loadModel('UserCourseClass');$this->loadModel('UserEvaluationLog');
        $work_list = $this->UserWork->find('all', array('conditions' => array('UserWork.user_id' => $id)));
        $master = $this->UserRelationship->find('all', array('conditions' => array('UserRelationship.user_id' => $id,'UserRelationship.status' =>1)));
        $apprentice = $this->UserRelationship->find('all', array('conditions' => array('UserRelationship.parent_user_id' => $id,'UserRelationship.status' =>1)));
		$apprentice_data=$master_data=array();
		if(!empty($apprentice)){
			foreach($apprentice as $kk=>$vv){
				$apprentice_info=$this->User->find('all', array('conditions' => array("User.id"=>$vv['UserRelationship']['user_id'])));
		...
		if(!empty($master)){
			foreach($master as $k=>$v){
				$master_info=$this->User->find('all', array('conditions' => array("User.id"=>$v['UserRelationship']['parent_user_id'])));
				$master_data[$k]['id']=$v['UserRelationship']['id'];
				$master_data[$k]['user_id']=$master_info[0]['User']['id'];
				$master_data[$k]['name']=$master_info[0]['User']['name'];
				$master_data[$k]['img']=$master_info[0]['User']['img01'];
        ...
        $experience_list = $this->UserExperience->find('all', array('conditions' => array('UserExperience.user_id' => $id)));
        $education_list = $this->UserEducation->find('all', array('conditions' => array('UserEducation.user_id' => $id)));
        $task_list = $this->UserTaskLog->task_log_infos($id);
        $course_list = $this->UserCourseClass->find('all', array('fields'=>"UserCourseClass.*,Course.*",'conditions' => array('UserCourseClass.user_id' => $id)));
        if(!empty($course_list)){
        	$user_course_ids=array();
        	foreach($course_list as $v){
        		$user_course_ids[]=$v['UserCourseClass']['id'];
        	}
        	$this->loadModel('UserCourseClassDetail');
        	$user_course_details=$this->UserCourseClassDetail->find('all',array('fields'=>"UserCourseClassDetail.user_course_class_id,count(*) read_count",'conditions'=>array('UserCourseClassDetail.user_course_class_id'=>$user_course_ids)));
        	$user_course_detail_data=array();
        	foreach($user_course_details as $v){
        		$user_course_detail_data[$v['UserCourseClassDetail']['user_course_class_id']]=$v[0]['read_count'];
        	}
        	foreach($course_list as $k=>$v){
        		$course_list[$k]['UserCourseClass']['readcount']=isset($user_course_detail_data[$v['UserCourseClass']['id']])?$user_course_detail_data[$v['UserCourseClass']['id']]:0;
        	}
        }
        $evaluation_list = $this->UserEvaluationLog->find('all', array('conditions' => array('UserEvaluationLog.user_id' => $id)));
        $contact_list = $this->UserContact->find('all', array('conditions' => array('UserContact.user_id' => $id)));
        if(!empty($contact_list)){
        	foreach($contact_list as $con_k=>$con_v){
        		$con_operator = $this->Operator->find('first', array('conditions' => array('Operator.id' => $con_v['UserContact']['operator_id'])));
        		$contact_list[$con_k]['UserContact']['operator_name']=isset($con_operator['Operator']['name'])?$con_operator['Operator']['name']:"";
        	}
        }
        ...
        $this->set('master', $master_data);
        $this->set('apprentice', $apprentice_data);
        $this->set('work_list', $work_list);
        $this->set('experience_list', $experience_list);
        $this->set('education_list', $education_list);
        
        if(isset($this->configs['enable_user_log'])&&$this->configs['enable_user_log']=='1'){
        	$user_action_list=$this->UserActionLog->find('all',array('conditions'=>array('UserActionLog.user_id'=>$id),'order'=>'UserActionLog.modified desc'));
        	$this->set('user_action_list', $user_action_list);
        }
    }
    ...
    public function useraddress($user_id = 0, $type = 'list')
    {
        if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
            ...
            if ($type == 'list') {
                ...
                $user_addressInfo = $this->UserAddress->find('first', array('conditions' => array('UserAddress.id' => $addr_id,'UserAddress.user_id'=>$user_id)));
    ...
    */
    ...
    public function user_point_list($user_id = 0, $page = 1)
    {
        if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
            ...
            $this->Pagination->init($condition, $parameters, $options);
            $data = $this->UserPointLog->find('all', array('conditions' => $condition, 'page' => $page, 'limit' => $rownum, 'order' => 'UserPointLog.id desc'));
    ...
    public function add()
    {
        ...
        if ($this->RequestHandler->isPost()) {
            ...
            $this->data['User']['operator_id'] = $this->admin['id'];
    ...
    public function remove($id)
    {
        ...
        if (constant('Product') == 'AllInOne') {
            $this->loadModel('Order');
            $orders = $this->Order->find('first', array('conditions' => array('Order.user_id' => $id)));
            if(!empty($orders)){
                die(json_encode(array('flag' => 2, 'message' => "该用户已有订单，禁止删除")));
            }
        }
        $user_info=$this->User->findbyId($id);
        if(!empty($user_info)){
	        $this->User->user_remove($id);
	        //操作员日志
	        if (isset($this->configs['operactions-log']) && $this->configs['operactions-log'] == 1) {
    ...
    public function batch_operations()
    {
        ...
        if (sizeof($user_ids) > 0) {
            $this->User->user_remove($user_ids);
    ...
    public function order_search_user_information($page = 1)
    {
        ...
        $user_list = $this->User->find('all', array('conditions' => $condition, 'fields' => 'User.id,User.user_sn,User.name,User.address_id,User.mobile,User.email,User.first_name,User.operator_id'));
        if(!empty($user_list)){
            $user_address_ids=array();
            $user_operator_ids=array();
            foreach($user_list  as $v){
                $user_address_ids[]=$v['User']['address_id'];
                $user_operator_ids[]=$v['User']['operator_id'];
            }
            $user_address_info = $this->UserAddress->find('list', array('conditions' => array('UserAddress.id' => $user_address_ids), 'fields' => 'UserAddress.id,UserAddress.consignee'));
            $user_operator_info = $this->Operator->find('all', array('conditions' => array('Operator.id' => $user_operator_ids), 'fields' => "Operator.id,Operator.name,Operator.email"));
            $user_operator_list=array();
            foreach($user_operator_info as $v){
                $user_operator_list[$v['Operator']['id']]=$v['Operator'];
            }
            foreach ($user_list as $k => $v) {
                $user_list[$k]['User']['consignee'] = isset($user_address_info[$v['User']['address_id']])?$user_address_info[$v['User']['address_id']]:'';
                $user_list[$k]['Operator'] = isset($user_operator_list[$v['User']['operator_id']])?$user_operator_list[$v['User']['operator_id']]:array();
            }
        }
        Configure::write('debug', 0);
        $result['type'] = '0';
    ...
    public function balance_index($page = 1)
    {
        ...
        $log_list = $this->UserBalanceLog->find('all', array('conditions' => $condition, 'order' => 'UserBalanceLog.created desc','page'=>$page,'limit'=>$rownum));
...
//				}
                foreach ($fields_array as $kk => $vv) {
                    ...
                    if (isset($order_products_flag) && $order_products_flag == 1 && stristr($vv, 'Order.')) {
                        ...
                        if($vv == 'User.password'){//密码数据处理
                            $datas[] = '';
                        }else if ($vv == 'Order.type') {
                        	...
                        	$data_value=isset($v[$fields_kk[0]][$fields_kk[1]]) ? $v[$fields_kk[0]][$fields_kk[1]] : '';
                        	$data_value=preg_replace_callback('/./u',function (array $match) {return strlen($match[0]) >= 4 ? '' : $match[0];},$data_value);
                            $datas[] =$data_value;
                        }
                    }
                }
    ...
    public function user_type()
    {
        ...
        if ($this->System->checkmodule('B2C','ThirdParty')){
    ...
    public function batch_add_user()
    {
        if (!empty($this->data)) {
            ...
            foreach ($this->data as $key => $data) {
                ...
                if (trim($data['User']['mobile']) == ''||trim($data['User']['email']) == '') {
                    continue;
                }
                $info = array();
                if (trim($data['User']['mobile']) != '') {
                    $info = $this->User->find('first', array('conditions' => array('User.mobile' => trim($data['User']['mobile']))));
                }
                if (empty($info)&&trim($data['User']['email']) != '') {
                    $info = $this->User->find('first', array('conditions' => array('User.email' => trim($data['User']['email']))));
                }
                if (empty($info)) {
                    $info = $this->User->find('first', array('conditions' => array('User.name' => $data['User']['name'])));
                }
                ...
                if (!empty($info)) {
                    $user_data['User']['id']=$info['User']['id'];
                    ++$j;
                } else {
                    $user_data['User']['id']=0;
                    if(empty($user_data['User']['password'])){//初始密码错处理
                        $password_defult=isset($this->configs['password-defult'])?md5($this->configs['password-defult']):md5('123456');
                        $user_data['User']['password']=$password_defult;
                    }
                    ++$i;
                }
                $this->User->saveAll($user_data['User']);
    ...
    */
    ...
    function user_manager_change(){
        ...
        if($this->RequestHandler->isPost()){
            ...
            if(!empty($user_info)){
                ...
                //操作员日志
                if (isset($this->configs['operactions-log']) && $this->configs['operactions-log'] == 1) {
                    $this->OperatorLog->log(date('H:i:s').' '.$this->ld['operator'].' '.$this->admin['name'].' '.$this->ld['edit_member'].$user_info['User']['name'].$this->ld['user_manager'].":".$operator_id, $this->admin['id']);
                    $result['code'] = 1;
    ...
    function notify_balance($user_id,$charge_amount){
        ...
        if(!empty($user_data)){
            ...
            $notify_template_info=$this->NotifyTemplateType->typeformat("user_recharge","wechat");
	...
	function ajax_modify(){
        ...
        $result=array();
        $result['code']='0';
        $this->data['UserContact']['operator_id']=$this->admin['id'];
        $result['message']=$this->ld['modify_failed'];
        $this->UserContact->save($this->data);
        $result['code']='1';
        $result['message']='操作成功';
        die(json_encode($result));
    }

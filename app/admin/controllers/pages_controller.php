...
class PagesController extends AppController
{
    ...
    */
    function ajax_system_list(){
    		...
    		if (isset($_SESSION['dev']) && $_SESSION['dev'] == 1) {
    			$SystemList=$this->System->find('all',array('order'=>'System.id'));
    			$result['code']='1';
    			$result['message']=$SystemList;
    		}
    		die(json_encode($result));
    }
    
    function modified_system(){
    		Configure::write('debug', 1);
    		$this->layout='ajax';
    		...
    		$result['message']=$this->ld['have_no_operation_perform'];
    		if (isset($_SESSION['dev']) && $_SESSION['dev'] == 1) {
    			$system_id=isset($_POST['id'])?intval($_POST['id']):0;
    			$system_status=isset($_POST['status'])?$_POST['status']:0;
    			$this->System->save(array('id'=>$system_id,'status'=>$system_status));
    			$result['code']='1';
    ...
    function dashboard_quotation(){
    		...
    		$this->loadModel('SystemResource');
    		$condition=array();
    		if(!$this->operator_privilege('users_advanced',false)){
    			$condition['Quote.quoted_by']=$this->admin['name'];
    		}else{
    			$condition['Quote.quoted_by <>']='';
    		}
    		$quote_infos=$this->Quote->find('all',array('fields'=>'Quote.quoted_by,Quote.status,count(*) as quote_count','conditions'=>$condition,'group'=>'Quote.quoted_by,Quote.status','order'=>'Quote.quoted_by,Quote.status'));
    		$this->set('quote_infos',$quote_infos);
    		
    		$systemresource_info = $this->SystemResource->resource_formated(array("quote_status"),$this->backend_locale);
		$this->set('quote_status',isset($systemresource_info['quote_status'])?$systemresource_info['quote_status']:array());
    }
    ...
    function dashboard_customer(){
    		...
    		$condition=array();
    		if(!$this->operator_privilege('users_advanced',false)){
    			$condition['User.operator_id']=$this->admin['name'];
    		}else{
    			$condition['User.operator_id >']=0;
    		}
    		$user_infos=$this->User->find('all',array('fields'=>'User.operator_id,count(*) as user_count','conditions'=>$condition,'group'=>'User.operator_id','order'=>'User.operator_id'));
    		if(!empty($user_infos)){
    			$operator_ids=array();
    			foreach($user_infos as $v)$operator_ids[]=$v['User']['operator_id'];
    			$operator_list=$this->Operator->find('list',array('fields'=>'id,name','conditions'=>array('Operator.id'=>$operator_ids)));
    			$this->set('user_infos',$user_infos);

...
class EmailListsController extends AppController
{
    ...
    public $uses = array('MailTemplate','MailTemplateI18n','Resource','UserRank','NewsletterList','MailSendQueue','User','OperatorLog','UserGroup','SmsSendHistory');
        ...
        if ($usermode == 'newsletter_user') {
            ...
            foreach ($newsletterlist_data as $k => $v) {
        	  ...
        	  $text_body=str_replace('$unsubscribe_link',$unsubscribe_link,$text_body);
                ...
                $this->Notify->phpmailer_send($mailsendqueue, $this->configs);
            }
            $mailpdate = array(
    ...
    { 
        $shop_name = isset($this->configs['shop_name'])?$this->configs['shop_name']:"";//template
        if ($usermode == 'newsletter_user') {
            $condition['status'] = '1';
            $condition['mobile <>'] = '';
            $newsletterlist_data = $this->NewsletterList->find('list', array('fields'=>array('NewsletterList.id','NewsletterList.mobile'),'conditions' => $condition));
            $message = $sms_body;
            foreach ($newsletterlist_data as $k => $v) {
                if (trim($v)!= '') {
			$sms_kanal=isset($this->configs['sms_kanal'])?$this->configs['sms_kanal']:'0';
			$sms_result=$this->Notify->send_sms($v,$message,$sms_kanal,$this->configs);
                }
            }
        } elseif ($usermode == 'user_all') {
            $condition['status'] = '1';
            $condition['email_flag'] = '1';
            $condition['mobile <>'] = '';
            $user_data = $this->User->find('list', array('fields'=>array('User.id','User.mobile'),'conditions' => $condition));
            $message = $sms_body;
            foreach ($user_data as $k => $v) {
                $shop_name = $this->configs['shop_name'];//template
                if (trim($v) != '') {
    ...
    public function send_email_test($email='', $mailid='')
    {
        ...
        if ($this->RequestHandler->isPost()) {
        	 ...
        	 $unsubscribe_link=$this->server_host;
        	 $html_body=str_replace('$unsubscribe_link',$unsubscribe_link,$html_body);
        	  $text_body=str_replace('$unsubscribe_link',$unsubscribe_link,$text_body);
	        ...
	        if($email!=""){
		        ...
		        $this->MailSendQueue->saveAll(array('MailSendQueue' => $mailsendqueue));//保存邮件队列
		        $mail_flag=$this->Notify->phpmailer_send($mailsendqueue, $this->configs);
		        if($mail_flag){
		        	$result['code']='1';
		        }
	        }
    ...
    public function send_sms_test($phone='', $mailid='')
    {
        ...
        if ($this->RequestHandler->isPost()) {
	        ...
	        if($mobile!=""){
	        	 $send_flag=$this->sms_send($mobile,$sms_body);
		        if($send_flag){
		        	$result['code']='1';
		        }
	        }

...
class ArticlesController extends AppController
{
    ...
    public function index($page = 1)
    {
        ...
        if (isset($this->params['url']['article_cat']) && $this->params['url']['article_cat'] != 0) {
            ...
            $category_ids = $this->CategoryArticle->find('list',array('conditions'=>array('CategoryArticle.parent_id'=>$category_id),'fields'=>'CategoryArticle.id'));
            if(!empty($category_ids)){
            	$sub_category_ids = $this->CategoryArticle->find('list',array('conditions'=>array('CategoryArticle.parent_id'=>$category_ids),'fields'=>'CategoryArticle.id'));
        ...
        if (isset($this->params['url']['title']) && $this->params['url']['title'] != '') {
            $condition['or']['ArticleI18n.title LIKE'] = '%'.$this->params['url']['title'].'%';
            $condition['or']['ArticleI18n.subtitle LIKE'] = '%'.$this->params['url']['title'].'%';
            $condition['or']['ArticleI18n.meta_description LIKE'] = '%'.$this->params['url']['title'].'%';
            $condition['or']['ArticleI18n.meta_keywords LIKE'] = '%'.$this->params['url']['title'].'%';
            $condition['or']['ArticleI18n.content LIKE'] = '%'.$this->params['url']['title'].'%';
            $this->set('titles', $this->params['url']['title']);
        }
        if (isset($this->params['url']['start_date']) && $this->params['url']['start_date'] != '') {
            $condition['Article.created >='] = date("Y-m-d 00:00:00",strtotime($this->params['url']['start_date']));
        ...
        if (isset($this->params['url']['end_date']) && $this->params['url']['end_date'] != '') {
            $condition['Article.created <='] = date("Y-m-d 23:59:59",strtotime($this->params['url']['end_date']));
...
//				}	
        ...
        $category_tree = $this->CategoryProduct->tree('P','all', $this->backend_locale);//商品分类树
        ...
        //资源库信息
        $Resource_info = $this->Resource->getformatcode(array('sub_type','article_tamplate'), $this->backend_locale, false);//资源库信息
        $this->set('Resource_info', $Resource_info); //pr($Resource_info);
    ...
    function ajax_article_search(){
    		Configure::write('debug', 0);
        	$this->layout = 'ajax';
        	$result['content'] = array();
        	$result['flag'] = 2;
        	$article_keyword = empty($_REQUEST['article_keyword']) ? '' : trim($_REQUEST['article_keyword']);//关键字
        	$condition=array();
        	$condition['Article.status'] = '1';
		$condition['ArticleI18n.locale'] = $this->backend_locale;
		$condition['ArticleI18n.title like'] = "%$article_keyword%";
		$article_info = $this->Article->find('all', array('conditions' => $condition,'fields'=>'Article.id,ArticleI18n.title'));
		if(!empty($article_info)){
			$result['flag'] = 1;
            	$result['content'] = $article_info;
		}

...
class ConfigvaluesController extends AppController
{
     ...
     *显示商店的各个内容.
     */
    public function index($group_code = '')
    {
        ...
        $basics = array();
        $group_codes = isset($Resource_info['configvalues'])&&!empty($Resource_info['configvalues'])?array_keys($Resource_info['configvalues']):array();
        $conditions=array();
  //              pr($group_codes);
  	if($group_code!="") {
  	$conditions['Config.group_code']=$group_code;
  	}	else {
  	$conditions['Config.group_code']=$group_codes;
  	}
        $conditions['Config.readonly']='0';
        $conditions['Config.status']='1';
        $none_config_codes=array(
		'shop_description',
		'shop_logo',
		'icp_number',
		'shop_icon',
        ...
        $conditions['not']['Config.code']=$none_config_codes;
//pr($conditions);
        ...
        foreach ($configs as $k => $v) {
            ...
            foreach ($v['ConfigI18n'] as $kk => $vv) {
                ...
                if ($v['Config']['type'] == 'radio' || $v['Config']['type'] == 'checkbox' || $v['Config']['type'] == 'image'|| $v['Config']['type'] == 'multiselect') {
    ...
    public function test_allemail($receiver_emails = '')
    {
        ...
        $result = $this->Notify->send_email($mailsendqueue,$this->configs);
    ...
    public function test_email()
    {
        ...
        $mail_secure = isset($_POST['mail_secure']) ? $_POST['mail_secure'] : '';
        $smtp_pass = isset($_POST['smtp_pass']) ? $_POST['smtp_pass'] : '';
        $email_addr = isset($_POST['email_addr']) ? $_POST['email_addr'] : '';
        
        $this->configs['mail-requires-authorization']=$smtp_auth;
        $this->configs['mail-service']=$mail_service;
        $this->configs['mail-smtp']=$smtp_host;
        $this->configs['mail-port']=$smtp_port;
        $this->configs['mail-account']=$smtp_user;
        $this->configs['mail-password']=$smtp_pass;
        $this->configs['mail-secure']=$mail_secure;
        $mailsendqueue = array(
                'sender_name' => $this->configs['shop_name'],//发送从姓名
                'receiver_email' => $email_addr,//接收人姓名;接收人地址
                'cc_email' => "",//抄送人
                'bcc_email' => "",//抄送人
                'title' => $this->configs['shop_name'],//主题
                'html_body' => $this->ld['test_mail_received_correct'],//内容
                'text_body' => $this->ld['test_mail_received_correct'],//内容
                'sendas' => 'html'
        );
        $result = $this->Notify->phpmailer_send($mailsendqueue,$this->configs);
        die(json_encode($result));
    }
    ...
    public function changehome($route, $keyword = '')
    {
        ...
        if ($route == 'TOPIC') {
            ...
            $condition['Topic.status'] = '1';
            $condition['TopicI18n.title like'] = "%$keyword%";
            $s_info = $this->Topic->find('all', array('conditions' => $condition));
        }
    ...
    function ajax_modified_config(){
		...
		$result['content']='';
		if ($this->RequestHandler->isPost()) {
			if(isset($this->data['ConfigI18n'])&&isset($this->data['ConfigI18n']['value'])){
				$config_id=$this->data['ConfigI18n']['config_id'];
				$config_locale=$this->data['ConfigI18n']['locale'];
				$this->Config->set_locale($config_locale);
				$config_info=$this->Config->find('first',array('conditions'=>array('Config.id'=>$config_id)));
				if(!empty($config_info['Config'])){
					$config_data=$this->data['ConfigI18n'];
					$this->ConfigI18n->save($config_data);
					$last_config_id=$this->ConfigI18n->id;
					$result['flag']='1';
					$result['content']=stripslashes($this->data['ConfigI18n']['value']);
					$result['last_config_id']=$last_config_id;
				}
			}
		}
		die(json_encode($result));

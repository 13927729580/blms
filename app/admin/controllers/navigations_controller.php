...
class NavigationsController extends AppController
{
    ...
    public function view($id = 0, $nav_type = '')
    {
        ...
        //所有专题
        $topic_info = $this->Topic->find('all', array('conditions' => array('Topic.status' => 1, 'TopicI18n.locale' => $this->backend_locale)));
	...
	$this->Navigation->set_locale($this->backend_locale);
        ...
        if ($nextone == 0) {
            ...
            if ($updowm == 'up') {
                $navigation_change = $this->Navigation->find('first', array('conditions' => array('Navigation.orderby <' => $navigation_one['Navigation']['orderby'], 'Navigation.parent_id' => 0, 'Navigation.type' => $position), 'order' => 'orderby desc,Navigation.id', 'limit' => '1'));
            }
            if ($updowm == 'down') {
                $navigation_change = $this->Navigation->find('first', array('conditions' => array('Navigation.orderby >' => $navigation_one['Navigation']['orderby'], 'Navigation.parent_id' => 0, 'Navigation.type' => $position), 'order' => 'orderby asc,Navigation.id', 'limit' => '1'));
            }
        }
        if ($nextone == 'next') {
            $navigation_one = $this->Navigation->find('first', array('conditions' => array('Navigation.id' => $id)));
            if ($updowm == 'up') {
                $navigation_change = $this->Navigation->find('first', array('conditions' => array('Navigation.orderby <' => $navigation_one['Navigation']['orderby'], 'Navigation.parent_id' => $navigation_one['Navigation']['parent_id'], 'Navigation.type' => $position), 'order' => 'orderby desc,Navigation.id', 'limit' => '1'));
            }
            if ($updowm == 'down') {
                $navigation_change = $this->Navigation->find('first', array('conditions' => array('Navigation.orderby >' => $navigation_one['Navigation']['orderby'], 'Navigation.parent_id' => $navigation_one['Navigation']['parent_id'], 'Navigation.type' => $position), 'order' => 'orderby asc,Navigation.id', 'limit' => '1'));
            }
        }
...
//排序下拉框
    ...
    function ajax_modified_navigation(){
    		Configure::write('debug', 1);
        	$this->layout = 'ajax';
        	...
        	$result['content']='';
        	if ($this->RequestHandler->isPost()) {
        		if(isset($this->data['NavigationI18n']['name'])){
        			$navigation_id=isset($this->data['Navigation']['id'])?$this->data['Navigation']['id']:0;
        			$this->Navigation->set_locale($this->backend_locale);
				$navigation_info=$this->Navigation->findById($navigation_id);
				$navigation_data=array(
					'id'=>$navigation_id,
					'status'=>isset($navigation_info['Navigation'])?$navigation_info['Navigation']['status']:'1'
				);
				$this->Navigation->save($navigation_data);
				$navigation_id=$this->Navigation->id;
				$navigation_i18n_data=$this->data['NavigationI18n'];
				$navigation_i18n_data['id']=isset($navigation_info['NavigationI18n'])?$navigation_info['NavigationI18n']['id']:0;
				$navigation_i18n_data['navigation_id']=$navigation_id;
				$navigation_i18n_data['locale']=$this->backend_locale;
				$this->NavigationI18n->save($navigation_i18n_data);
				
				$result['flag']='1';
        			$result['content']=$navigation_i18n_data['name'];
        			$result['last_id']=$navigation_id;
        		}
        	}
        	die(json_encode($result));
    }

...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    */
    public function changeorder($updowm, $id, $nextone, $page = 1)
    {
        ...
        if ($updowm == 'up') {
            $topic_change = $this->Topic->find('first', array('conditions' => array('Topic.orderby <' => $topic_one['Topic']['orderby']), 'order' => 'Topic.orderby desc', 'limit' => '1'));
        }
        if ($updowm == 'down') {
            $topic_change = $this->Topic->find('first', array('conditions' => array('Topic.orderby >' => $topic_one['Topic']['orderby']), 'order' => 'Topic.orderby asc', 'limit' => '1'));
        }
    ...
    function ajax_topic_search(){
    		Configure::write('debug', 0);
        	$this->layout = 'ajax';
        	$result['content'] = array();
        	$result['flag'] = 2;
        	$topic_keyword = empty($_REQUEST['topic_keyword']) ? '' : trim($_REQUEST['topic_keyword']);//关键字
        	$condition=array();
        	$condition['Topic.status'] = '1';
		$condition['TopicI18n.locale'] = $this->backend_locale;
		$condition['TopicI18n.title like'] = "%$topic_keyword%";
		$topic_info = $this->Topic->find('all', array('conditions' => $condition,'fields'=>'Topic.id,TopicI18n.title'));
		if(!empty($topic_info)){
			$result['flag'] = 1;
            	$result['content'] = $topic_info;
		}
		die(json_encode($result));
    } 
}

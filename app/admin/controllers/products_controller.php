...
class ProductsController extends AppController{
    ...
    public function index($page = 1){
        ...
        $recommond_cond_flag=false;
        //索搜 参数 初始化
        $category_id = '';        //分类
        $brand_id = 0;            //品牌
        $product_type_id = 0;    //商品属性
        $category_type_id = 0;    //商品类目
        $is_recommond = '-1';    //推荐
        $forsale = '-1';        //上架
        $operator_id = '-1';        //操作员
        ...
        //类目
        if (isset($this->params['url']['category_type']) && $this->params['url']['category_type'] != '0') {
            $condition['and']['Product.category_type_id'] = $this->params['url']['category_type'];
            $category_type_id = $this->params['url']['category_type'];
        }
        //属性组
        ...
        if (isset($this->params['url']['operator_id']) && $this->params['url']['operator_id'] != '-1') {
            ...
            $recommond_cond_flag=true;
        }
        //产品管理员
        if (isset($this->params['url']['product_manager']) && $this->params['url']['product_manager'] != '-1') {
            $condition['and']['Product.product_manager'] = $this->params['url']['product_manager'];
            $product_manager = $this->params['url']['product_manager'];
            $this->set('product_manager', $product_manager);
        }
        //上架
        ...
        if (isset($this->params['url']['product_keywords']) && $this->params['url']['product_keywords'] != '') {
            ...
            if ($product_keyword != '') {
                ...
                $tag_conditions['and']['Tag.type']="P";
                $tag_conditions['and']['Tag.status']="1";
                foreach ($keyword as $k => $v) {
                	$condition['AND']['OR'][]['Product.code like'] = "%$v%";
                	$tag_conditions['and']['or'][]['TagI18n.name like']= "%$v%";
                    $conditions_p18n['AND']['or'][0]['and'][]['ProductI18n.name like'] = "%$v%";
                    $conditions_p18n['AND']['or'][1]['and'][]['ProductI18n.meta_keywords  like'] = "%$v%";
                }
                $tag_product_info=$this->Tag->find('all',array('fields'=>"Tag.id,Tag.type_id","conditions"=>$tag_conditions));
                if(!empty($tag_product_info)){
                	$tag_product_ids=array();
                	foreach($tag_product_info as $v){
                		$tag_product_ids[]=$v['Tag']['type_id'];
                	}
                }
                $product18n_pid = $this->ProductI18n->find_product18n_pid($conditions_p18n); //model
                if( isset($tag_product_ids) && !empty($tag_product_ids )){
                	$product18n_pid=array_merge($tag_product_ids,$product18n_pid);
                }
                if( isset($product18n_pid) && !empty($product18n_pid )){
        ...
        //库存
        if (isset($this->params['url']['min_quantity']) && $this->params['url']['min_quantity'] != '') {
        	$condition['and']['Product.quantity >='] = intval($this->params['url']['min_quantity']);
        	$this->set('min_quantity', intval($this->params['url']['min_quantity']));
        }
        if (isset($this->params['url']['max_quantity']) && $this->params['url']['max_quantity'] != '') {
        	$condition['and']['Product.quantity <='] = intval($this->params['url']['max_quantity']);
        	$this->set('max_quantity', intval($this->params['url']['max_quantity']));
        }
        //添加时间
		...
		$condition['and']['Product.recommand_flag'] = $is_recommond=='0'?array('0',''):'1';
		$recommond_cond_flag=false;
        }
        //商品分类搜索
        $category_arr = array();
        ...
        if (isset($_REQUEST['attr_cate_id']) && $_REQUEST['attr_cate_id'] != ''&& $_REQUEST['attr_cate_id'] != '0') {
        ...
        //Cross Reffer(关联搜索)
	  if(isset($this->configs['cross_reference'])&&$this->configs['cross_reference']=='1'&&isset($this->params['url']['product_cross_reference'])&&$this->params['url']['product_cross_reference']=='1'){
		if(isset($this->params['url']['product_keywords']) && $this->params['url']['product_keywords']!=''){
			$product_cross_reference=$this->params['url']['product_cross_reference'];
			$sku_pro_ids=$this->Product->find('list',array("conditions"=>$condition,"fields"=>"Product.id"));
			$sku_pro_names=$this->ProductI18n->find('list',array("conditions"=>array('ProductI18n.Product_id'=>$sku_pro_ids,'ProductI18n.locale'=>$this->backend_locale),"fields"=>"ProductI18n.name"));
			$sku_pro=$this->ProductI18n->find('list',array("conditions"=>array('ProductI18n.name'=>$sku_pro_names),"fields"=>"ProductI18n.Product_id"));
			$condition['AND']['OR']['Product.id'] =$sku_pro;
		}
		$this->set('product_cross_reference',$product_cross_reference);
	  }
	  //推荐高级搜索条件
	  if(isset($this->configs['recommond_filter'])&&$this->configs['recommond_filter']=='1'){
	  		if ($recommond_cond_flag) {
	  				$filter_cond=$condition;
	  				$condition['or'][0]=$filter_cond;
	  				if(isset($condition['AND']))unset($condition['AND']);
					if(isset($condition['and']))unset($condition['and']);
					$condition['or'][1]=$filter_cond;
					$condition['or'][1]['and']['Product.recommand_flag']=1;
					//操作员
					if(isset($this->params['url']['operator_id']) && $this->params['url']['operator_id']!='-1'){
						unset($condition['or'][1]['and']["Product.operator_id"]);
					}
					//开始价格
					if(isset($this->params['url']['min_price']) && $this->params['url']['min_price']!=''){
						unset($condition['or'][1]['and']["Product.shop_price >="]);
					}
					//结束价格
					if(isset($this->params['url']['max_price']) && $this->params['url']['max_price']!=''){
						unset($condition['or'][1]['and']["Product.shop_price <="]);
					...
					//添加时间
					if(isset($this->params['url']['start_date']) && $this->params['url']['start_date']!=''){
						unset($condition['or'][1]["and"]["Product.created >="]);
					}
					if(isset($this->params['url']['end_date']) && $this->params['url']['end_date']!=''){
						unset($condition['or'][1]["and"]["Product.created <="]);
					}
					//修改时间
					if(isset($this->params['url']['start_date_time']) && $this->params['url']['start_date_time']!=''){
						unset($condition['or'][1]["and"]["Product.last_update_time >="]);
					}
					if(isset($this->params['url']['end_date_time']) && $this->params['url']['end_date_time']!=''){
						unset($condition['or'][1]["and"]["Product.last_update_time <="]);
					}
	  ...
	  //搜索结果导出
        if (isset($this->params['url']['export_act_flag']) && $this->params['url']['export_act_flag'] == 1) {
            ...
            $export_code=isset($this->params['url']['export_code']) && $this->params['url']['export_code']!= ''?$this->params['url']['export_code']:'export_csv';
            if($export_code!='export_csv_by_category'){
            	$this->search_act_result($condition, $order, 'search_result','product_export');
            }else{
            	$this->export_search_result_by_category($condition, $order,'product_categories_export');
            }
        }
        ...
        $fields[] = 'Product.operator_name';//最后编辑人名称
        $fields[] = 'Product.last_update_time';//最后修改时间
        ...
        $fields[]="Product.product_manager"; //商品管理员
        $fields[]="Product.max_buy"; //最大购买数
        $fields[]="Product.is_subscription_send"; //是否发送订阅
        //排序
        //pr($condition);
        $product_order=isset($this->configs['product_order'])&&trim($this->configs['product_order'])!=''?'Product.'.$this->configs['product_order'].' desc':'Product.modified desc';
        $product_order.=isset($this->configs['product_order'])&&trim($this->configs['product_order'])!='id'?",Product.id":'';
        $product_list = $this->Product->find('all', array('conditions' => $condition, 'order' => $product_order, 'fields' => $fields, 'limit' => $rownum, 'page' => $page));
        ...
        if(isset($this->configs['product_location_price'])&&$this->configs['product_location_price']=='1'){
	        ...
	        $this->set('pro_city_price',$pro_city_price);
        }
        ...
        $this->set('category_type_id', $category_type_id);//索搜分类选中
        ...
        $this->set('category_type_tree', $category_type_tree);//类目树
...
*/
    ...
    public function view($id = 0, $page = 1, $category_id = 0){
        ...
        if ($this->RequestHandler->isPost()) {
            ...
            //获取租赁信息
            if(isset($this->data['Product']['lease_status'])){
                $pro_lease = $this->ProductLease->find('first', array('conditions' => array('ProductLease.product_code' => $this->data['Product']['code'])));
                $pro_lease['ProductLease']["product_code"]=$this->data['Product']['code'];
                $pro_lease['ProductLease']["lease_price"]=$this->data['ProductLease']["lease_price"];
                $pro_lease['ProductLease']["lease_deposit"]=$this->data['ProductLease']["lease_deposit"];
                $pro_lease['ProductLease']["unit"]=$this->data['ProductLease']["unit"];
                $this->ProductLease->saveAll(array('ProductLease' => $pro_lease["ProductLease"]));
            }
            //获取商品信息
            $product_info = $this->Product->localeformat($id);
            $this->data['Product']['lease_status'] = isset($this->data['Product']['lease_status']) ? $this->data['Product']['lease_status'] : '0';//租赁
            ...
            if(empty($product_info)||trim($product_info['Product']['forsale_time'])==''){
            		$this->data['Product']['forsale_time'] = date('Y-m-d H:i:s');
            }
            $this->data['Product']['recommand_flag'] = !empty($this->data['Product']['recommand_flag']) ? $this->data['Product']['recommand_flag'] : '0';//加入推荐
            $this->data['Product']['is_subscription_send'] = !empty($this->data['Product']['is_subscription_send']) ? $this->data['Product']['is_subscription_send'] : '0';//是否发送订阅
...
//					}
            ...
            */
            ...
            //多地区价格
            if(!empty($this->data['ProductLocalePrice'])){
            	$pro_price_info = $this->ProductLocalePrice->find("list",array("conditions"=>array("ProductLocalePrice.product_id"=>$id),"fields"=>array("ProductLocalePrice.locale","ProductLocalePrice.id")));
            	foreach($this->data['ProductLocalePrice']['locale'] as $k=>$v){
            		$ProductLocalePrice_data=array(
            			'id'=>isset($pro_price_info[$v])?$pro_price_info[$v]:0,
            			'locale'=>$v,
            			'product_id'=>$id,
            			'product_price'=>isset($this->data['ProductLocalePrice']['product_price'][$k])?$this->data['ProductLocalePrice']['product_price'][$k]:0
            		);
            		$this->ProductLocalePrice->save(array('ProductLocalePrice'=>$ProductLocalePrice_data));
            	}
            	$this->location_product_price_mail($id);
            }
            //淘宝销售渠道保存
...
//							$jingdong_item_data["transport_id"] = 0;//运费模版
            ...
            if(isset($this->configs['enable_product_log'])&&$this->configs['enable_product_log']=='1'){
	            if(isset($product_info['Product']['id'])){
		            $this->ProductAction->update_action(array(
		            		'product_id'=>$id,
		            		'operator_id'=>$this->admin['id'],
		            		'status'=>isset($product_info['Product']['status'])?$product_info['Product']['status']:'1',
		            		'remark'=>$this->ld['edit'].$this->ld['sku'].":".$this->data['Product']['code']
		            ),$this);
	            }else{
	            	$this->ProductAction->update_action(array(
		            		'product_id'=>$id,
		            		'operator_id'=>$this->admin['id'],
		            		'status'=>isset($product_info['Product']['status'])?$product_info['Product']['status']:'1',
		            		'remark'=>$this->ld['add'].$this->ld['sku'].":".$this->data['Product']['code']
		            ),$this);
	            }
            }
        ...
        */
        ...
        if(!empty($product_info)){
        	$lease_data=array();
        	$lease_price=$this->ProductLease->find('first', array('conditions' => array('ProductLease.product_code ' => $product_info['Product']['code'])));
        	if(empty($lease_price)){
        		$lease_price=$this->ProductLeasePrice->find('first', array('conditions' => array('ProductLeasePrice.price <=' => $product_info['Product']['shop_price']),'order'=>"ProductLeasePrice.price DESC"));
        		if(!empty($lease_price)){
        			$lease_data=$lease_price;
        			$lease_data['ProductLeasePrice']["lease_price"]=$product_info['Product']['shop_price']*($lease_price['ProductLeasePrice']['lease_price_percent']/100);
        			$lease_data['ProductLeasePrice']["lease_deposit_base"]=$lease_price["ProductLeasePrice"]["lease_deposit_base"]+($product_info['Product']['shop_price']-$lease_price["ProductLeasePrice"]["price"])*($lease_price['ProductLeasePrice']['lease_deposit_increase_percent']/100);
        		}
        	}else{
        		$lease_data['ProductLeasePrice']["lease_price"]=$lease_price["ProductLease"]["lease_price"];
        		$lease_data['ProductLeasePrice']["lease_deposit_base"]=$lease_price["ProductLease"]["lease_deposit"];
        		$lease_data['ProductLeasePrice']["lease_deposit_unit"]=$lease_price["ProductLease"]["unit"];
        	}
        	$this->set('lease_price', $lease_data);//租赁信息
        }
        //商品重量数字处理
        ...
        //商品属性
        $this->Attribute->locale=$this->backend_locale;
        ...
        */
        ...
        //商品管理员
        $Operator_list = $this->Operator->find('all',array('fields'=>"Operator.id,Operator.name","order"=>"Operator.name"));
        $this->set('Operator_list', $Operator_list);
        //多地区价格
        if(isset($this->configs['product_location_price'])&&$this->configs['product_location_price']=='1'){
	        $systemresource_info = $this->SystemResource->resource_formated(array("product_location"),$this->backend_locale);
	        $pro_price = $this->ProductLocalePrice->find("list",array("conditions"=>array("ProductLocalePrice.product_id"=>$id),"fields"=>array("ProductLocalePrice.locale","ProductLocalePrice.product_price")));
	        $this->set('systemresource_info',$systemresource_info);
	        $this->set('pro_price',$pro_price);
        }
        //淘宝销售渠道
        if (constant('Product') == 'AllInOne') {
            //加载淘宝相关模型
        ...
        if(isset($this->configs['enable_product_log'])&&$this->configs['enable_product_log']=='1'&&!empty($product_info['Product']['id'])){
        	$product_action_list=$this->ProductAction->action_list($product_info['Product']['id']);
        	$this->set('product_action_list',$product_action_list);
        }
    }
    ...
    public function recycle_bin($id)
    {
        ...
        $pn = $this->Product->find('first', array('fields' => array('Product.id','Product.code', 'ProductI18n.name'), 'conditions' => array('Product.product_id' => $id, 'ProductI18n.locale' => $this->backend_locale)));
    	if(isset($this->configs['enable_product_log'])&&$this->configs['enable_product_log']=='1'){
            if(isset($pn['Product']['id'])){
	            $this->ProductAction->update_action(array(
	            		'product_id'=>$id,
	            		'operator_id'=>$this->admin['id'],
	            		'status'=>'2',
	            		'remark'=>$this->ld['move_to_recycle_bin_failure']
	            ),$this);
            }
        }
        //操作员日志
        if ($this->configs['operactions-log'] == 1) {
            $this->OperatorLog->log(date('H:i:s').' '.$this->ld['operator'].' '.$this->admin['name'].' '.$this->ld['log_into_recycle_bin'].':id '.$id.' '.$pn['ProductI18n']['name'], $this->admin['id']);
        }
     ...
     *列表复制一个产品.
     */
    public function copy_product($original_pro_id=0)
    {
        ...
        $MaxProductIdInfo=$this->Product->find('first', array('fields' =>"Max(Product.id) as max_id"));
        $product_id = isset($MaxProductIdInfo[0]['max_id'])?$MaxProductIdInfo[0]['max_id']:0;
        $product_id+=1;
        foreach ($original_pro as $k => $v){
	      $v['Product']['id'] = $product_id;
            $v['Product']['code'] = $this->generate_product_code($product_id);
            ...
            $v['ProductI18n']['id'] = '0';
            $v['ProductI18n']['product_id'] = $product_id;
        ...
        if (isset($this->apps['Applications']['APP-PRODUCTS']) && $this->apps['Applications']['APP-PRODUCTS']['status'] == 1) {
            ...
            if (isset($attr_info)) {
                foreach ($attr_info as $ka => $va) {
                    ...
                    $products['ProductAttribute'][$ka]['product_id'] = $product_id;
                }
                if (isset($products['ProductAttribute'])) {
                    foreach ($products['ProductAttribute'] as $ka => $va) {
        ...
        if(isset($this->configs['enable_product_log'])&&$this->configs['enable_product_log']=='1'){
	        $this->ProductAction->update_action(array(
	        		...
	        		'operator_id'=>$this->admin['id'],
	        		'status'=>'1',
	        		'remark'=>$this->ld['log_copy_product'].isset($v['Product']['code'])?$v['Product']['code']:''
	            ));
    	}
        $back_url = $this->operation_return_url();//获取操作返回页面地址
        $this->redirect($back_url);
    ...
    public function search_act_result($condition = '', $order = '', $out_type = '', $code){
    	  Configure::write('debug', 1);
        $this->layout = 'ajax';
    	  $export_type_location_price=isset($_REQUEST['export_type_location_price'])?$_REQUEST['export_type_location_price']:0;
        //取出所有公共属性
		...
		if(isset($this->configs['product_location_price'])&&$this->configs['product_location_price']=='1'&&$export_type_location_price!='0'){
			$systemresource_info = $this->SystemResource->resource_formated(array("product_location"),$this->backend_locale);
			$product_location=isset($systemresource_info['product_location'])?$systemresource_info['product_location']:array();
		}
            $profilefiled_info = $this->ProfileFiled->find('all', array('fields' => array('ProfileFiled.code', 'ProfilesFieldI18n.description','ProfileFiled.currency_format'), 'conditions' => array('ProfilesFieldI18n.locale' => $this->backend_locale, 'ProfileFiled.profile_id' => $profile_id['Profile']['id'], 'ProfileFiled.status' => 1), 'order' => 'ProfileFiled.orderby asc,ProfileFiled.id'));
            $tmp = array();
            $fields_array = array();
            $profie_fields=array();
            $profie_currency_format=array();
            foreach ($profilefiled_info as $k => $v) {
            	   if(trim($v['ProfileFiled']['currency_format'])!='')$profie_currency_format[$v['ProfileFiled']['code']]=trim($v['ProfileFiled']['currency_format']);
	                $profie_fields[] = $v['ProfileFiled']['code'];
	                if($v['ProfileFiled']['code']=='ProductLocalePrice'){
		                	if(isset($product_location)&&!empty($product_location)){
		                		foreach($product_location as $kk=>$vv){
		                			if($export_type_location_price=='all'||$export_type_location_price==$kk){
		                				$tmp[] = $vv;
				                	}
		                		}
		                	}
	                }else if($v['ProfileFiled']['code']=='PublicAttribute'||$v['ProfileFiled']['code']=='ProductTypeAttribute'){
             ...
             if (in_array('PublicAttribute', $profie_fields)||in_array('ProductTypeAttribute', $profie_fields)) {
	             $attribute_cond=array();
	             $attribute_cond['Attribute.status']='1';
	             $attribute_cond['Attribute.type <>']='customize';
	             $public_attr_ids = $this->ProductTypeAttribute->getattrids(0);
	             if(!in_array('PublicAttribute', $profie_fields)&&!empty($public_attr_ids)){
	             	$attribute_cond['not']['Attribute.id']=$public_attr_ids;
	             }else if(!in_array('ProductTypeAttribute', $profie_fields)&&!empty($public_attr_ids)){
	             	$attribute_cond['Attribute.id']=$public_attr_ids;
	             }
			$this->Attribute->set_locale($this->backend_locale);
			$attribute_info = $this->Attribute->find('all', array('conditions' => $attribute_cond, 'fields' => 'Attribute.id,AttributeI18n.name'));
             }
             $pat = array();$pat_ids = array();
		if (!empty($attribute_info)) {
			foreach ($attribute_info as $k => $p) {
				$pat[$k]['id'] = $p['Attribute']['id'];
				$pat[$k]['name'] = $p['AttributeI18n']['name'];
			}
			foreach ($attribute_info as $pa) {
				$pat_ids[] = $pa['Attribute']['id'];
			}
		}
            $brand_names = array();
            ...
            $product_ids = $this->Product->find('list', array('fields' => array('Product.code', 'Product.id'), 'conditions' => $condition,'recursive'=>-1));
            if(isset($product_location)&&!empty($product_location)){
        		$pro_city_price=$this->ProductLocalePrice->get_pro_city_price_list(array("product_id"=>$product_ids,'fields'=>'ProductLocalePrice.id,ProductLocalePrice.product_id,ProductLocalePrice.locale,ProductLocalePrice.product_price,ProductLocalePrice.status'));
            }
            //取出所有属性
            if(!empty($pat_ids)&&!empty($product_ids)){
        ...
        foreach ($productlist as $k => $v) {
            $product_tmp = array();
            foreach ($profie_fields as $kk => $vv) {
                ...
                } elseif ($vv == 'Product.id'||$vv=='ProductTypeAttribute'||$vv=='PublicAttribute') {
                    continue;
                } elseif($vv=='ProductLocalePrice'){
                		if(isset($product_location)&&!empty($product_location)){
	                		foreach($product_location as $kk=>$vv){
	                			if($export_type_location_price!='all'&&$export_type_location_price!=$kk){
	                				//$product_tmp[] = '';
	                			}else{
	                				$product_tmp[] = isset($pro_city_price[$v['Product']['id']][$kk]['ProductLocalePrice']['product_price']) ? $pro_city_price[$v['Product']['id']][$kk]['ProductLocalePrice']['product_price'] : '0';
	                			}
	                		}
	                	}
                }else {
                		$product_currency_format=isset($profie_currency_format[$vv])?$profie_currency_format[$vv]:'';
                		$product_value=isset($v[$fields_kk[0]][$fields_kk[1]]) ? $v[$fields_kk[0]][$fields_kk[1]] : '';
                    	$product_tmp[] = $product_currency_format!=''?sprintf($product_currency_format, $product_value):$product_value;
                }
            }
//    		if(in_array('APP-PRODUCTS',$this->apps)){
            foreach ($pat as $pp) {
...
//		    }
    ...
    public function export_search_result_by_category($condition = '',$order = '',$code){
    		  Configure::write('debug', 1);
	        $this->layout = 'ajax';
	    	  $export_type_location_price=isset($_REQUEST['export_type_location_price'])?$_REQUEST['export_type_location_price']:0;
	        $this->Profile->hasOne = array();
	        ...
	        $this->Product->set_locale($this->backend_locale);
	        $productlist = $this->Product->find('all', array('conditions' => $condition, 'order' => $order));//'recursive'=>-1
	        $newdatas[] = $tmp;
	        foreach ($productlist as $k => $v) {
	        ...
	        if(isset($data[0])){
		    		$row = array();
				$row[] = $this->ld['is_not_category'];
				$newdatas[] = $row;
				$tmp = $this->category_products_tree($data, 0, $productlist, $fields_array, $pat, $attr_info, $brand_names, $product_type_names,(isset($product_location)?$product_location:array()),(isset($pro_city_price)?$pro_city_price:array()),$profie_currency_format);
				$newdatas = array_merge($newdatas, $tmp);
	        }
	        $category_tree = $this->CategoryProduct->tree('P','all',$this->backend_locale);
	        foreach ($category_tree as $k => $v) {
	                ...
	                if (!empty($data[$v['CategoryProduct']['id']])) {
	                    ...
	                    if (isset($data[$v['CategoryProduct']['id']])) {
	                        $tmp = $this->category_products_tree($data, $v['CategoryProduct']['id'], $productlist, $fields_array, $pat, $attr_info, $brand_names, $product_type_names,(isset($product_location)?$product_location:array()),(isset($pro_city_price)?$pro_city_price:array()),$profie_currency_format);
	                        $newdatas = array_merge($newdatas, $tmp);
	                    }
	                }
	                    ...
	                    foreach ($v['SubCategory'] as $kk => $vv) {
	                        ...
	                        if (!empty($data[$vv['CategoryProduct']['id']])) {
	                            ...
	                            if (isset($data[$vv['CategoryProduct']['id']])) {
	                                $tmp = $this->category_products_tree($data, $vv['CategoryProduct']['id'], $productlist, $fields_array, $pat, $attr_info, $brand_names, $product_type_names,(isset($product_location)?$product_location:array()),(isset($pro_city_price)?$pro_city_price:array()),$profie_currency_format);
	                                $newdatas = array_merge($newdatas, $tmp);
	                            }
	                        }
	                            ...
	                            foreach ($vv['SubCategory'] as $kkk => $vvv) {
	                                ...
	                                if (!empty($data[$vvv['CategoryProduct']['id']])) {
	                                    ...
	                                    if (isset($data[$vvv['CategoryProduct']['id']])) {
	                                        $tmp = $this->category_products_tree($data, $vvv['CategoryProduct']['id'], $productlist, $fields_array, $pat, $attr_info, $brand_names, $product_type_names,(isset($product_location)?$product_location:array()),(isset($pro_city_price)?$pro_city_price:array()),$profie_currency_format);
	                                        $newdatas = array_merge($newdatas, $tmp);
	                                    }
	                                }
	                            }
        ...
        $this->Phpexcel->output('search_result'.date('YmdHis').'.xls', $newdatas);
        die();
    }
    
    public function category_products_tree($data, $category_id, $p, $fields_array, $pat = array(), $attr_info = array(), $brand_names, $product_type_names,$product_location,$pro_city_price,$profie_currency_format=array())
    {
    	  $export_type_location_price=isset($_REQUEST['export_type_location_price'])?$_REQUEST['export_type_location_price']:0;
        $products = array();
        if (isset($data) && is_array($data)) {
            foreach ($data[$category_id] as $k => $v) {
                $product = array();
                $product[] = '';
                foreach ($fields_array as $kk => $vv) {
                    ...
                    if ($vv == 'Product.brand_id') {
                    ...
                    }else if($vv == 'PublicAttribute'||$vv == 'ProductTypeAttribute'){
                    	
                    }elseif($vv == 'ProductLocalePrice'){
                    	if(!empty($product_location)){
	                		foreach($product_location as $kkk=>$vvv){
	                			if($export_type_location_price!='all'&&$export_type_location_price!=$kkk){
	                				//$product[] = '';
	                			}else{
	                				$product[] = isset($pro_city_price[$v['Product']['id']][$kkk]['ProductLocalePrice']['product_price']) ? $pro_city_price[$v['Product']['id']][$kkk]['ProductLocalePrice']['product_price'] : '0';
	                			}
	                		}
	                	}
                    }else {
                    	$product_currency_format=isset($profie_currency_format[$vv])?$profie_currency_format[$vv]:'';
                		$product_value=isset($v[$fields_kk[0]][$fields_kk[1]]) ? $v[$fields_kk[0]][$fields_kk[1]] : '';
                    	$product[] = $product_currency_format!=''?sprintf($product_currency_format, $product_value):$product_value;
                    }
                }
                if (!empty($pat)) {
                    foreach ($pat as $pp) {
    ...
    public function batch_operations($type){
        ...
        $product_checkboxes = array();
        //pr($type);exit();
        if ($type != 'export_csv'&&$type!= 'export_csv_by_category') {
            $product_checkboxes = isset($_REQUEST['checkboxes'])?$_REQUEST['checkboxes']:array();
            foreach ($product_checkboxes as $k => $v) {
                ...
                if ($type == 'transfer_category') {
						...
						'operator_id'=>$this->admin['id'],
						'status'=>'1',
						'remark'=>$this->ld['log_edit_product_category']
					));
				}
                }
                ...
                if ($type == 'batch_notsale') {
                    ...
                    if(isset($this->configs['enable_product_log'])&&$this->configs['enable_product_log']=='1'){
						...
						'operator_id'=>$this->admin['id'],
						'status'=>'1',
						'remark'=>$this->ld['batch_notsale']
					));
			}
                }
                // 修改发送订阅邮件标志位
                if ($type == 'batch_set_subscription') {
                    $this->Product->updateAll(array('is_subscription_send' => isset($_POST['subscription_send_flag'])?$_POST['subscription_send_flag']:0), array('id' => $v));
                    if(isset($this->configs['enable_product_log'])&&$this->configs['enable_product_log']=='1'){
                        $this->ProductAction->update_action(array(
	                        'product_id'=>$v,
	                        'operator_id'=>$this->admin['id'],
	                        'status'=>'1',
	                        'remark'=>$this->ld['batch_set_subscription']
	                    ));
            	      }
                }
            }
            ...
            // 修改
            if($type == 'barch_subscription'){
                if ($this->configs['operactions-log'] == 1) {
                    $this->OperatorLog->log(date('H:i:s').' '.$this->ld['operator'].' '.$this->admin['name'].' '.$this->ld['batch_set_subscription'], $this->admin['id']);
                }
            }
        	...
        	$export_type_code='product_export';
        	$export_csv=$_REQUEST['export_csv'];
        	$this->exprot_out($_REQUEST['export_csv'], $_REQUEST['export_type'], $_REQUEST['export_type_re'], $export_type_code);
        }else if($type == 'export_csv_by_category'){
        	$export_type_code='product_categories_export';
        	$export_csv=$_REQUEST['export_csv'];
        	if($export_csv=='choice_export'){
        		$this->export_by_check_category($_REQUEST['export_type'], $_REQUEST['export_type_re']);
    		}else{
...
//			}
    ...
    public function update_product_attr()
    {
        ...
        $val = $_REQUEST['val'];
        $product_attr_info = $this->ProductAttribute->find('first', array('conditions' => array('ProductAttribute.product_id' => $id[0], 'ProductAttribute.attribute_id' => $id[1], 'ProductAttribute.locale' => $this->backend_locale)));
        if (isset($product_attr_info['ProductAttribute']['id'])) {
            $request = $this->ProductAttribute->updateAll(
                array('ProductAttribute.attribute_value' => "'".$val."'"),
                array('ProductAttribute.product_id' => $id[0], 'ProductAttribute.attribute_id' => $id[1], 'ProductAttribute.locale' => $this->backend_locale)
            );
        } else {
    ...
    public function update_recommand_flag(){
        $this->operator_privilege('products_edit');
        Configure::write('debug', 0);
        $this->layout = 'ajax';
        $this->Product->hasMany = array();
        ...
        $result = array();
        $result['flag'] = 2;
        $product_info = $this->Product->find('first', array('conditions' => array('id' => $id), 'fields' => 'id,code,status,recommand_flag'));
        if(!empty($product_info)){
            $result['flag'] = 1;
            $result['content'] = stripslashes($val);
            $product_data = array(
            ...
            $this->ProductAction->update_action(array(
        		'product_id'=>$id,
        		'operator_id'=>$this->admin['id'],
        		'status'=>$product_info['Product']['status'],
        		'remark'=>$this->ld['edit'].$this->ld['recommend'].':'.($val=='1'?$this->ld['yes']:$this->ld['no'])
            ));
            if ($this->configs['operactions-log'] == 1) {
                $this->OperatorLog->log(date('H:i:s').' '.$this->ld['operator'].' '.$this->admin['name'].' '.'编辑货号'.'.'.$product_info['Product']['code'].' '.'推荐'.'.'.$val, $this->admin['id']);
            }
        }
        die(json_encode($result));
    }
    ...
    public function update_product_code()
    {
        ...
        $p = $this->Product->find('first', array('conditions' => array('Product.id' => $id), 'fields' => 'Product.id,Product.status,Product.code'));
        //判断是否在仓库里 如果在不允许该货号
        if (in_array('APP-WAREHOUSE', $this->apps['codes']) && constant('Product') == 'AllInOne') {
            $this->loadModel('Stock');
            $stockInfo = $this->Stock->find('first', array('conditions' => array('Stock.product_code' => $p['Product']['code']), 'callbacks' => 0));
            if (!empty($stockInfo)) {
                $result['flag'] = 2;
        ...
        if (empty($product_info) &&!empty($p['Product'])&& $this->Product->save(array('id' => $id, 'code' => $val, 'operator_id' => $this->admin['id']))) {
            $result['flag'] = 1;
            $result['content'] = stripslashes($val);
            if (constant('Product') == 'AllInOne') {
                $this->loadModel('TaobaoItem');
                $this->TaobaoItem->updateAll(array('TaobaoItem.outer_id' => "'".$val."'", 'TaobaoItem.is_update_taobao' => 0), array('TaobaoItem.outer_id' => $p['Product']['code']));
            }
            $this->ProductAction->update_action(array(
        		'product_id'=>$id,
        		'operator_id'=>$this->admin['id'],
        		'status'=>$p['Product']['status'],
        		'remark'=>$this->ld['log_modify_product_code']
            ));
            if ($this->configs['operactions-log'] == 1) {
    ...
    public function update_product_quantity()
    {
        ...
        $id = $_REQUEST['id'];
        $val = $_REQUEST['val'];
        $product_code = $this->Product->find('first', array('fields' => array('code', 'quantity','status'), 'conditions' => array('id' => $id)));
        $result = array();
        if (!is_numeric($val)) {
            $result['flag'] = 2;
        ...
        if (is_numeric($val) && $this->Product->save(array('id' => $id, 'quantity' => $val, 'operator_id' => $this->admin['id']))) {
            ...
            $this->ProductAction->update_action(array(
        		'product_id'=>$id,
        		'operator_id'=>$this->admin['id'],
        		'status'=>$product_code['Product']['status'],
        		'remark'=>$this->ld['log_modify_product_quantity'].':'.$val
            ));
            //操作员日志
            if ($this->configs['operactions-log'] == 1) {
    ...
    /*
		列表最大购买数修改
	*/
	function update_product_max_buy(){
		$this->operator_privilege('products_edit');
	    	$this->Product->hasMany = array();
	    	$this->Product->hasOne = array();
	    	$id = $_REQUEST["id"];
	    	$val = $_REQUEST["val"];
	    	$product_code=$this->Product->find("first",array("fields"=>array("code","status"),"conditions"=>array("id"=>$id)));
	    	
	    	$result = array();
	    	if(!is_numeric($val)){
	    		$result["flag"] = 2;
				$result["content"] = $this->ld['enter_correct_number'];
	    	}
	    	if(is_numeric($val)&&$this->Product->save(array("id"=>$id,"max_buy"=>$val,"operator_id" => $this->admin['id']))){
			$result["flag"] = 1;
			$result["content"] = stripslashes($val);
			
			//操作员日志
			if ($this->configs['operactions-log'] == 1) {
				$this->OperatorLog->log(date('H:i:s').' '.$this->ld['operator'].' '.$this->admin['name'].' '.$this->ld['edit_product'].'.'.$product_code['Product']['code'].' '.'Offer Qty'.'.'.$val,$this->admin['id']);
			}
			$this->ProductAction->update_action(array(
	        		'product_id'=>$id,
	        		'operator_id'=>$this->admin['id'],
	        		'status'=>$product_code['Product']['status'],
	        		'remark'=>$this->ld['edit'].$this->ld['max_purchase_qty'].':'.$val
	             ));
			$this->product_log_save($id);
		}
	    	Configure::write('debug',0);
    ...
    public function update_product_price()
    {
        ...
        $id = $_REQUEST['id'];
        $val = $_REQUEST['val'];
        $product_code = $this->Product->find('first', array('fields' => array('code','purchase_price','status'), 'conditions' => array('id' => $id)));
        $result = array();
        if (!is_numeric($val)) {
	            $result['flag'] = 2;
	            $result['content'] = $this->ld['enter_correct_price'];
        }else if(floatval($val)<floatval($product_code['Product']['purchase_price'])){
        		$result['flag'] = 2;
	              $result['content'] = $this->ld['j_price_range_setting'];
        }else if (is_numeric($val) && $this->Product->save(array('id' => $id, 'shop_price' => $val, 'market_price' => $val * 1.2, 'operator_id' => $this->admin['id']))) {
            $result['flag'] = 1;
            $result['content'] = stripslashes($val);
            if (constant('Product') == 'AllInOne') {
                $this->loadModel('TaobaoItem');
            ...
            $this->ProductAction->update_action(array(
        		...
        		'operator_id'=>$this->admin['id'],
        		'status'=>$product_code['Product']['status'],
        		'remark'=>$this->ld['log_modify_product_price'].':'.$val
            ));
            $this->product_log_save($id);
        }
        Configure::write('debug', 0);
    ...
    public function update_product_purchase_price()
    {
        ...
        $id = $_REQUEST['id'];
        $val = $_REQUEST['val'];
        $product_code = $this->Product->find('first', array('fields' => array('code','status','shop_price'), 'conditions' => array('id' => $id)));
        $result = array();
        if (!is_numeric($val)) {
		$result['flag'] = 2;
		$result['content'] = $this->ld['enter_correct_price'];
        }else if(floatval($val)>floatval($product_code['Product']['shop_price'])){
		$result['flag'] = 2;
		$result['content'] = $this->ld['j_price_range_setting'];
        }else if (is_numeric($val) &&!empty($product_code)&& $this->Product->save(array('id' => $id, 'purchase_price' => $val, 'operator_id' => $this->admin['id']))) {
	            $result['flag'] = 1;
	            $result['content'] = stripslashes($val);
	            if ($this->configs['operactions-log'] == 1) {
	                	$this->OperatorLog->log(date('H:i:s').' '.$this->ld['operator'].' '.$this->admin['name'].' '.$this->ld['purchase_price'].':'.$val, $this->admin['id']);
	            }
	            if(isset($this->configs['enable_product_log'])&&$this->configs['enable_product_log']=='1'){
		            $this->ProductAction->update_action(array(
		        		'product_id'=>$id,
		        		'operator_id'=>$this->admin['id'],
		        		'status'=>$product_code['Product']['status'],
		        		'remark'=>$this->ld['edit'].$this->ld['purchase_price'].':'.$val
		            ));
	            }
	            $this->product_log_save($id);
        }
	...
	/*
		地区价格编辑
	*/
	function update_product_city_price($pro_price_id,$locale){
		$this->operator_privilege('products_edit');
		Configure::write('debug',0);
    		$this->layout="ajax";
		$id = $_REQUEST["id"];
    		$val = $_REQUEST["val"];
    		$product_code=$this->Product->find("first",array("fields"=>array("code","status"),"conditions"=>array("Product.id"=>$id)));
    		$systemresource_info = $this->SystemResource->resource_formated(array("product_location"),$this->backend_locale);
    		
	    	$result = array();
	    	$result["flag"] = 0;
		$result["content"] = $systemresource_info['product_location'][$locale];
	    	...
	    	if($pro_price_id!=0){
		    	if(is_numeric($val)&&$this->ProductLocalePrice->save(array("id"=>$pro_price_id,"product_price"=>$val))){
				$result["flag"] = 1;
				$result["content"] = stripslashes($val);
				if(isset($this->configs['enable_product_log'])&&$this->configs['enable_product_log']=='1'){
					$this->ProductAction->update_action(array(
			        		...
			        		'operator_id'=>$this->admin['id'],
			        		'status'=>$product_code['Product']['status'],
			        		'remark'=>$this->ld['edit'].$systemresource_info['product_location'][$locale].$this->ld['price'].':'.$val
			            ),$this);
		        	}
				//操作员日志
				if ($this->configs['operactions-log'] == 1) {
					$this->OperatorLog->log($this->ld['operator'].' '.$this->admin['name'].' '.$this->ld['edit_product'].'.'.$product_code['Product']['code'].' '.$systemresource_info['product_location'][$locale].'Price'.'.'.$val, $this->admin['id']);
				}
				$this->product_log_save($id);
			}
		}else{
				$ProductLocalePrice_list=$this->ProductLocalePrice->find("first",array("fields"=>array("ProductLocalePrice.id"),"conditions"=>array("ProductLocalePrice.product_id"=>$id,"ProductLocalePrice.locale"=>$locale)));
				if(!empty($ProductLocalePrice_list)){
					if(is_numeric($val)&&$this->ProductLocalePrice->save(array("id"=>$ProductLocalePrice_list['ProductLocalePrice']['id'],"product_price"=>$val))){
						$result["flag"] = 1;
						$result["content"] = stripslashes($val);
						if(isset($this->configs['enable_product_log'])&&$this->configs['enable_product_log']=='1'){
							$this->ProductAction->update_action(array(
					...
					$data['product_id']=$id;
					$data['locale']=$locale;
					$data['product_price']=$val;
					if(is_numeric($val)&&$this->ProductLocalePrice->save(array("ProductLocalePrice"=>$data))){
						$result["flag"] = 1;
						$result["content"] = stripslashes($val);
						if(isset($this->configs['enable_product_log'])&&$this->configs['enable_product_log']=='1'){
							$this->ProductAction->update_action(array(
    ...
    public function toggle_on_forsale()
    {
		...
		'id' => $id,
		'forsale' => $val,
		'forsale_time'=>date('Y-m-d H:i:s'),
		'operator_id' => $this->admin['id']
        );
        $product_code = $this->Product->find('first', array('fields' => array('code','status'), 'conditions' => array('id' => $id)));
        $result = array();
        if (is_numeric($val) &&!empty($product_code) && $this->Product->save($product_info)) {
            ...
            $this->ProductAction->update_action(array(
        		...
        		'operator_id'=>$this->admin['id'],
        		'status'=>$product_code['Product']['status'],
        		'remark'=>$this->ld['edit'].$this->ld['forsale_status'].':'.($val=='1'?$this->ld['yes']:$this->ld['no'])
            ));
            $this->product_log_save($id);
        }
        Configure::write('debug', 0);
    ...
    public function toggle_on_subscription()
    {
        $this->operator_privilege('products_edit');
        $this->Product->hasMany = array();
        $this->Product->hasOne = array();
        ...
        if (is_numeric($val) &&!empty($product_code) && $this->Product->save($product_info)) {
            ...
            //操作员日志
            if ($this->configs['operactions-log'] == 1) {
                    $this->OperatorLog->log(date('H:i:s').' '.$this->ld['operator'].' '.$this->admin['name'].' '.'编辑货号'.'.'.$product_code['Product']['code'].' '.$this->ld['is_send_subscription'].'.'.$val, $this->admin['id']);
            }
            $this->ProductAction->update_action(array(
                'product_id'=>$id,
                'operator_id'=>$this->admin['id'],
                'status'=>$product_code['Product']['status'],
                'remark'=>$this->ld['edit'].$this->ld['is_send_subscription'].':'.($val=='1'?$this->ld['yes']:$this->ld['no'])
            ));
            $this->product_log_save($id);
        }
        Configure::write('debug', 0);
    ...
    public function toggle_on_recommand_flag()
    {
        ...
        $product_code = $this->Product->find('first', array('fields' => array('code','status'), 'conditions' => array('id' => $id)));
        $result = array();
        if (is_numeric($val) &&!empty($product_code) && $this->Product->save(array('id' => $id, 'recommand_flag' => $val, 'operator_id' => $this->admin['id']))) {
            $result['flag'] = 1;
            $result['content'] = stripslashes($val);
            //操作员日志
            if ($this->configs['operactions-log'] == 1) {
...
//		$this->Product->save(array("Product"=>$product_gallery_info["ProductGallery"]));
        ...
        $this->ProductAction->update_action(array(
    		...
    		'operator_id'=>$this->admin['id'],
    		'status'=>'1',
    		'remark'=>$this->ld['log_set_default_picture'].':'.$product_gallery_info['ProductGallery']['img_original']
        ));
    ...
    */
    ...
    public function export_by_category($export_type, $export_type_re)
    {
        ...
        if (isset($profile_id) && !empty($profile_id)) {
			...
			$product_location=isset($systemresource_info['product_location'])?$systemresource_info['product_location']:array();
             ...
             $profilefiled_info = $this->ProfileFiled->find('all', array('fields' => array('ProfileFiled.code', 'ProfilesFieldI18n.description','ProfileFiled.currency_format'), 'conditions' => array('ProfilesFieldI18n.locale' => $this->locale, 'ProfileFiled.profile_id' => $profile_id['Profile']['id'], 'ProfileFiled.status' => 1), 'order' => 'ProfileFiled.orderby asc'));
            $tmp = array();
            $fields_array = array();
            $fields_info=array();
            $profie_currency_format=array();
            ...
            foreach ($profilefiled_info as $k => $v) {
            	...
            	if($v['ProfileFiled']['code']=="ProductLocalePrice"){
            		...
            		$fields_info[] = $v['ProfileFiled']['code'];
                		$tmp[] = $v['ProfilesFieldI18n']['description'];
                }
            }
            $brand_names = array();
            ...
            if (in_array('PublicAttribute', $fields_array)||in_array('ProductTypeAttribute', $fields_array)) {
	             $attribute_cond=array();
	             $attribute_cond['Attribute.status']='1';
	             $attribute_cond['Attribute.type <>']='customize';
	             $public_attr_ids = $this->ProductTypeAttribute->getattrids(0);
	             if(!in_array('PublicAttribute', $fields_array)&&!empty($public_attr_ids)){
	             	$attribute_cond['not']['Attribute.id']=$public_attr_ids;
	             }else if(!in_array('ProductTypeAttribute', $fields_array)&&!empty($public_attr_ids)){
	             	$attribute_cond['Attribute.id']=$public_attr_ids;
	             }
			$this->Attribute->set_locale($this->backend_locale);
			$attribute_info = $this->Attribute->find('all', array('conditions' => $attribute_cond, 'fields' => 'Attribute.id,AttributeI18n.name'));
		...
		foreach ($pat as $pp) {
            ...
            $fields_info[]="Product.id";
            $fields_info[]="Product.category_id";
            $p = $this->Product->find('all', array('conditions' => $conditon,'fields'=>$fields_info, 'order' => $order));
            $p_ids = array();
            if (!empty($p)) {
                foreach ($p as $s) {
                    $p_ids[] = $s['Product']['id'];
                ...
                if(isset($product_location)&&!empty($product_location)){
                		$pro_city_price=$this->ProductLocalePrice->get_pro_city_price_list(array("product_id"=>$p_ids,'fields'=>'ProductLocalePrice.id,ProductLocalePrice.product_id,ProductLocalePrice.locale,ProductLocalePrice.product_price,ProductLocalePrice.status'));
                }
            }
            $attr_info = array();
		if(!empty($pat_ids)){
			...
			$row[] = $this->ld['is_not_category'];
			$excel[] = $row;
			$tmp = $this->category_products_tree($data, 0, $p, $fields_array, $pat, $attr_info, $brand_names, $product_type_names,(isset($product_location)?$product_location:array()),(isset($pro_city_price)?$pro_city_price:array()),$profie_currency_format);
            ...
            foreach ($category_tree as $k => $v) {
                ...
                if (!empty($data[$v['CategoryProduct']['id']])) {
                    ...
                    $excel[] = $row;
                    if (isset($data[$v['CategoryProduct']['id']])) {
                        $tmp = $this->category_products_tree($data, $v['CategoryProduct']['id'], $p, $fields_array, $pat, $attr_info, $brand_names, $product_type_names,(isset($product_location)?$product_location:array()),(isset($pro_city_price)?$pro_city_price:array()),$profie_currency_format);
                        $excel = array_merge($excel, $tmp);
                    }
                }
                if (isset($v['SubCategory'])) {
                    foreach ($v['SubCategory'] as $kk => $vv) {
                        ...
                        if (!empty($data[$vv['CategoryProduct']['id']])) {
                            ...
                            $excel[] = $row;
                            if (isset($data[$vv['CategoryProduct']['id']])) {
                                $tmp = $this->category_products_tree($data, $vv['CategoryProduct']['id'], $p, $fields_array, $pat, $attr_info, $brand_names, $product_type_names,(isset($product_location)?$product_location:array()),(isset($pro_city_price)?$pro_city_price:array()),$profie_currency_format);
                                $excel = array_merge($excel, $tmp);
                            }
                        }
                        if (isset($vv['SubCategory'])) {
                            foreach ($vv['SubCategory'] as $kkk => $vvv) {
                                ...
                                if (!empty($data[$vvv['CategoryProduct']['id']])) {
                                    ...
                                    $excel[] = $row;
                                    if (isset($data[$vvv['CategoryProduct']['id']])) {
                                        $tmp = $this->category_products_tree($data, $vvv['CategoryProduct']['id'], $p, $fields_array, $pat, $attr_info, $brand_names, $product_type_names,(isset($product_location)?$product_location:array()),(isset($pro_city_price)?$pro_city_price:array()),$profie_currency_format);
                                        $excel = array_merge($excel, $tmp);
                                    }
                                }
                            }
    ...
    /*
    		按分类选择导出
    */
    public function export_by_check_category($export_type, $export_type_re){
    		Configure::write('debug',1);
        	$this->layout = 'ajax';
    		$product_check = explode('&checkboxes[]=', isset($_REQUEST['method'])?$_REQUEST['method']:'');
    		$product_check=array_filter($product_check);
    		$export_type_location_price=isset($_REQUEST['export_type_location_price'])?$_REQUEST['export_type_location_price']:0;
		...
		$this->Profile->hasOne = array();
			...
			$this->Product->set_locale($this->backend_locale);
	            $conditon=array(
	            	'Product.id'=>$product_check
	            );
	            $fields_info[]="Product.id";
                  $fields_info[]="Product.category_id";
	            $product_list = $this->Product->find('all', array('conditions' => $conditon,'fields'=>$fields_info,'order' =>'Product.id'));
	            $brand_names = array();
	            $product_type_names = array();
	            $pat = array();
	            if(!empty($product_list)){
			...
			if (!empty($attribute_info)) {
				...
				$row[] = $this->ld['is_not_category'];
				$excel[] = $row;
				$tmp = $this->category_products_tree($data, 0, $product_list, $fields_array, $pat, $attr_info, $brand_names, $product_type_names,(isset($product_location)?$product_location:array()),(isset($pro_city_price)?$pro_city_price:array()),$profie_currency_format);
				$excel = array_merge($excel, $tmp);
	            }
	            $category_tree = $this->CategoryProduct->tree('P','all',$this->backend_locale);
	            foreach ($category_tree as $k => $v) {
	                ...
	                if (isset($v['SubCategory'])) {
	                    foreach ($v['SubCategory'] as $kk => $vv) {
	                        ...
	                        if (!empty($data[$vv['CategoryProduct']['id']])) {
	                            ...
	                            $excel[] = $row;
	                            if (isset($data[$vv['CategoryProduct']['id']])) {
	                                $tmp = $this->category_products_tree($data, $vv['CategoryProduct']['id'], $product_list, $fields_array, $pat, $attr_info, $brand_names, $product_type_names,(isset($product_location)?$product_location:array()),(isset($pro_city_price)?$pro_city_price:array()),$profie_currency_format);
	                                $excel = array_merge($excel, $tmp);
	                            }
	                        }
	                        if (isset($vv['SubCategory'])) {
	                            foreach ($vv['SubCategory'] as $kkk => $vvv) {
	                                ...
	                                if (!empty($data[$vvv['CategoryProduct']['id']])) {
	                                    ...
	                                    $excel[] = $row;
	                                    if (isset($data[$vvv['CategoryProduct']['id']])) {
	                                        $tmp = $this->category_products_tree($data, $vvv['CategoryProduct']['id'], $product_list, $fields_array, $pat, $attr_info, $brand_names, $product_type_names,(isset($product_location)?$product_location:array()),(isset($pro_city_price)?$pro_city_price:array()),$profie_currency_format);
	                                        $excel = array_merge($excel, $tmp);
	                                    }
	                                }
	                            }
        	...
        	unset($fields_array);
        	unset($data);
        	$this->Phpexcel->output('products_by_choice_category'.date('YmdHis').'.xls', $excel);
    		exit();
    }
...
//					$this->redirect("/products/view/".$_POST['product_id']);
    ...
    */
    ...
    public function uploadproduct()
    {
        ...
        $categories_tree = $this->CategoryProduct->tree('P','all', $this->locale);
        ...
        if ($this->RequestHandler->isPost()) {
            ...
            if (!empty($_FILES['file'])){
                if ($_FILES['file']['error'] > 0) {
                    ...
                    $profilefiled_info = $this->ProfileFiled->find('all', array('fields' => array('ProfileFiled.code', 'ProfilesFieldI18n.description','ProfileFiled.currency_format'), 'conditions' => array('ProfilesFieldI18n.locale' => $this->backend_locale, 'ProfileFiled.profile_id' => $profile_info['Profile']['id'], 'ProfileFiled.status' => 1), 'order' => 'ProfileFiled.orderby asc,ProfileFiled.id'));
                    ...
                    $preview_key=array();$preview_key_code=array();
                    ...
                    while ($row = $this->fgetcsv_reg($handle, 10000, ',')) {
                        ...
                        $temp = array();$preview_key_code=array();
                        foreach ($row as $k => $v) {
                            $data_key_code=isset($key_code[$preview_key[$k]])?$key_code[$preview_key[$k]]:$preview_key[$k];
                            if(!in_array($data_key_code,$key_arr)&&!in_array($data_key_code,$attr_code_arr)){
                            	continue;
                            }
                            $preview_key_code[]=$data_key_code;
                        ...
                        if (!isset($temp) || empty($temp)||!in_array('code',$preview_key_code)) {
                            echo "<meta http-equiv='Content-Type' content='text/html; charset=utf-8' /><script>alert('".$this->ld['file_upload_error']."');window.location.href='/admin/products/uploadproduct';</script>";
                            exit();
                        }
                        $data[] = $temp;
                    ...
                    $this->set('preview_key_code', $preview_key_code);
                    $titer_arr="";
                    $ip=0;
    ...
    public function batch_add_products()
    {
        ...
        if ($this->RequestHandler->isPost()) {
            ...
            $this->CategoryType->set_locale($this->backend_locale);
            $category_type_tree = $this->CategoryType->find('all', array("fields"=>"CategoryType.id,CategoryTypeI18n.name",'conditions' => array('CategoryType.status' => 1)));
            //获得商品分类名称
            foreach ($category_type_tree as $v) {
                $category_type_names[$v['CategoryTypeI18n']['name']] = $v['CategoryType']['id'];
            }
            //取出商品分类
            $product_category_names = array();
            $this->CategoryProduct->set_locale($this->backend_locale);
		$category_tree = $this->CategoryProduct->tree('P','all', $this->backend_locale);
		foreach ($category_tree as $v) {
			$p_category_id=$v['CategoryProduct']['id'];
			$category_name=trim($v['CategoryProductI18n']['name']);
			$product_category_names[$category_name]['id'] = $p_category_id;
			$sub_category=array();
			if(isset($v['SubCategory'])&&sizeof($v['SubCategory'])>0){
				foreach($v['SubCategory'] as $vv){
					$child_category_id=$vv['CategoryProduct']['id'];
					$child_category_name=trim($vv['CategoryProductI18n']['name']);
					$sub_category[$child_category_name]['id'] = $child_category_id;
					$sub_child_category=array();
					if(isset($vv['SubCategory'])&&sizeof($vv['SubCategory'])>0){
						foreach($vv['SubCategory'] as $vvv){
							$sub_child_category_id=$vvv['CategoryProduct']['id'];
							$sub_child_category_name=trim($vvv['CategoryProductI18n']['name']);
							$sub_child_category[$sub_child_category_name]['id'] = $sub_child_category_id;
						}
						$sub_category[$child_category_name]['SubCategory'] = $sub_child_category;
					}
				}
				$product_category_names[$category_name]['SubCategory'] = $sub_category;
			}
		}
            //取出商品属性组
            $product_type_names = array();
			...
			continue;
                ...
                if (isset($data['shop_price'])&&isset($data['purchase_price'])) {
                		if(floatval($data['purchase_price'])>floatval($data['shop_price'])){
                			$error_msg .= $this->ld['product'].$data['code'].' '.$this->ld['j_price_range_setting'];
					continue;
                		}
                }
                ...
                if(isset($data['option_type_id'])&&$data['option_type_id']!=''&&$data['option_type_id']>=0&&$data['option_type_id']<=2){
            		$Product['option_type_id']=$data['option_type_id'];
                }else if(!isset($Product['option_type_id'])){
            		$Product['option_type_id']=0;
                }
                if(isset($data['category_id'])||!empty($category_id)){
				$product_category_list  = isset($data['category_id'])?explode("-",$data['category_id']):array();
				$product_category_id=0;
				$next_category_names=$product_category_names;
				foreach($product_category_list as $c_name){
					$c_name=trim($c_name);
					$product_category_id=isset($next_category_names[$c_name])?$next_category_names[$c_name]['id']:0;
					$next_category_names=isset($next_category_names[$c_name]['SubCategory'])?$next_category_names[$c_name]['SubCategory']:array();
				}
				$Product['category_id']=$product_category_id>0?$product_category_id:$category_id;
                }
                if(isset($data['category_type_id'])&&isset($category_type_names[$data['category_type_id']])){
                	$Product['category_type_id'] =$category_type_names[$data['category_type_id']];
                }
                if(isset($data['shop_price'])&&is_numeric($data['shop_price'])){
                	$Product['shop_price'] = trim($data['shop_price']);
                	$Product['custom_price'] = $data['shop_price'];
                }
                if(isset($data['market_price'])&&is_numeric($data['market_price'])){
                	$Product['market_price'] = trim($data['market_price']);
                }
                ...
                if(isset($data['quantity'])&&is_numeric($data['quantity'])){
                	...
                	$Product['quantity'] = isset($this->configs['default_stock'])?$this->configs['default_stock']:0;
                }
                if(isset($data['purchase_price'])&&is_numeric($data['purchase_price'])){
                	$Product['purchase_price'] = $data['purchase_price'];
                }
                if(isset($data['recommand_flag'])&&is_numeric($data['recommand_flag'])){
                	$Product['recommand_flag'] = $data['recommand_flag'];
                }
                if(isset($data['forsale'])){
                	$Product['forsale'] = $data['forsale'];
                	if(empty($product_info)||trim($product_info['Product']['forsale_time']=='')){
                		$Product['forsale_time'] = date('Y-m-d H:i:s');
                	}
                }
                if(isset($data['alone'])){
                	$Product['alone'] = $data['alone'];
                }
		  if(isset($data['min_buy'])&&intval($data['min_buy'])>0){
                	$Product['min_buy'] = $data['min_buy'];
                }else if(!isset($Product['min_buy'])){
                	$Product['min_buy'] = 1;
                }
		 if(isset($data['max_buy'])&&intval($data['max_buy'])>0){
                	$Product['max_buy'] = $data['max_buy'];
                }else if(!isset($Product['max_buy'])){
                	$Product['max_buy'] = 100;
                }
                if(isset($data['point'])){
                	$Product['point'] = $data['point'];
                }
                if(isset($data['point_fee'])&&is_numeric($data['point_fee'])){
                	$Product['point_fee'] = $data['point_fee'];
                }
                if(isset($data['brand_id'])&&isset($brand_list[$data['brand_id']])){
                	$Product['brand_id'] =$brand_list[$data['brand_id']];
                }
                if(isset($data['product_type_id'])&&isset($product_type_names[$data['product_type_id']])){
                	$Product['product_type_id'] =$product_type_names[$data['product_type_id']];
                }
                $Product['operator_id']=$this->admin['id'];
                $Product['operator_name']=$this->admin['name'];
                $Product['last_update_time']=date('Y-m-d H:i:s');
                if (empty($Product['id'])) {
                    ...
                    if(isset($this->configs['enable_product_log'])&&$this->configs['enable_product_log']=='1'){
			    		...
			    		'operator_id'=>$this->admin['id'],
			    		'status'=>'1',
			    		'remark'=>$this->ld['add'].':'.$Product['code']
			        ));
		        }
                } else {
                    ...
                    if(isset($this->configs['enable_product_log'])&&$this->configs['enable_product_log']=='1'){
			    		...
			    		'operator_id'=>$this->admin['id'],
			    		'status'=>'1',
			    		'remark'=>$this->ld['edit'].':'.$Product['code']
			        ));
		        }
                }
                ++$i;
                //属性保存  公共属性
                if (!empty($data['ProductAttribute'])&&is_array($this->front_locales)){
                    $this->ProductAttribute->deleteAll(array('ProductAttribute.product_id' => $id));
                    $product_type_cond=array();
                    $product_type_cond[]=0;
                    if(isset($Product['product_type_id'])&&$Product['product_type_id']!=0){
                        	$product_type_cond[]=$Product['product_type_id'];
                    }
                    $attbute_id=$this->ProductTypeAttribute->find('list',array('fields'=>'ProductTypeAttribute.attribute_id','conditions'=>array('ProductTypeAttribute.product_type_id'=>$product_type_cond)));
                    if(!empty($attbute_id)){
                        ...
                        foreach($this->front_locales as $k =>$v){
                            foreach ($attr_infos as $ak =>$a){
                                if (isset($data['ProductAttribute'][$a]) && !empty($data['ProductAttribute'][$a])) {
                                    ...
                                    $attr_check_flag=false;
                                    if(in_array($att['attribute_id'],$attbute_id)){
                                        if(isset($attbutes_array[$att['attribute_id']])){
                                            if(in_array($att['attribute_value'],$attbutes_array[$att['attribute_id']])){
                                                $attr_check_flag=true;
                                            }
                                        }else{
                                            $attr_check_flag=true;
                                        }
                ...
                if (!empty($Product['id'])) {
                    ...
                    if(isset($data['tag'])&&trim($data['tag'])!=""){
                        $product_tag_info=split(",",$data['tag']);
                        $pro_tag_ids=$this->Tag->find('list',array("conditions"=>array("Tag.type"=>'P',"Tag.type_id"=>$id)));
                        if(!empty($pro_tag_ids)){
                            $this->Tag->deleteAll(array("Tag.id"=>$pro_tag_ids));
                            $this->TagI18n->deleteAll(array("TagI18n.tag_id"=>$pro_tag_ids));
                        }
                        if (isset($this->front_locales)&&is_array($this->front_locales)&&!empty($product_tag_info)){
                            foreach($product_tag_info as $vv){
                                if(trim($vv)=="")continue;
                                $tag_data=array(
                                    "id"=>0,
                                    "type"=>'P',
                                ...
                                foreach ($this->front_locales as $k => $v) {
                                    ...
                                    $this->TagI18n->save($tag_i18n_data);
                                }
                            }
                        }
                    }else if(isset($data['tag'])&&trim($data['tag'])==""){
				$pro_tag_ids=$this->Tag->find('list',array("conditions"=>array("Tag.type"=>'P',"Tag.type_id"=>$id)));
				if(!empty($pro_tag_ids)){
					$this->Tag->deleteAll(array("Tag.id"=>$pro_tag_ids));
					$this->TagI18n->deleteAll(array("TagI18n.tag_id"=>$pro_tag_ids));
                    ...
                    if (!empty($ProductGallery['img_thumb']) || !empty($ProductGallery['img_detail']) || !empty($ProductGallery['img_original'])) {
                        ...
                        if (!empty($ProductGallery['img_thumb'])) {
                            $iamge_url=trim($ProductGallery['img_thumb']);
                        }else if(!empty($ProductGallery['img_detail'])){
                            $iamge_url=trim($ProductGallery['img_detail']);
                        }else if(!empty($ProductGallery['img_original'])){
                            $iamge_url=trim($ProductGallery['img_original']);
                        }
                        //货号对应图片
                        if ($iamge_url!="") {
                            $code = isset($Product['code']) ? $Product['code'] : '';
                            $path = $_SERVER['DOCUMENT_ROOT'].'/data/import'.$iamge_url;
                            //路径存在，图片空间上传图片流程
                            $image_name = substr($path, strrpos($path, '/') + 1);
                            $imgname_arr=array();
                            $imgname_arr[0] = substr($image_name , 0, strripos($image_name, '.'));
                            $imgname_arr[1] = substr($image_name , strripos($image_name , '.') + 1);
                            $image_name = md5($code.$imgname_arr[0]).".".$imgname_arr[1];
                            //获取图片格式(后缀)
                            ...
                            if (file_exists($path)) {
                                if ($check_type != '') {
                                    ...
                                    $imgaddr_original = WWW_ROOT.'/media/photos/'.date('Ym').'/'.$category_id.'/'.$this->admin['id'].'/original/';
                                    $imgaddr_detail = WWW_ROOT.'/media/photos/'.date('Ym').'/'.$category_id.'/'.$this->admin['id'].'/detail/';
                                    $imgaddr_big = WWW_ROOT.'/media/photos/'.date('Ym').'/'.$category_id.'/'.$this->admin['id'].'/big/';
                                    $imgaddr_small = WWW_ROOT.'/media/photos/'.date('Ym').'/'.$category_id.'/'.$this->admin['id'].'/small/';
                                    ...
                                    $old_img_original=isset($product_info['Product']['img_original'])?$product_info['Product']['img_original']:'';
                                    $old_img_content="";
                                    if($old_img_original!=""&&file_exists(WWW_ROOT.$old_img_original)){
                                        $old_img_content=md5(file_get_contents(WWW_ROOT.$old_img_original));
                                        $new_img_content=md5(file_get_contents($path));
                                        if($old_img_content==$new_img_content){
                                    ...
                                    if ($result_url) {
                                        ...
                                        $photo_category_gallery_info=$this->PhotoCategoryGallery->find('first',array('conditions'=>array("PhotoCategoryGallery.img_original"=>$photo_img_original)));
                                        $photo_category_galleries = array(
                                            'id'=>isset($photo_category_gallery_info['PhotoCategoryGallery']['id'])?$photo_category_gallery_info['PhotoCategoryGallery']['id']:0,
                                        ...
                                        $ProductGallery['orderby'] = '1';
                                        $this->Product->saveAll($Product);
                                        $ProductGallery_data=$this->ProductGallery->find('first',array("conditions"=>array("ProductGallery.product_id"=>$id,'orderby'=>'1')));
                                        $ProductGallery['id']=isset($ProductGallery_data['ProductGallery']['id'])?$ProductGallery_data['ProductGallery']['id']:'0';
                                    ...
                                    //图片格式不正确，返回报错
                                    $error_msg .= $this->ld['code'].' '.$code.' '.$this->ld['picture'].$iamge_url.$this->ld['file_format_valid'].'\\r\\n';
                                }
                            } else {
                                //路径不存在，报错
                                $error_msg .= $this->ld['code'].$code.' '.$this->ld['picture'].$iamge_url.$this->ld['not_exist'].'\\r\\n';
                            }
                        }
                    }
                    //商品图片处理
                    $product_image=array();
                    if(isset($data['product_image1'])&&trim($data['product_image1'])!=""){
                        $iamge_data=$this->product_image_process($data['product_image1'],$data['code'],$category_id,isset($product_info['Product']['product_image1'])?$product_info['Product']['product_image1']:'');
                        if($iamge_data['code']=='1'){
                            if(isset($iamge_data['data']['img_original'])){
                                $product_image['product_image1']=$iamge_data['data']['img_original'];
                            }
                        }else{
                            $error_msg .= $iamge_data['data'];
                        }
                    }
                    if(isset($data['product_image2'])&&trim($data['product_image2'])!=""){
                        $iamge_data=$this->product_image_process($data['product_image2'],$data['code'],$category_id,isset($product_info['Product']['product_image2'])?$product_info['Product']['product_image2']:'');
                        if($iamge_data['code']=='1'){
                            if(isset($iamge_data['data']['img_original'])){
                                $product_image['product_image2']=$iamge_data['data']['img_original'];
                            }
                        }else{
                            $error_msg .= $iamge_data['data'];
                        }
                    ...
                    if(!empty($product_image)){
                        $product_image['id']=$id;
                        $this->Product->save($product_image);
                    }
                }
            }
            //操作员日志
    ...
    public function download_csv_example()
    {
        ...
        $category_type_names = array();//商品属性组
        ...
        if (isset($profile_id) && !empty($profile_id)) {
            ...
            //商品类目
            if (in_array('Product.category_type_id', $fields_array)) {
                $this->CategoryType->set_locale($this->backend_locale);
                $category_type_tree = $this->CategoryType->find('all', array("fields"=>"CategoryType.id,CategoryTypeI18n.name",'conditions' => array('CategoryType.status' => 1)));
                //获得商品分类名称
                foreach ($category_type_tree as $v) {
        ...
        if (in_array('Product.tag', $product_fields)) {
            unset($product_fields[array_search('Product.tag', $product_fields)]);
        }
        ...
        foreach ($product_all as $k => $v) {
            ...
            $product_all[$k]['Product']['alone']='1';
        }
        $product_tag_data=array();
        if (in_array('Product.tag', $fields_array)) {//商品标签
            $this->Tag->set_locale($this->backend_locale);
            $product_tag_info=$this->Tag->find('all',array("conditions"=>array("Tag.type"=>'P',"Tag.status"=>'1',"Tag.type_id"=>$product_id)));
            if(!empty($product_tag_info)){
                foreach($product_tag_info as $v){
                    if(trim($v['TagI18n']['name']==""))continue;
                    $product_tag_data[$v['Tag']['type_id']][]=trim($v['TagI18n']['name']);
                }
            }
        }
        ...
        foreach ($product_all as $k => $v) {
            ...
            foreach ($fields_array as $kk => $vv) {
                ...
                }elseif($vv == 'Product.category_type_id'){
                    $user_tmp[] = isset($category_type_names[$v['Product']['category_type_id']]) ? $category_type_names[$v['Product']['category_type_id']] : '';
                }else if($vv == 'Product.tag'){
                    $pro_tag_data=isset($product_tag_data[$v['Product']['id']])?$product_tag_data[$v['Product']['id']]:array();
                    $user_tmp[] = implode(",",$pro_tag_data);
                }else {
                    $fields_kk = explode('.', $vv);
    ...
    /*
        商品图片处理
    */
    private function product_image_process($image_url="",$code="",$category_id=0,$old_img_original=""){
        $result=array();
        $result['code']='0';
        $result['data']="";
        $error_msg="";
        $image_data=array();
        $image_path=$path = $_SERVER['DOCUMENT_ROOT'].'/data/import'.$image_url;
        //路径存在，图片空间上传图片流程
        ...
        $image_name = md5($code.$imgname_arr[0]).".".$imgname_arr[1];
        //获取图片格式(后缀)
        $type = substr($image_path, strrpos($image_path, '.'));
        $files_type = '*.gif; *.jpg; *.png; *.jpeg; *.GIF; *.JPG; *.PNG; *.JPEG';
        $check_type = strstr($files_type, $type);
        //获取图片名(不包括后缀)
        $name = substr($image_name, 0, strrpos($image_name, '.'));
        if (file_exists($image_path)) {
            if ($check_type != '') {
                ...
                if($old_img_original!=""&&file_exists(WWW_ROOT.$old_img_original)){
                    $old_img_content=md5(file_get_contents(WWW_ROOT.$old_img_original));
                    $new_img_content=md5(file_get_contents($image_path));
                    if($old_img_content==$new_img_content){
                        $img_upload_flag=false;
                    }
                }
                $result_url=false;
                if($img_upload_flag){
                    $result_url = copy($image_path, $imgaddr_original.$image_name);
                }
                if ($result_url) {
                    $img_original = $imgaddr_original.$image_name;//原图地址  
                    $img_detail = $imgaddr_detail.$image_name;//详细图 中图地址
                    ...
                    $result['error'] = false;
                    if (!file_exists($img_thumb)) {
                        $error_msg .= $this->ld['code'].' '.$code.' '.$this->ld['picture'].$image_url.$this->ld['thumbnail_generate_failed'].'\\r\\n';
                    }
                    if (!file_exists($img_detail)) {
                        $error_msg .= $this->ld['code'].' '.$code.' '.$this->ld['picture'].$image_url.$this->ld['detail_image_generate_failed'].'\\r\\n';
                    }
                    if (!file_exists($img_original)) {
                        $error_msg .= $this->ld['code'].' '.$code.' '.$this->ld['picture'].$image_url.$this->ld['original_image_build_failure'].'\\r\\n';
                    }
                    if (!file_exists($img_big)) {
                        $error_msg .= $this->ld['code'].' '.$code.' '.$this->ld['picture'].$image_url.$this->ld['big_image_generate_failure'].'\\r\\n';
                    }
                }
            } else {
                //图片格式不正确，返回报错
                $error_msg .= $this->ld['code'].' '.$code.' '.$this->ld['picture'].$image_url.$this->ld['file_format_valid'].'\\r\\n';
            }
        }else{
            $error_msg .= $this->ld['code'].$code.' '.$this->ld['picture'].$image_url.$this->ld['file_fath'].$this->ld['not_exist'].'\\r\\n';
        }
        if($error_msg==""){
            $result['code']='1';
   ...
   */
    ...
    /*
    		商品租赁价格处理
    */
    function ajax_lease_price(){
		Configure::write('debug', 1);
		$this->layout = 'ajax';
		$result=array();
		$result['code'] = 0;
		if(!$this->operator_privilege('products_edit',false)&&!$this->operator_privilege('products_add',false)){
			$result['message'] = $this->ld['have_no_operation_perform'];
			die(json_encode($result));
		}
		$shop_price=isset($_POST['shop_price'])?$_POST['shop_price']:0;
		$lease_price=isset($_POST['lease_price'])?$_POST['lease_price']:0;
		if($lease_price<=0){
			$lease_price=$shop_price;
		}
		$lease_price_info=$this->ProductLeasePrice->find('first', array('conditions' => array('ProductLeasePrice.price <=' => $lease_price),'order'=>"ProductLeasePrice.price DESC"));
		if(!empty($lease_price_info['ProductLeasePrice'])){
			if($lease_price<=0){
				$lease_price=$shop_price*($lease_price_info['ProductLeasePrice']['lease_price_percent']/100);
			}
			$lease_data=array();
			$lease_data["lease_price"]=$lease_price;
        			$lease_data["lease_deposit_base"]=$lease_price_info["ProductLeasePrice"]["lease_deposit_base"]+($shop_price-$lease_price_info["ProductLeasePrice"]["price"])*($lease_price_info['ProductLeasePrice']['lease_deposit_increase_percent']/100);
        		$result['code'] = 1;
        		$result['data']=$lease_data;
		}
		die(json_encode($result));
    }
    
    function ajax_update_product_manager(){
  	if($this->RequestHandler->isPost()){
  		Configure::write('debug',0);
		$this->layout="ajax";
		$result=array();
  		$result['code']=0;
  		$result['message']=$this->ld['have_no_operation_perform'];
		if($this->operator_privilege('products_edit',true)){
			$product_id=isset($_POST['product_id'])?intval($_POST['product_id']):0;
			$product_manager=isset($_POST['product_manager'])?intval($_POST['product_manager']):0;
			$product_info=$this->Product->find('first',array('conditions'=>array('Product.id'=>$product_id)));
			if(!empty($product_info)){
				$operator_info=$this->Operator->find("first",array('conditions'=>array('Operator.id'=>$product_manager,'Operator.status'=>'1')));
				$product_data=array(
					'id'=>$product_id,
				...
				$this->Product->save($product_data);
				$result['code']=1;
				$result['message']=isset($operator_info['Operator']['name'])?$operator_info['Operator']['name']:'-';
				//操作员日志
				if($this->configs['operactions-log'] == 1){
					$this->OperatorLog->log($this->ld['operator'].' '.$this->admin['name'].' '.$this->ld['edit_product'].'.'.$product_info['Product']['code'].' '.$this->ld['product_manager'].':'.$result['message'], $this->admin['id']);
				}
				$this->ProductAction->update_action(array(
					'product_id'=>$product_id,
					'operator_id'=>$this->admin['id'],
					'status'=>'1',
					'remark'=>$this->ld['edit'].$this->ld['product_manager'].':'.$result['message']
				));
			}else{
    ...
    function location_product_price_mail($product_id=0){
    		if(isset($this->configs['product_location_price_change_notification'])&&$this->configs['product_location_price_change_notification']=='1'){
    			$this->Product->set_locale($this->backend_locale);
    			$product_data=$this->Product->find('first',array('fields'=>'Product.id,Product.code,Product.brand_id,Product.quantity,Product.last_update_time,Product.operator_name,ProductI18n.name','conditions'=>array('Product.id'=>$product_id)));
    			if(empty($product_data['Product']))return false;
    			$product_code=isset($product_data['Product']['code'])?$product_data['Product']['code']:'';
    			$brand_id=$product_data['Product']['brand_id'];
    			$brand_info=$this->Brand->find('first',array('fields'=>'Brand.id,BrandI18n.name','conditions'=>array('Brand.id'=>$brand_id,'Brand.status'=>'1')));
    			$this->loadModel('QuoteProduct');
    			...
    			$joins=array(
    						...
    						'table' => 'svoms_quotes',
			                    'alias' => 'Quote',
			                    'type' => 'left',
			                    'conditions' => array('Quote.id = QuoteProduct.quote_id')
			        	)
                		);
                	$conditions=array(
                		"QuoteProduct.product_code"=>$product_code,
                		"Quote.is_sendmail"=>'0'
                	);
    			$quote_operator_infos=$this->QuoteProduct->find('list',array("fields"=>"Quote.id,Quote.quoted_by",'conditions'=>$conditions,"joins"=>$joins));
    			if(!empty($quote_operator_infos)){
    				$operator_list=$this->Operator->find('list',array('fields'=>"Operator.id,Operator.email",'conditions'=>array('Operator.name'=>$quote_operator_infos,"Operator.status"=>'1')));
    				if(!empty($operator_list)){
    					$this->NotifyTemplateType->set_locale($this->backend_locale);
	     			$totify_template=$this->NotifyTemplateType->typeformat('product_location_price','email');
	     			if(empty($totify_template))return false;
    					$systemresource_info = $this->SystemResource->resource_formated(array("product_location"),$this->backend_locale);
    					$location_price_show="<table width='100%' align='center' border='1px' cellspacing='0' cellpadding='5px'>";
    					$location_price_show.="<tr>";
    					$location_price_show.="<td>".$this->ld['sku']."</td>";
    					$location_price_show.="<td>".$this->ld['description']."</td>";
    					$location_price_show.="<td>".$this->ld['brand']."</td>";
    					$location_price_show.="<td>".$this->ld['quantity']."</td>";
    					if(isset($systemresource_info['product_location'])&&!empty($systemresource_info['product_location'])){
    						foreach($systemresource_info['product_location'] as $k=>$v){
    							$location_price_show.="<td>".$v."</td>";
    						}
    					}
    					$location_price_show.="<td>Last  Update Date</td>";
    					$location_price_show.="<td>Last  Update By</td>";
    					$location_price_show.="</tr>";
    					$location_price_show.="<tr>";
    					$location_price_show.="<td>".$product_data['Product']['code']."</td>";
    					$location_price_show.="<td>".$product_data['ProductI18n']['name']."</td>";
    					$location_price_show.="<td>".(isset($brand_info['BrandI18n']['name'])?$brand_info['BrandI18n']['name']:'-')."</td>";
    					$location_price_show.="<td>".$product_data['Product']['quantity']."</td>";
    					$ProductLocalePrice_list=$this->ProductLocalePrice->find("list",array("fields"=>array("ProductLocalePrice.locale","ProductLocalePrice.product_price"),"conditions"=>array("ProductLocalePrice.product_id"=>$product_id)));
    					if(isset($systemresource_info['product_location'])&&!empty($systemresource_info['product_location'])){
    						foreach($systemresource_info['product_location'] as $k=>$v){
    							$location_price_show.="<td>".(isset($ProductLocalePrice_list[$k])?$ProductLocalePrice_list[$k]:0)."</td>";
    						}
    					}
    					$location_price_show.="<td>".$product_data['Product']['last_update_time']."</td><td>".$product_data['Product']['operator_name']."</td>";
    					$location_price_show.="</tr>";
    					$location_price_show.="</table>";
    					$shop_name = $this->configs['shop_name'];
    					$operator_signature=$this->admin['signature'];
    					...
    					$text_body = $totify_template['email']['NotifyTemplateTypeI18n']['param02'];
                			...
                			$html_body.=$operator_signature;
                			$text_body.=$operator_signature;
    					$mailsendqueue = array(
						'sender_name' => $shop_name,//发送从姓名
						'receiver_email' => $operator_list,//接收人姓名;接收人地址
						'cc_email' => ';',//抄送人
						'bcc_email' => ';',//暗送人
						'title' => $subject,//主题 
						...
						'pri' => 0
		                	...
		                	$this->configs['sender_name']=$this->admin['name'];
		                	$mailsendname=$this->admin['email'];
		                	$this->Notify->send_email($mailsendqueue, $this->configs,$mailsendname);
    				}
    			}
    		}
    }

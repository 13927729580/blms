...
class RegionsController extends AppController{
    ...
    public function view($id = 0)
    {
        ...
        if ($this->RequestHandler->isPost()) {
        	if(!isset($this->data['Region']['orderby'])||$this->data['Region']['orderby']=='')$this->data['Region']['orderby']=50;
...
//    	    }
    ...
    public function remove($id = 0)
    {
        ...
        $child_region=$this->Region->find('list',array('fields'=>'Region.id','conditions'=>array('Region.parent_id'=>$id)));
        if(!empty($child_region)){
	 	 $sub_region=$this->Region->find('list',array('fields'=>'Region.id','conditions'=>array('Region.parent_id'=>$child_region)));
	 	 if(!empty($sub_region)){
 	 	 	$this->Region->deleteAll(array('Region.id' => $sub_region));
        		$this->RegionI18n->deleteAll(array('RegionI18n.region_id' => $sub_region));
 	 	 }
 	 	 $this->Region->deleteAll(array('Region.id' => $child_region));
        	 $this->RegionI18n->deleteAll(array('RegionI18n.region_id' => $child_region));
	 }
...
//		}
    ...
    function load_region(){
    		...
    		$result=array();
    		$result['code']='0';
    		$parent_id=isset($_REQUEST['parent_id'])&&!empty($_REQUEST['parent_id'])?$_REQUEST['parent_id']:'0';
    		if(is_array($parent_id)){
    			$parent_id[]=0;
    		}
    		$this->Region->set_locale($this->backend_locale);
    		$region_info=$this->Region->find('all',array('fields'=>'Region.id,Region.parent_id,RegionI18n.name','conditions'=>array('Region.parent_id'=>$parent_id),'order'=>'Region.parent_id,RegionI18n.name'));
    		$region_data=array();
    		if(!empty($region_info)){
    			$result['code']='1';
    			foreach($region_info as $v){
    				$region_data[$v['Region']['parent_id']][]=$v;
    			}
    		}
    		
    		$result['data']=$region_data;
    		die(json_encode($result));
    }

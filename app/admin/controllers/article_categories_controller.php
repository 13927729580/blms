...
class ArticleCategoriesController extends AppController
{
    ...
    public function view($id = 0)
    {
        ...
        $Resource_info = $this->Resource->getformatcode(array('sub_type','article_category_template','article_tamplate'), $this->backend_locale, false);//资源库信息
        $this->set('Resource_info', $Resource_info);
    }
    ...
    */
    ...
    function ajax_category_search(){
    		Configure::write('debug', 1);
        	$this->layout = 'ajax';
        	$result['content'] = array();
        	$result['flag'] = 2;
        	$category_keyword = empty($_REQUEST['category_keyword']) ? '' : trim($_REQUEST['category_keyword']);//关键字
        	$condition=array();
        	$condition['CategoryArticle.status'] = '1';
        	if($category_keyword==""){
        		$condition['CategoryArticle.parent_id'] = '0';
        	}else{
        		$condition['CategoryArticleI18n.name like'] = "%{$category_keyword}%";
        	}
		$condition['CategoryArticleI18n.locale'] = $this->backend_locale;
		$category_info = $this->CategoryArticle->find('all', array('conditions' => $condition,'fields'=>'CategoryArticle.id,CategoryArticle.parent_id,CategoryArticleI18n.name'));
		if(!empty($category_info)){
			...
			$sub_condition=array();
			$sub_condition['CategoryArticle.status'] = '1';
			$sub_condition['CategoryArticleI18n.locale'] = $this->backend_locale;
			$sub_condition['CategoryArticle.parent_id'] = $category_ids;
			$sub_category_info=$this->CategoryArticle->find('all', array('conditions' => $sub_condition,'fields'=>'CategoryArticle.id,CategoryArticle.parent_id,CategoryArticleI18n.name'));
			if(!empty($sub_category_info)){
				$sub_category_ids=array();
				$sub_category_datas=array();
				foreach($sub_category_info as $v){
					$sub_category_ids[]=$v['CategoryArticle']['id'];
				}
				$child_condition=array();
				$child_condition['CategoryArticle.status'] = '1';
				$child_condition['CategoryArticleI18n.locale'] = $this->backend_locale;
				$child_condition['CategoryArticle.parent_id'] = $sub_category_ids;
				$child_category_info=$this->CategoryArticle->find('all', array('conditions' => $child_condition,'fields'=>'CategoryArticle.id,CategoryArticle.parent_id,CategoryArticleI18n.name'));
				$child_category_ids=array();
				$child_category_datas=array();
				if(!empty($child_category_info)){
					foreach($child_category_info as $v){
						$child_category_ids[]=$v['CategoryArticle']['id'];
						$child_category_datas[$v['CategoryArticle']['parent_id']][]=$v;
					}
				}
				foreach($sub_category_info as $k=>$v){
					$v['SubCategory']=isset($child_category_datas[$v['CategoryArticle']['id']])?$child_category_datas[$v['CategoryArticle']['id']]:array();
					$sub_category_datas[$v['CategoryArticle']['parent_id']][]=$v;
				}
				foreach($category_info as $k=>$v){
					if(in_array($v['CategoryArticle']['id'],$sub_category_ids)||in_array($v['CategoryArticle']['id'],$child_category_ids)){
						unset($category_info[$k]);
						continue;
					}
					$category_info[$k]['SubCategory']=isset($sub_category_datas[$v['CategoryArticle']['id']])?$sub_category_datas[$v['CategoryArticle']['id']]:array();
				}
			}
			$result['flag'] = 1;
            	$result['content'] = $category_info;
		}
		die(json_encode($result));
    }
}

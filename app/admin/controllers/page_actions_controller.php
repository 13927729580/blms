...
class PageActionsController extends AppController
{
    ...
    *引用的model
    */
    public $uses = array('PageModule','PageModuleI18n','Resource','InformationResource','Template','Template','PageAction','PageType','OperatorLog');
    /*
    *预定义变量：模块位置
    */
...
//		$this->navigations[] = array('name'=>$this->ld['module_management'],'url'=>'/page_types/');
    ...
    public function set_modelorfunction()
    {
        ...
        if (isset($resInfo) && !empty($resInfo)) {
            ...
            if ($val != '') {
                $val_arr = explode(':', $val);
                $result['model'] = isset($val_arr[0])?$val_arr[0]:'';
                $result['function'] = isset($val_arr[1])?$val_arr[1]:'';
            } else {
                $result['status'] = '0';
                $result['model'] = '';
                $result['function'] = '';

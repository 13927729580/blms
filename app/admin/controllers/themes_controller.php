...
//	    }
    ...
    function template($template_name=''){
		...
		$this->navigations[] = array('name' => $this->ld['manager_templates'],'url' => '/themes/');
		if($template_name==''){
			$template_info=$this->Template->find('first',array('conditions'=>array('Template.is_default'=>1)));
		...
		$this->navigations[] = array('name' => $this->ld['template'] , 'url' => '/themes/template/'.$template_name);
		if(!(isset($template_info)&&!empty($template_info))){
			...
			if(empty($template_info))$this->redirect('index');
		}
		$this->set('template_info',$template_info);
		
		$template_list=$this->Template->find('list',array('fields'=>'Template.name,Template.description','conditions'=>array('Template.status'=>'1')));
		$this->set('template_list',$template_list);
		
		$page_locale=isset($_REQUEST['backend_locale'])&&trim($_REQUEST['backend_locale'])!=''?$_REQUEST['backend_locale']:$this->backend_locale;
		$this->set('page_locale', $page_locale);
		...
		$conditions=array();
		$conditions['Config.code'] = $config_codes;
		$conditions['Config.status'] = 1;
		$conditions['Config.readonly'] = 0;
		$web_config_infos = $this->Config->find('all', array('conditions' => $conditions, 'order' => 'Config.orderby,Config.group_code'));
		$web_config_list=array();
		$web_config_datas=array();
		if(!empty($web_config_infos)){
			foreach($web_config_infos as $v){
				$web_config=array();
				$web_config['Config']=$v['Config'];
				foreach($v['ConfigI18n'] as $vv){
					$web_config['ConfigI18n'][$vv['locale']]=$vv;
					if ($v['Config']['type'] == 'radio' || $v['Config']['type'] == 'checkbox' || $v['Config']['type'] == 'select') {
						$web_config['ConfigI18n'][$vv['locale']]['options'] = explode("\n", $vv['options']);
					}
				}
				$web_config_list[$v['Config']['code']]=$web_config;
				$web_config_datas[$v['Config']['code']]=$web_config['ConfigI18n'][$this->backend_locale]['name'];
			}
		}
		$this->set('web_config_list',$web_config_list);
		$this->set('web_config_datas',$web_config_datas);
		
		$this->loadModel('Navigation');
		$this->Navigation->set_locale($this->backend_locale);
		$navigation_type_info=array(
			'T' => 'top_navigation',
			'H' => 'middle_navigation',
			'B' => 'bottom_navigation',
			'M' => 'help_navigation',
			'PM'=>'mobile_bottom_navigation',
			'BP'=>'mobile_middle_navigation'
		);
		$this->set('navigation_type_info',$navigation_type_info);
		$navigation_types=array('T','H','B','M','PM','BP');
		$navigation_info=$this->Navigation->find('all',array('fields'=>'Navigation.id,Navigation.type,Navigation.status,NavigationI18n.name,NavigationI18n.url','order'=>'Navigation.type,Navigation.orderby,Navigation.id','conditions'=>array('Navigation.type'=>$navigation_types)));
		$navigation_list=array();
		if(sizeof($navigation_info)>0){
			foreach($navigation_info as $v){
				$navigation_list[$v['Navigation']['type']][]=$v;
			}
		}
		$this->set('navigation_list',$navigation_list);
		
		$this->loadModel('Link');
		$this->Link->set_locale($this->backend_locale);
		$link_list=$this->Link->find('all',array('fields'=>'Link.id,Link.status,LinkI18n.name,LinkI18n.url','order'=>'Link.orderby'));
		$this->set('link_list',$link_list);
		
		$page_action_ids=array();
		$page_type_list = $this->PageType->find('list', array('fields'=>'PageType.id,PageType.name','conditions'=>array('PageType.code'=>$template_name)));
		if(!empty($page_type_list)){
			$page_type_ids=array_keys($page_type_list);
			$page_action_list = $this->PageAction->find('all',array('fields'=>'PageAction.id,PageAction.page_type_id,PageAction.name','conditions'=>array('PageAction.page_type_id'=>$page_type_ids,'PageAction.status'=>'1'),'order'=>'PageAction.page_type_id,PageAction.id'));
			$page_action_data=array();
			foreach($page_action_list as $v){
				$page_action_ids[]=$v['PageAction']['id'];
				$page_action_data[$v['PageAction']['page_type_id']][]=$v['PageAction'];
				if(strstr($this->ld['home'],$v['PageAction']['name'])&&!isset($_REQUEST['page_action_id'])){
					$_REQUEST['page_action_id']=$v['PageAction']['id'];
				}
			}
			$template_page=array();
			foreach($page_type_list as $k=>$v){
				$template_page[]=array(
					'id'=>$k,
					'name'=>$v,
					'page_action'=>isset($page_action_data[$k])?$page_action_data[$k]:array()
				);
			}
			$this->set('template_page',$template_page);
		}
		$page_action_id=isset($_REQUEST['page_action_id'])&&intval($_REQUEST['page_action_id'])>0?intval($_REQUEST['page_action_id']):0;
		if($page_action_id>0&&in_array($page_action_id,$page_action_ids)){
			$this->set('page_action_id',$page_action_id);
			
			$page_action_cond=array('PageModule.page_action_id'=>$page_action_id);
			$pagemodule_list = $this->PageModule->tree($this->locale, $page_action_cond);
    ...
    public function templatecopy($id='0', $name=''){
		...
		$this->Template->save($templateInfo_copy);
            $pagetype_info=$this->PageType->find('all',array('conditions'=>array("PageType.code"=>$templateInfo['Template']['name'])));
            foreach($pagetype_info as $pagetype_data){
            		...
            		$pageaction_info=$this->PageAction->find('all',array("conditions"=>array("PageAction.page_type_id"=>$page_type_id)));

...
class StaticPagesController extends AppController
{
    ...
    public function index($page = 1)
    {
        ...
        $parameters['route'] = array('controller' => 'static_pages','action' => 'index','page' => $page,'limit' => $rownum);
    ...
    public function view($id = 0)
    {
        ...
        if (empty($id)) {
            ...
            $this->operator_privilege('static_page_edit');
                //查找映射路径的内容
            $conditions = array('Route.controller' => 'pages','Route.action' => 'view','Route.model_id' => $id,'Route.url <>' => '/');
            ...
            if (empty($rurl)) {
                ...
                if (empty($id)) {
                    if ($routeurl['Route']['url'] != $this->data['Route']['url']) {
                        //foreach($this->data[])
                        $this->data['Route']['controller'] = 'pages';
    ...
    public function update_page_subtitle()
    {
        $this->Page->hasOne = array();
        $id = $_REQUEST['id'];
        $val = $_REQUEST['val'];

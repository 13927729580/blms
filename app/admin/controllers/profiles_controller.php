...
class  ProfilesController extends AppController
{
    ...
    public function index($page = 1)
    {
        ...
        $group_tree = $this->Profile->find('all', array('conditions'=>array('ProfileI18n.name <>'=>'','Profile.group <>'=>''),'group' => 'Profile.group', 'fields' => 'Profile.group,ProfileI18n.name'));
	...
	if (isset($this->params['url']['system_code']) && $this->params['url']['system_code'] != '') {
		$conditions['Profile.system_code'] = $this->params['url']['system_code'];
		$this->set('system_code', $this->params['url']['system_code']);
	}
	if (isset($this->params['url']['module_code']) && $this->params['url']['module_code'] != '') {
		$conditions['Profile.module_code'] = $this->params['url']['module_code'];
    ...
    function system_modified(){
		...
		$profile_id=isset($_POST['id'])?$_POST['id']:0;
		$system_code=isset($_POST['val'])?trim($_POST['val']):'';
		$this->Profile->save(array('id'=>$profile_id,'system_code'=>$system_code));
		$result['flag'] = 1;
      ...
      function module_modified(){
		...
		$this->Profile->save(array('id'=>$profile_id,'module_code'=>$module_code));
		$result['flag'] = 1;

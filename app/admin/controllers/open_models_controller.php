...
class OpenModelsController extends AppController
{
    ...
    */
    ...
    function ajax_wechat_sign(){
        	...
        	$this->layout = 'ajax';
        	$result=array();
        	$open_model=$this->OpenModel->find('first',array('conditions'=>array('OpenModel.status'=>'1','OpenModel.open_type'=>'wechat')));
        	if(!empty($open_model)){
	        	$request_link=isset($_SERVER['HTTP_REFERER'])?$_SERVER['HTTP_REFERER']:$this->server_host;
			$appId=$open_model['OpenModel']['app_id'];
			$appsecret=$open_model['OpenModel']['app_secret'];
			...
			$tmpArr = array(
				'noncestr' => $nonceStr,
				'timestamp' => $timestamp,
				'jsapi_ticket' => $this->wechat_ticket($appId,$appsecret),
				'url' => $request_link
			);
			...
			$result['wechat_config']=$wechat_data;
    	...
    	function wechat_ticket($appId,$appsecret,$openmodel_id=0){
		// access_token 应该全局存储与更新，以下代码以写入到文件中做示例
		$access_token_file=WWW_ROOT."data/access_token.json";
		...
		if (!isset($data['expire_time'])||$data['expire_time'] < time()) {
			...
			$access_token = isset($result['access_token'])?$result['access_token']:false;
			if ($access_token) {
				...
				fclose($fp);
				if(!empty($openmodel_id)){
					$this->OpenModel->save(array('id'=>$openmodel_id,'token'=>$access_token));
				}
			}
		...
		$jsapi_ticket_file=WWW_ROOT."data/jsapi_ticket.json";
		if(file_exists($jsapi_ticket_file)){
			$data = json_decode(file_get_contents($jsapi_ticket_file),true);
		}else{
			$data=array();
		}
		if (!isset($data['expire_time'])||$data['expire_time'] < time()) {
			...
			$result = json_decode($json,true);
			$ticket = isset($result['ticket'])?$result['ticket']:false;
			if ($ticket) {
				$data['expire_time'] = time() + 7000;
				...
				fwrite($fp, json_encode(array()));
				fclose($fp);
				return $this->wechat_ticket($appId,$appsecret);
			}
		}else if(isset($data['jsapi_ticket'])){

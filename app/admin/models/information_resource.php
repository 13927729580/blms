...
class InformationResource extends AppModel
{
     ...
     */
    public function information_formated($code, $locale, $parent_show = true,$system_module=array())
    {
        $condition = array();
        $condition['and']['InformationResource.code'] = $code;
        $condition['and']['InformationResource.status'] = '1';
        $condition['and']['InformationResourceI18n.locale'] = $locale;
        if(sizeof($system_module)>0){
        	foreach($system_module as $k=>$v){
        		$condition['and']['or'][]=array('InformationResource.system_code'=>$k,'InformationResource.module_code'=>'');
        		        if(isset($v['modules']) && is_array($v['modules'])>0){
			        	foreach($v['modules'] as $kk=>$vv){
			        		$condition['and']['or'][]=array('InformationResource.system_code'=>$k,'InformationResource.module_code'=>$kk);
			        	}
			        }
	       	}
        }
        ...
        $condition = array();
        $condition['and']['InformationResource.parent_id'] = $parent_id_array;
        $condition['and']['InformationResource.status'] = '1';
        $condition['and']['InformationResourceI18n.locale'] = $locale;
        if(sizeof($system_module)>0){
        	foreach($system_module as $k=>$v){

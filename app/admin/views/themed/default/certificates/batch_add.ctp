...
<?php echo $form->create('certificates',array('action'=>'/batch_add/','name'=>"theForm"));?>
<div class="tablelist"  class="tablelist am-u-md-12 am-u-sm-12">
	<table  id="t1" class="am-table  table-main">
		...
		<tr>
			...
			?>
				<td><?php if(in_array($fields_kk[1],$textarea_code)){ ?><textarea name="data[<?php echo $k?>][<?php echo $fields_kk[1]?>]" ><?php echo isset($v[$fields_desc])?$v[$fields_desc]:"";?></textarea><?php }else{ ?><input type='text' name="data[<?php echo $k?>][<?php echo $fields_kk[1]?>]" value="<?php echo isset($v[$fields_desc])?$v[$fields_desc]:"";?>" /><?php } ?></td>
			<?php }?>
		</tr>
		<?php }}?>
	</table>

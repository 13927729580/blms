...
function editaddr(Id){
	...
	$.ajax({ url: admin_webroot+"users/useraddress/"+user_id+"/edit",
		type:"POST",
		data:{'Id':Id},
		dataType:"html",
		success: function(data){
...
function deladdr(Id){
	if(confirm("<?php echo $ld['confirm_delete'] ?>")){
		var user_id=document.getElementsByName("data[User][id]")?document.getElementsByName("data[User][id]")[0].value:0;
		$.ajax({ url: admin_webroot+"users/useraddress/"+user_id+"/del",
			type:"POST",
			data:{'Id':Id},
			dataType:"json",
			success: function(data){
  ...
  <div class="am-tab-panel am-fade am-active am-in am-form-detail am-form am-form-horizontal" id="addredittables">
	...
	<div class="am-form-group">
      ...
      <div class="am-u-lg-8 am-u-md-8 am-u-sm-8"><input type="hidden" id="region_hidden_id0" value="<?php echo isset($user_addressInfo['UserAddress']['regions'])?$user_addressInfo['UserAddress']['regions']:''; ?>" name="data[UserAddress][regions]" >
        <span id="regionsupdate" style="float:none;">
		<select gtbfieldid="1" name="data[UserAddress][country]" id="AddressRegionUpdate00" onchange="reload_region(this)">
			<option><?php echo $ld['please_select']?></option>
			<option>...</option>
		</select>
		<select gtbfieldid="2" name="data[UserAddress][province]" id="AddressRegionUpdate01" onchange="reload_region(this)">
			<option><?php echo $ld['please_select']?></option>
			<option>...</option>
		</select>
		<select gtbfieldid="3" name="data[UserAddress][city]" id="AddressRegionUpdate02" onchange="reload_region(this)">
			<option><?php echo $ld['please_select']?></option>
			<option>...</option>
		</select>
		</span>
		<script type="text/javascript">
			var regions_add = <?php $user_address_data=isset($user_addressInfo['UserAddress'])?$user_addressInfo['UserAddress']:array();echo json_encode($user_address_data); ?>;
			load_region(regions_add);
		</script>
	  </div>
    </div>
    <div class="am-form-group">
	...
	<div class="am-form-group">
		...
		<div class="am-u-lg-8 am-u-md-8 am-u-sm-8"><a class="am-btn am-btn-success am-radius am-btn-sm" href="javascript:void(0);" onclick="saveaddr(this)"><?php echo $ld['save']; ?></a><a class="am-btn am-btn-success am-radius am-btn-sm" href="javascript:void(0);" onclick="clearaddr()"><?php echo $ld['cancel']; ?></a></div>
	</div>
	</form>
  </div>
...
<style type="text/css">
#addredittables #regionsupdate select,#addredittables #regionsupdate input.region_input{float:left;width:31%;margin-right:5px;}
...
function saveaddr(save_btn){
	var user_id=document.getElementsByName("data[User][id]")?document.getElementsByName("data[User][id]")[0].value:0;
	var addr_id=document.getElementsByName("data[UserAddress][id]")[0].value;
	document.getElementsByName("data[UserAddress][user_id]")[0].value=user_id;
	...
	var province_input="";
	if(typeof($(province_select).next('input.region_input')[0])!='undefined'){
		province_input=$(province_select).next('input.region_input')[0].value;
	}
	var city_select=document.getElementById("AddressRegionUpdate02");
	var city_input="";
	if(typeof($(city_select).next('input.region_input')[0])!='undefined'){
		city_input=$(city_select).next('input.region_input')[0].value;
	}
	...
	if(user_id==""||user_id==0){
	...
	}else if(province_select.selectedIndex==0&&province_input==""){
		alert(j_please_province);return false;
	}else if(city_select.selectedIndex==0&&city_input==""){
		...
		var post_form=$(save_btn).parents('form');
		var post_data=$(post_form).serialize();
		$.ajax({ url: admin_webroot+"users/useraddress/0/save",
			type:"POST",
			data:post_data,
			dataType:"json",
			success: function(data){

...
<div class="am-g">
	<div class="am-u-lg-2 am-u-md-3 am-u-sm-4">
		<ul class="am-list admin-sidebar-list" data-am-scrollspy-nav="{offsetTop: 45}" style="position: fixed; z-index: 1000; width: 15%;max-width:200px;">
			...
			<?php if(isset($Action) && sizeof($Action)>0){foreach($Action as $k=>$v){?>
				<li><a href="#<?php echo $v['Action']['name']?>"><?php echo $v['Action']['name']?></a></li>
			<?php }}?>
		</ul>
	</div>
	...
	<div class="am-panel-group admin-content" id="accordion" style="width:83%;float:right;">
		<div id="basic_information" class="am-panel am-panel-default">
		    <div class="am-panel-hd">
			    <h4 class="am-panel-title">
					...
					<input type="hidden" name="data[Role][id]" value="<?php echo $Role['Role']['id']?>" />			
				</h4>
			</div>
			    ...
			    <div class="am-panel-bd ">
			    	<div class="am-form-group">
						...
						<?php if(isset($backend_locales) && sizeof($backend_locales)>0){foreach ($backend_locales as $k => $v){?>
							<div class="am-u-lg-8 am-u-md-8 am-u-sm-8" style="margin-bottom:10px;">
								<input type="text" id="name<?php echo $v['Language']['locale']?>" name="data[RoleI18n][<?php echo $k;?>][name]" value="<?php if(!empty($Role['RoleI18n'][$v['Language']['locale']]['name'])){echo $Role['RoleI18n'][$v['Language']['locale']]['name'];}?>" />
							</div>
						...
						<?php if(isset($backend_locales) && sizeof($backend_locales)>0){foreach ($backend_locales as $k => $v){?>
							...
							<input name="data[RoleI18n][<?php echo $k;?>][locale]" type="hidden" value="<?php echo  $v['Language']['locale'];?>">
							<input id="RoleI18n<?php echo $k;?>Id" name="data[RoleI18n][<?php echo $k;?>][id]" type="hidden" value="<?php echo @$this->data['RoleI18n'][$v['Language']['locale']]['id'];?>">
							<input id="RoleI18n<?php echo $k;?>RoleId" name="data[RoleI18n][<?php echo $k;?>][operator_role_id]" type="hidden" value="<?php echo  $this->data['Role']['id'];?>">
							</div>
						<?php }}?>
					</div>
					<div class="am-form-group">
						...
						<div class="am-u-lg-6 am-u-md-6 am-u-sm-6">
							<div class="am-u-lg-8 am-u-md-8 am-u-sm-8">
								<?php echo $Role['Role']['id']?><input type="hidden" class="text_inputs"  name="data[Role][id]" value="<?php echo $Role['Role']['id']?>" />
							</div>
						</div>
					</div>
					<div class="am-form-group">
						...
						<div class="am-u-lg-6 am-u-md-6 am-u-sm-6">
							<div class="am-u-lg-8 am-u-md-8 am-u-sm-8">
								<input class="am-form-field am-radius" name="data[Role][orderby]" value="<?php echo $Role['Role']['orderby']?>"/>
					...
					<div class="am-form-group">
						...
						<div class="am-u-lg-6 am-u-md-7 am-u-sm-9">
							<label class="am-radio am-success"><input type="radio" name="data[Role][status]" value="1" data-am-ucheck checked style="margin-left:0px;" />&nbsp;<?php echo $ld['yes']?>&nbsp;&nbsp;</label>
							<label class="am-radio am-success"><input type="radio" name="data[Role][status]" value="0" data-am-ucheck style="margin-left:0px;" />&nbsp;<?php echo $ld['no']?></label>
						</div>
					</div>
					<div class="btnouter">
		...
		<?php if(isset($Action) && sizeof($Action)>0){foreach($Action as $k=>$v){?>
		<div id="<?php echo $v['Action']['name']?>" class="am-panel am-panel-default">
		    <div class="am-panel-hd">
			    <h4 class="am-panel-title">
				    <label class="am-checkbox am-success" style="font-weight:bold;">
				    	<input type="checkbox" data-am-ucheck  onclick='checkall(this)'/>
				    	<?php echo $v['Action']['name']?>
				   	</label>
			    </h4>
			</div>
			<div class="Action_list">
				...
				<div class="am-form-group" style="margin-left:9%">
					<label class="am-u-lg-2 am-u-md-3 am-u-sm-3 am-checkbox am-success " style="font-weight:bold;cursor: pointer;margin-left:12px;"> 
						<input type="checkbox" name='<?php echo "ops_".$v["Action"]["id"];?>' data-am-ucheck value="<?php echo $vv['Action']['id']?>" onclick="checktr(this)" />
						&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $vv['Action']['name']?>
					</label>
					...
					<?php if(isset($vv['SubAction']) && sizeof($vv['SubAction'])>0){?>
						...
						<?php foreach($vv['SubAction'] as $vvv){?>
							...
							<label class="am-checkbox am-success">
								<input type="checkbox" name="competence[]" data-am-ucheck value="<?php echo $vvv['Action']['id']?>" <?php if(in_array($vvv['Action']['id'],$actions_arr)) echo 'checked';?> />

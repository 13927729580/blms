...
<div class="am-g am-other_action" style="margin:10px 0 0 0">
    <div class="am-text-right  am-btn-group-xs ">
       <?php if(isset($open_type) && !empty($open_type)){ ?>
            <?php if($svshow->operator_privilege("open_menus_add")){echo $html->link($ld['synchronization_to_micro_message'],"javascript:void(0);",array("data-am-modal"=>"{target: '#open_type_select', closeViaDimmer: 0, width: 400, height: 180}","class"=>"am-btn am-btn-warning am-radius am-btn-sm "));}?>
        <?php }?> 
                ...
                <div class="am-panel-bd fuji">
                    ...
                    <div class="am-u-lg-2 am-u-md-3 am-u-sm-4 am-action" > 
                        ...
                        <span class="am-icon-pencil-square-o"></span> <?php echo $ld['edit']; ?>
		                            ...
		                            <a class="am-btn am-btn-default am-btn-xs am-text-danger am-seevia-btn-delete" href="javascript:;" onclick="list_delete_submit(admin_webroot+'open_menus/remove/<?php echo $v['OpenMenu']['id'] ?>')">
                    ...
                    <div class="am-panel-collapse am-collapse am-panel-child" id="openMenu_<?php echo $v['OpenMenu']['id']?>">
                        <?php $j=0;foreach($v['SubMenu'] as $kk=>$vv){$j++;?>
                            ...
                            <?php if($svshow->operator_privilege("open_menus_remove")){?>
                                    <a class="am-btn am-btn-default am-btn-xs am-text-danger shanchu am-seevia-btn-delete" href="javascript:;" onclick="list_delete_submit(admin_webroot+'open_menus/remove/<?php echo $vv['OpenMenu']['id'] ?>')">
                        <span class="am-icon-trash-o"></span> <?php echo $ld['delete']; ?>
                      </a>
                                 <?php   }?>
                                </div>
...
<div class="am-modal am-modal-no-btn" tabindex="-1" name="open_type_select" id="open_type_select">
    <div class="am-modal-dialog">
        <div class="am-modal-hd" style="border-bottom:1px solid #ddd"><?php echo $ld['synchronization_to_micro_message']?>
            <a href="javascript: void(0)" class="am-close am-close-spin" data-am-modal-close>&times;</a>
        </div>
        <div class="am-modal-bd am-form" style="padding-top:15px;">
                <div class="am-g">
                    <div class="am-u-lg-4 am-u-md-5 am-u-sm-5" style="line-height:32px;font-size:16px;">
                        <?php echo $ld['open_model_account'] ?>
                    </div>
                    <div class="am-u-lg-8 am-u-md-7 am-u-sm-7 am-text-left">
                        <select id="open_type_id" data-am-selected="{btnWidth:'80%'}">
                ...
                <div class="am-g am-margin-top-sm">
                        <input class="am-btn  am-btn-success am-radius am-btn-sm" type="button" onclick="api_menu_action()" value="<?php echo $ld['d_submit']?>" />
                </div>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    function api_menu_action(){
        ...
        if(open_type_id!=""){
            window.location.href=admin_webroot+"open_menus/api_menu_action/"+open_type_id;
        }
    }
    ...
    function changeOrder(updown,id,next,thisbtn){
        var sUrl = admin_webroot+"open_menus/changeorder/"+updown+"/"+id+"/"+next;//访问的URL地址
        $.ajax({
            type: "POST",
            url: sUrl,
            data:{},

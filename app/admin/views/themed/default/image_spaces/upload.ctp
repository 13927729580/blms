...
<p class="am-u-md-12" style="padding-right:0;"><?php echo $html->link($ld['picture_list'],"/image_spaces/",array("class"=>"am-btn am-btn-default am-radius am-btn-xs am-fr"),false,false);?></p>
        ...
        <div id="upload_setting" class="am-panel-collapse am-collapse am-in">
            <div class="am-panel-bd am-form-detail am-form am-form-horizontal" style="padding-bottom:0;">
                <table id="t1" class="am-table">
                    ...
                    <tr id='size' name='size' style='display:none'><th><?php echo $ld['big_picture']?></th><td><p class="c_txt"><?php echo $ld['width']?></p><input class='c_size' type='text' id='big_img_width'  value='<?php echo $big_img_width;?>' onchange="swf_upload_addr()" onkeyup="checknum(this)" /><p class="c_txt"> X <?php echo $ld['height']?></p><input class='c_size' type='text' id='big_img_height' onkeyup="checknum(this)" value='<?php echo $big_img_height;?>' onchange="swf_upload_addr()"/><p class="c_txt"><?php echo $ld['pixel']?></p><td></tr>
                    <tr id='size' name='size' style='display:none'><th><?php echo $ld['middle_picture']?></th><td><p class="c_txt"><?php echo $ld['width']?></p><input class='c_size' type='text' id='mid_img_width' onkeyup="checknum(this)" value='<?php echo $mid_img_width;?>' onchange="swf_upload_addr()"/><p class="c_txt"> X <?php echo $ld['height']?></p><input class='c_size' type='text' id='mid_img_height' onkeyup="checknum(this)" value='<?php echo $mid_img_height;?>' onchange="swf_upload_addr()"/><p class="c_txt"><?php echo $ld['pixel']?></p><td></tr>
                    <tr id='size' name='size' style='display:none'><th><?php echo $ld['small_picture']?></th><td><p class="c_txt"><?php echo $ld['width']?></p><input class='c_size' type='text' id='small_img_width' onkeyup="checknum(this)" value='<?php echo $small_img_width;?>' onchange="swf_upload_addr()"/><p class="c_txt"> X <?php echo $ld['height']?></p><input class='c_size' type='text' id='small_img_height' onkeyup="checknum(this)" value='<?php echo $small_img_height;?>' onchange="swf_upload_addr()"/><p class="c_txt"><?php echo $ld['pixel']?></p><td></tr>
                    ...
                    <tr><th ><?php echo $ld['upload_picture']?></th>
                        <td>
                            <?php if((isset($configs['image-watermake-upload'])&&$configs['image-watermake-upload']==0)||(!isset($configs['image-watermake-upload']))){
                                ?>
                                <input type="file" class="am-btn" id="file_upload" name="file_upload" style="padding-left:0;" />
                            <?php }else{ ?>
                                <input type="file"  class="am-btn" name="Filedata" onchange="ajaxFileUpload(this)" />
                            <?php } ?>
                        </td>
                    </tr>
                </table>
...
<div class="am-modal am-modal-no-btn" tabindex="-1" name="photocat" id="photocat">
	<div class="am-modal-dialog">
        <div class="am-modal-hd" style="border-bottom:1px solid #ddd"><?php echo $ld['add_category']?>
            <a href="javascript: void(0)" class="am-close am-close-spin" data-am-modal-close onclick="clr()">&times;</a>
        </div>
        <div class="am-modal-bd">
    		<form id='catform1' class="am-form" method="POST">
	            ...
	            <?php }}?>
	            <div class="am-g">
                    <div class="am-u-lg-4 am-u-md-4 am-u-sm-4 div_lineheight am-margin-top-xs am-text-left" style="padding-right:0;"><?php echo $ld['picture_category_name']?>:</div>
                    <div class="am-u-lg-8 am-u-md-8 am-u-sm-8">
                             <?php if(isset($backend_locales)&&sizeof($backend_locales)>0){foreach ($backend_locales as $k => $v){?>
                        <div class="am-u-sm-12 am-margin-top-xs" style="padding-left:0;">
                             <div class="am-u-sm-9" style="padding-left:0;">
                                <input id="name_<?php echo $v['Language']['locale'];?>" name="data[PhotoCategoryI18n][<?php echo $k;?>][name]" class="type-clear1" type="text" value=""><?php if(sizeof($backend_locales)>1){?>
                              </div>  
                              <div class="am-u-sm-1" style="line-height:37px;padding-left:0;">
	            ...
	            <div class="am-g am-margin-top-xs am-margin-bottom-sm">
                    <div class="am-u-lg-4 am-u-md-4 am-u-sm-4 div_lineheight am-text-left" style="padding-right:0;"><?php echo $ld['sort']?>:</div>

                    <div class="am-u-lg-8 am-u-md-8 am-u-sm-8" >
                        <div class="am-u-sm-12" style="padding-left:0;">
                        <div class="am-u-sm-9" style="padding-left:0;">
                        <input type="text" class="type-clear2" name="data[PhotoCategory][orderby]" value="<?php echo isset($photo_categories_info['PhotoCategory'])?$photo_categories_info['PhotoCategory']['orderby']:'';?>" />
                        </div>
                        </div>
                    </div>
            ...
            <input type="button" class="am-btn am-btn-success am-radius am-btn-sm" name="changeDomainRankButton" value="<?php echo $ld['confirm']?>" onclick="javascript:doinsertphotocat();clr();">
            </form>
		</div>
    </div>
</div>
...
function get_size(cat_id){
    $.ajax({
		...
		var image_custom_size=cat_id=='0'?'0':'1';
		$("input[name='custom_size'][value='"+image_custom_size+"']").uCheck('check');
		custom_size(image_custom_size);
            swf_upload_addr();
        }
    });
}
...
function clr(){
    $(".type-clear1").val("");
    $(".type-clear2").val("<?php echo isset($photo_categories_info['PhotoCategory'])?$photo_categories_info['PhotoCategory']['orderby']:'';?>");
}

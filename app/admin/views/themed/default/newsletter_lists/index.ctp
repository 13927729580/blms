...
<p class="am-u-md-12 am-text-right am-btn-group-xs">
    		...
    		if($svshow->operator_privilege("user_groups_view")){echo $html->link($ld['user_group'],'/user_groups/',array("class"=>"am-btn am-radius am-btn-sm am-btn-default "),false,false);}echo"&nbsp;";
    	      if($svshow->operator_privilege('users_add')){
    	      	  if(isset($profile_id) && !empty($profile_id) ){
    	      	  echo $html->link($ld['batch_upload_user'],'/newsletter_lists/newsletter_list_upload',array("class"=>"am-btn am-btn-default am-radius am-btn-sm "),false,false);
    	      	  }
    	      }
    	      	  	  ?>
    ...
    <?php if($svshow->operator_privilege("newsletter_lists_status") && $svshow->operator_privilege("newsletter_lists_remove")){?>
        <?php if(isset($newsletterlist_data) && sizeof($newsletterlist_data)){?>
            <div id="btnouterlist" class="btnouterlist">
            	<div class="am-u-lg-6 am-u-md-6 am-u-sm-12   am-hide-sm-down" style="margin-left:-2px;">
	                ...
	                <div class=" am-u-lg-5 am-u-md-6  am-fl"><select  id="barch_opration_select" data-am-selected onchange="barch_list_opration_select_onchange(this)">
					...
					<?php } ?>
	                  ...
	                  <div class="am-fl" style="display:none;margin-left:3px;">
                       ...
                       <div class=" am-u-lg-5 am-u-md-3   am-fl"><input type="button" class="am-btn am-btn-danger am-radius am-btn-sm" onclick="select_batch_operations(this);" value="<?php echo $ld['submit']?>" id="act_type" name="act_type" />
                      </div>
...
function select_batch_operations(){
	var barch_opration_select = document.getElementById("barch_opration_select");
      ...
      if(barch_opration_select.value==0){
       ...
       if(barch_opration_select.value=='unsubscribe'){
		batch_change_status('unsubscribe');
	}
      if(barch_opration_select.value=='confirm'){
		batch_change_status('confirm');
	}
	if(barch_opration_select.value=='delete'){
		batch_change_status('remove');
	}
	if(barch_opration_select.value=='export_csv'){
		if(export_csv.value=='all_export_csv'){
			window.location.href=admin_webroot+"/newsletter_lists/all_export_csv";
...
function choice_upload(){
	...
	window.location.href=admin_webroot+"newsletter_lists/choice_export/"+postData;
	
	}
}	

//触发子下拉
function barch_list_opration_select_onchange(obj){
...
function batch_operations(){
	...
	if(confirm("<?php echo $ld['confirm_delete'] ?>")){
		$.ajax({
			url:admin_webroot+"newsletter_lists/batch_operations/",
			type:"POST",
			data:postData,
			dataType:"json",
    ...
    function newsletter_list_export(){
        ...
        window.location.href = admin_webroot+"newsletter_lists/export/?group_id="+group_id+"&email="+email+"&date="+date+"&date2="+date2;
    }
</script>
<style>
 ...
 .am-radio input[type="radio"]{margin-left:0px;padding-top:2px;}
  .am-u-lg-1 .am-dropdown{margin-top:10px;} 
  header .am-u-lg-1 .am-dropdown{margin-top:0px;}
  .scrollspy-nav {
    top: 0;
    z-index: 100;
    background: #5eb95e;
			...
			<div id="basic_information" class="am-panel am-panel-default">
			    ...
			    <div class="am-panel-collapse am-collapse am-in">
					  ...
					  <div class="am-form-group">
			    			...
			    			<div class="am-u-lg-7 am-u-md-7 am-u-sm-7"><div>
			    			  <input type="text" class="url-addr" name="data[FlashImage][url]" value="<?php echo isset($flash_image_info['FlashImage'])?$flash_image_info['FlashImage']['url']:'';?>" /> <?php echo $ld['page_url_desc']; ?>
			    			  </div></div>
			    		</div>	
                                     <div class="am-form-group">
...
typechange();
	...
	function searchInforationresources(code){
		...
		YUI().use('io',function(Y){
			var sUrl = admin_webroot+"information_resources/searchInforationresources/";//访问的URL地址
	...
	function removeInforationresources(id){
		YUI().use('io',function(Y){
			var sUrl = admin_webroot+"information_resources/removeInforationresources/";//访问的URL地址
			var postData = "id="+id;
			var cfg = {
					method: 'POST',
					data: postData
	...
	function editInforationresources(id){
		YUI().use('io',function(Y){
			var sUrl = admin_webroot+"information_resources/editInforationresources/";//访问的URL地址
			if(id!=""){
	...
	function updateInformationresources(code){
		YUI().use('io',function(Y){
			var sUrl = admin_webroot+"information_resources/updateInformationresources/";//访问的URL地址
			var postData = "code="+code;
			var cfg = {
				method: 'POST',
				data: postData
	...
	function checkform(){
		...
		if(flashImg.src==""||flashImg.src.indexOf(admin_webroot+"skins/default/img/default_no_photo.png")>0){
			alert('请选择轮播图片');
			return false;
		}
//		var match = /^((ht|f)tps?):\/\/[\w\-]+(\.[\w\-]+)+([\w\-\.,@?^=%&:\/~\+#]*[\w\-\@?^=%&\/~\+#])?$/;
//		var urladdr = $(".url-addr").val();
//		if(match.test(urladdr) == false){
//			alert('请输入正确的url地址');
//			return false;
//		}
	}
	...
	function clear_flashInfo(){
		...
		show_flash_image.src=admin_webroot+"skins/default/img/default_no_photo.png";
	}
</script> 

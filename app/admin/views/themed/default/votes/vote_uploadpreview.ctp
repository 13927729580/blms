...
<?php echo $form->create('votes',array('action'=>'/vote_uploadpreview/','name'=>"theForm"));?>
<div id="tablelist" class="tablelist am-u-md-12 am-u-sm-12">
	<table id="t1" class="am-table  table-main">
		...
		<tr>
			...
			<?php foreach($key_arr as $kk => $vv){?>
				<?php $fields_ks=explode('.',$key_arr[$kk]); if($fields_ks[0]=='VoteOption'||$fields_ks[0]=='VoteOptionI18n'){ ?>
				<td><textarea name="data[<?php echo $k?>][<?php echo $fields_ks[0]; ?>][<?php echo $fields_ks[1]; ?>]"><?php echo isset($v[$vv])?$v[$vv]:'';?></textarea></td>
				<?php }else{ ?>
				<td><input type='text' name="data[<?php echo $k?>][<?php echo $fields_ks[0]; ?>][<?php echo $fields_ks[1]; ?>]" value="<?php echo isset($v[$vv])?$v[$vv]:'';?>" /></td>
				<?php } ?>
			<?php }?>
	...
	<div id="btnouterlist" class="btnouterlist" style="margin-left:0;">
		<div>
			...
			<button type="reset"  class="am-btn am-btn-success am-radius am-btn-sm"  ><?php echo $ld['d_reset']?></button>
		</div>
	</div>
</div>
<?php $form->end();?>
...
{
document.theForm.action=admin_webroot+"votes/vote_uploadpreview";

...
<?php echo $form->create('configs',array('action'=>'/config_uploadpreview/','name'=>"theForm"));?>
	...
	<table id="t1" class="am-table  table-main">
		...
		<tr>
			...
			<?php foreach($key_arr as $kk => $vv){?>
				...
				<?php	if($fields_ks[1]=='options'){   ?>
				<td><textarea class="user_<?php echo""?>" name="data[<?php echo $k?>][<?php echo $fields_ks[0]; ?>][<?php echo $fields_ks[1]; ?>]"  ><?php echo isset($v[$vv])?$v[$vv]:'';?></textarea></td>
				<?php }else{  ?>
...
{
document.theForm.action=admin_webroot+"configs/config_uploadpreview";

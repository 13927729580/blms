...
<?php echo $form->create('products',array('action'=>'/batch_add_products/','name'=>"theForm"));
		...
		$textarea_code=array('description','description02','meta_description','seller_note');
?>
	...
	<table id="t1" class="am-table  table-main">
		<tr>
			...
			<?php foreach($profilefiled_info as $thk => $thv){
					$field_code_arr=explode(".",$thv['ProfileFiled']['code']);
					$field_code=isset($field_code_arr[1])?$field_code_arr[1]:'';
					if(!in_array($field_code,$preview_key_code))continue;?>
		...
		<tr>
			...
			<?php foreach($profilefiled_info as $kk => $vv){
					$field_code_arr=explode(".",$vv['ProfileFiled']['code']);
					$field_code=isset($field_code_arr[1])?$field_code_arr[1]:'';
					if(!in_array($field_code,$preview_key_code))continue;
					$fields_desc=$vv['ProfilesFieldI18n']['description'];
					$data_name=isset($key_code[$fields_desc])?$key_code[$fields_desc]:'';
            		?>
			<td><?php if(!in_array($field_code,$textarea_code)){ ?><input type='text' class="user_<?php echo $field_code; ?>" name="data[<?php echo $k?>][<?php echo $field_code; ?>]" value="<?php echo isset($v[$field_code])?$v[$field_code]:"";?>" /><?php }else{ ?><textarea  class="user_<?php echo $field_code; ?>" name="data[<?php echo $k?>][<?php echo $field_code; ?>]" ><?php echo isset($v[$field_code])?$v[$field_code]:"";?></textarea><?php } ?></td>
			<?php }?>
			<?php  if(isset($titer_arr)&&$titer_arr!=""){foreach($show_attr_code_arr as $attrk => $attrv){ 
	   	        if(in_array($attrv,$titer_arr)){ ?><td><input type='text' class='product_<?php echo $attrv; ?>' name='data[<?php echo $k;?>][ProductAttribute][<?php echo $attrv; ?>]'   value='<?php echo isset($v[$attrv])?htmlspecialchars($v[$attrv]):''; ?>'/></td>
	   	         <?php }} ?>
		  </tr>
		<?php }}}?>
	</table>

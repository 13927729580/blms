			...
			<div class="am-panel-collapse am-collapse am-in">
			    <div class="am-panel-bd ">
			       <label class="am-u-lg-2 am-u-md-3 am-u-sm-3 am-form-group-label"> </label>
			         	<div class="am-u-lg-10 am-u-md-9 am-u-sm-9" style="">
				    <?php if(isset($backend_locales)&&sizeof($backend_locales)>0){foreach ($backend_locales as $k => $v){?>
					<?php
						if($configs["show_edit_type"]){?>
						<div class="am-form-group" style="margin-left:15px;">
...
function check_unique(attr_id,attr_code){
	$.ajax({
		url:admin_webroot+"attributes/check_attr_unique/",
		type:'POST',
		dataType:"json",
...
function loadAttrOption(){
    ...
    if(attr_id!="0"){
    	$.ajax({
    		url:admin_webroot+"attributes/attribute_option/"+attr_id,
    		type:"POST",
    		dataType:"html",
    		data:{ },
    		success:function(data){

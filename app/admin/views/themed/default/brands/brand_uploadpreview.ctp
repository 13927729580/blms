...
.btnouterlist input{position: relative;*position:static;}
...
<?php echo $form->create('brands',array('action'=>'/brand_uploadpreview/','name'=>"theForm"));?>
	...
	<table id="t1" class="am-table  table-main">
		...
		<tr>
			...
			<?php foreach($key_arr as $kk => $vv){?>
				...
				<td><?php if($fields_ks[1]=='description'){ ?><textarea name="data[<?php echo $k?>][<?php echo $fields_ks[0]; ?>][<?php echo $fields_ks[1]; ?>]" ><?php echo isset($v[$vv])?$v[$vv]:'';?></textarea><?php }else{ ?><input type='text' name="data[<?php echo $k?>][<?php echo $fields_ks[0]; ?>][<?php echo $fields_ks[1]; ?>]" value="<?php echo isset($v[$vv])?$v[$vv]:'';?>" /><?php } ?></td>
			<?php }?>
		</tr>
		<?php }}?>
	</table>
...
{
document.theForm.action=admin_webroot+"brands/brand_uploadpreview";

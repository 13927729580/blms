...
<div id="tablelist" class="tablelist am-u-md-12 am-u-sm-12">
    ...
    <table class="am-table  table-main">
        ...
        <tr>
            ...
            <th style="width:25%"> <?php echo $ld['title']?> </th>
            <th class="thwrap am-hide-md-down"><?php echo $ld['custom_path']?></th>
            <th style="width:5%;" class="thwrap am-hide-sm-down"><?php echo $ld['type']?></th>
        ...
        <?php if(isset($element_list) && sizeof($element_list)>0){foreach($element_list as $k=>$v){?>
            <tr>
                ...
                <?php if(!empty($v['OpenElement']['media_url'])){echo $html->image($v['OpenElement']['media_url'].'?date='.time(),array('style'=>'width:105px;','class'=>'media_Img media_url'));}?></label></td>
                ...
                <td class="am-action">
                   ...
                   <?php  if($svshow->operator_privilege("open_elements_edit")){?> 
                    ...
                    <?php }
                    ?>
                    <a class="am-btn am-btn-default am-btn-xs am-text-secondary am-seevia-btn" onclick="upload_element('<?php echo $v['OpenElement']['id']; ?>','<?php echo $v['OpenElement']['element_type']; ?>')" href="javascript:void(0);">
                        <span class="am-icon-pencil-square-o"></span> <?php echo isset($openmedia_list[$v['OpenElement']['id']])?$ld['updates_to_platform']:$ld['uploaded_to_platform']; ?>
                    </a>
                </td>
            </tr>
        <?php }}else{?>
...
<div class="am-modal am-modal-no-btn" tabindex="-1" id="upload_element">
  <div class="am-modal-dialog">
    <div class="am-modal-hd" style="border:1px solid #ddd"><?php echo $ld['upload'] ?>
      <a href="javascript: void(0)" class="am-close am-close-spin" data-am-modal-close>&times;</a>
    </div>
    <div class="am-modal-bd" style="padding-top:15px;">
    		<form id="upload_element_form" class="am-form">
    			<input type="hidden" name="element_id" value="0">
    			<input type="hidden" name="element_type" value="0">

	    			<div class="am-g">
	    				<div class="am-u-sm-12">
                            <select  name="open_type_id"  data-am-selected="{btnWidth:'50%'}">
	    			...
	    			<div class="am-g am-margin-top-sm">
	    					...
	    					<span id="upload_info" style="display:none;">封面图：<i id="num"></i>/<i id="total"></i></span>
	    					<span id="content_upload_info" style="display:none;">内容图：<i id="content_num"></i>/<i id="content_total"></i></span>
	    			</div>
...
function ajax_upload_element_bak(btn_obj){
...
function ajax_upload_element(btn_obj){
	...
	if(open_type_id!='0'){
		if(confirm(confirm_operation)){
			 $(btn_obj).button('loading');
			 var sUrl = admin_webroot+"open_elements/ajaxGetElement/";//访问的URL地址
		        $.ajax({
		            type: "POST",
		            url: sUrl,
		            dataType: 'json',
		            ...
		            success: function (result) {
		                	if(result.code=='1'){
		                		var id = 0;
		                		var length = result.img_num;
		                		$('#num').html(id);
		                		$('#total').html(length);
		                		$('#upload_info').show();
		                		ajax_upload_image(id,length);
		                	}else{
		                		alert(result.msg);
		                	}
		            },
...
var img_media = new Array();
function ajax_upload_image(id,length){

	if(id >= length) {
		//ajax_element_upload(img_media);
		ajax_get_total_image(img_media);
		return;
	}
	var element_type = $("#upload_element input[name='element_type']").val();
	var element_id = $("#upload_element input[name='element_id']").val();
	var open_type_id = $("#upload_element select[name='open_type_id']").val();
	$.ajax({
		type:'POST',
		url:admin_webroot+"open_elements/ajaxGetUploadImage/",
		dataType:'json',
		data:{'key':id,'element_type':element_type,'element_id':element_id,'open_type_id':open_type_id},
		success:function(result){
			if(result.code == 0){
				alert(result.msg);
				return;
...
function ajax_get_total_image(img_media){
	var element_type = $("#upload_element input[name='element_type']").val();
	var element_id = $("#upload_element input[name='element_id']").val();
	var open_type_id = $("#upload_element select[name='open_type_id']").val();
	$.ajax({
		type:'POST',
		url:admin_webroot+"open_elements/ajaxGetTotalImageByContent/",
		dataType:'json',
		data:{'element_type':element_type,'element_id':element_id,'open_type_id':open_type_id,'img_media':img_media},
		success:function(result){
			if(result.code == 1){
				var id = 0;
		        var length = result.data.length;
		        $('#content_num').html(id);
		        $('#content_total').html(length);
		        $('#content_upload_info').show();
				
				ajax_upload_content_image(id,length);
			}else
				alert(result.msg);
		},
		error:function(){
...
function ajax_upload_content_image(id,length){
	...
	$.ajax({
		...
		url:admin_webroot+"open_elements/ajaxGetTotalImageByContent/",
		dataType:'json',
		data:{'key':id,'element_type':element_type,'element_id':element_id,'open_type_id':open_type_id,'img_media':img_media},
		success:function(result){
			if(result.code == 1){
				id += 1;
				$('#content_num').html(id);
...
function ajax_element_upload(img_media){
	var element_type = $("#upload_element input[name='element_type']").val();
	var element_id = $("#upload_element input[name='element_id']").val();
	var open_type_id = $("#upload_element select[name='open_type_id']").val();
	$.ajax({
		type:'POST',
		url:admin_webroot+"open_elements/ajaxUploadElement/",
		dataType:'json',
		data:{'element_type':element_type,'element_id':element_id,'open_type_id':open_type_id,'img_media':img_media},
		success:function(result){
			if(result.code == 1)

            ...
            <tr>
                ...
                <td  ><?php
                    if($svshow->operator_privilege("scores_edit")){
                        if($v['Score']['status'] == 1){
                            echo $html->image('/admin/skins/default/img/yes.gif',array('style'=>'cursor:pointer;','onclick'=>'listTable.toggle(this, "scores/toggle_on_status", '.$v["Score"]["id"].')'));
                        }else{
                            echo $html->image('/admin/skins/default/img/no.gif',array('style'=>'cursor:pointer;','onclick'=>'listTable.toggle(this, "scores/toggle_on_status", '.$v["Score"]["id"].')'));
                        }
                    }else{
                        if($v['Score']['status'] == 1){
                            echo $html->image('/admin/skins/default/img/yes.gif',array('style'=>'cursor:pointer;'));
                        }else{
                            echo $html->image('/admin/skins/default/img/no.gif',array('style'=>'cursor:pointer;'));
                        }
                    }
                    ?>
                </td>
                <td class="am-action"><?php if($svshow->operator_privilege("scores_edit")){?>
                  ...
                  <?php   }
                    if($svshow->operator_privilege("scores_delete")){?>
                        		...
                        		<a class="am-btn am-btn-default am-btn-xs am-text-danger am-seevia-btn-delete" href="javascript:void(0);" onclick="list_delete_submit(admin_webroot+'scores/remove/<?php echo $v['Score']['id'] ?>')">

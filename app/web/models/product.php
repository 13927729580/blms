...
class product extends AppModel
{
    ...
    public function find_svcart_products_list($id)
    {
        ...
        if (isset($svcart_products) && sizeof($svcart_products) > 0) {
            ...
            foreach ($svcart_products as $k => $v) {
                ...
                $package_product_list=$this->get_product_package_list($params);
                ...
                $svcart_products_list[$v['Product']['id']]['package_product'] = $package_product_list;
                $svcart_products_list[$v['Product']['id']]['package_product_total'] = $package_product_total;
            }
        }
    ...
    public function find_all_products($options)
    {
        ...
        if (!empty($options['set'])) {
            ...
            $this->viewVars[1][$options['set2']] = $products;
            return $products;
        } else {
            return $products;
        }
    ...
    public function get_module_pro_info($params)
    {
        ...
        if ($this->check_product($params['id'])) {
            $pro_fields = array('Product.*,ProductI18n.name,ProductI18n.description,ProductI18n.description02,ProductI18n.seller_note,ProductI18n.seller_note,ProductI18n.delivery_note,ProductI18n.meta_keywords,ProductI18n.meta_description,ProductI18n.api_site_url,ProductI18n.api_cart_url');
...
//					$pkg_sale_pro=$Attribute->find('all',array('conditions'=>$attr_cond));
    ...
    public function get_module_pro_detail($params)
    {
        ...
        $conditions['Product.forsale'] = 1;
        if ($this->check_product($params['id'])) {
            $product_detail = $this->find('first', array('conditions' => $conditions, 'order' => 'Product.'.$order, 'fields' => 'Product.id,Product.brand_id,Product.category_id,ProductI18n.name,ProductI18n.seller_note,ProductI18n.description'));
            $result['product_detail']=$product_detail;
            if(!empty($product_detail)){
			$Brand = ClassRegistry::init('Brand');
			$BrandInfo = $Brand->find('first', array('conditions' => array('Brand.id' => $product_detail['Product']['brand_id'])));
			if (!empty($BrandInfo)) {
    ...
    /**
     *函数get_module_pro_brand 获取该商品所在品牌下的其他商品.
     *
     * @param  查询参数集合
     *
     * @return $product_relation 根据param，返回其他商品数组
     */
    public function get_module_pro_brand($params){
    		$conditions = '';
        $limit = 10;
        if (isset($params['limit'])) {
            $limit = $params['limit'];
        ...
        if (isset($params['brand_id'])&&!empty($params['brand_id'])) {
            $conditions['Product.brand_id'] = $params['brand_id'];
        }else{
        ...
        if ($this->check_product($params['id'])) {
            ...
            if (!empty($comment_infos)) {
                ...
                if(isset($_SESSION['User'])){
	                    ...
	                    $comment_infos['user_comment_count']=$ProductComment->find('count',array('conditions' => $conditions));
                }
    ...
    */
    public function get_product_package_list($params)
    {
        ...
        $cond['fields'] = array('Product.id','Product.code','Product.img_thumb','Product.img_detail','Product.shop_price','Product.product_type_id','PackageProduct.package_product_qty','PackageProduct.package_product_name');
        $cond['joins'] = array(
            array('table' => 'svoms_products',
                  'alias' => 'Product',
                  'type' => 'right',

...
//品牌详细
    ...
    public function findassoc($locale = '')
    {
        ...
        if ($lists_formated) {
            ...
            $lists = $this->find('all', array('order' => array($orderby, 'BrandI18n.name asc'),
                        'fields' => array('Brand.id','Brand.url', 'Brand.flash_config', 'Brand.url', 'Brand.img01', 'Brand.modified', 'BrandI18n.img01', 'BrandI18n.name',
                        ),
    ...
    public function get_module_infos($params)
    {
        ...
        $first_letter_keyword="";
        if (isset($params['first_letter_keyword'])&&trim($params['first_letter_keyword'])!="") {
        	$first_letter_keyword=trim($params['first_letter_keyword']);
        	$conditions['and']['Brand.code like'] = $first_letter_keyword."%";
        }
        $brand_keyword="";
        if (isset($params['brand_keyword'])&&$params['brand_keyword']!="") {
        	$brand_keyword=trim($params['brand_keyword']);
        	$conditions['and']['or']['Brand.code like'] = "%".$brand_keyword."%";
        	$conditions['and']['or']['BrandI18n.name like'] = "%".$brand_keyword."%";
        }
        $conditions['and']['Brand.status'] = 1;
        //分页start
        ...
        //get参数
        $parameters['get'] = array('first_letter_keyword'=>$first_letter_keyword,'brand_keyword'=>$brand_keyword);
        ...
        $brand_infos = $this->find('all', array('conditions' => $conditions, 'limit' => $limit, 'order' => 'Brand.'.$order, 'fields' => array('Brand.id','Brand.url', 'BrandI18n.name', 'BrandI18n.img01'), 'page' => $page));
    ...
    public function get_module_brand_infos($params)
    {
        ...
        $brand_infos = $this->find('first', array('fields' => array('Brand.id','Brand.url', 'Brand.created', 'BrandI18n.name', 'BrandI18n.description', 'BrandI18n.meta_keywords', 'BrandI18n.img01'), 'conditions' => $conditions, 'order' => $order, 'limit' => $limit));
    ...
    public function get_module_brand_flash($params)
    {
		...
		$conditions['Flash.page_id'] = 0;
		$conditions['Flash.page'] = 'BL';
        }
    ...
    public function get_recommended_brand($params=array()){
    	 $conditions = '';
        $limit = 10;
        if (isset($params['limit'])) {
            $limit = $params['limit'];
        ...
        $conditions['Brand.recommand_flag'] = '1';
        $conditions['Brand.status'] = '1';
        $brand_infos = $this->find('all', array('fields' => array('Brand.id','Brand.url', 'Brand.created', 'BrandI18n.name', 'BrandI18n.description', 'BrandI18n.meta_keywords', 'BrandI18n.img01'), 'conditions' => $conditions, 'order' => $order));
        return $brand_infos;
    }
}

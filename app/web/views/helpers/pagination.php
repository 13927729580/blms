 ...
 */
    ...
    public function setPaging($paging)
    {
        ...
        unset($getParams['url']);if(isset($getParams['ext'])&&!isset($_REQUEST['ext'])){unset($getParams['ext']);}
        if(isset($paging['importParams']['get'])&&!empty($paging['importParams']['get']))$getParams=array_merge($getParams,$paging['importParams']['get']);
    ...
    public function amaze_pageNumbers($separator = null, $escapeTitle = true, $spacerLower = '...', $spacerUpper = '...')
    {
        if (empty($this->_pageDetails) || $this->_pageDetails['pageCount'] == 1) {
            return "<li class='am-disabled'>1</li>";
        }
        $total = $this->_pageDetails['pageCount'];
        $max = $this->maxPages;
        $span = $this->pageSpan;
        ...
        for ($i = $lowerLimit; $i <= $upperLimit; ++$i) {
            if ($i == $this->_pageDetails['page']) {
                $text = '<li class="am-disabled">'.$i.'</li>';
            } else {
                $text = $this->_generateLink($i, $i, $escapeTitle);
            }
            $t[] = $text;
      ...
      * @return string html for the list of page numbers
      **/
     public function pageNumbers2($separator = null, $escapeTitle = true, $spacerLower = '...', $spacerUpper = '...')
     {
         if (empty($this->_pageDetails) || $this->_pageDetails['pageCount'] == 1) {
             return '<em>1</em>';
         }
         ...
         for ($i = $lowerLimit; $i <= $upperLimit; ++$i) {
             if ($i == $this->_pageDetails['page']) {
                 $text = "<option value='".$this->get_url(array('page' => $i))."' selected>".$i.'</option>';
             } else {
                 $text = "<option value='".$this->get_url(array('page' => $i))."'>".$i.'</option>';
             }
             $t[] = $text;
         }
         if ($dottedUpperLimit) {
         ...
         $t = "<select onchange='javascript:window.location=this.value'>".implode('', $t).'</select>';
     ...
     **/
    public function prevPage($text = 'prev', $escapeTitle = true)
    {
        if (empty($this->_pageDetails)) {
            return false;
        }
     ...
     **/
    public function nextPage($text = 'next', $escapeTitle = true)
    {
        if (empty($this->_pageDetails)) {
            return false;
        }
     ...
     **/
    public function _generateLink($title, $page = null, $escapeTitle, $pageDetails = null)
    {
    ...
    public function get_url($options = array())
    {
        $url = '/';
        $prettyString = $this->_pageDetails['importParams']['route'];
        foreach ($options as $k => $v) {
            if (isset($prettyString[$k])) {
                $prettyString[$k] = $v;
            }
        }
        //$prettyString['page'] = $page;
        ...
        $getString = $this->_pageDetails['importParams']['get'];
        if ($getString) {
            $getString = implode('&', $getString);
            $url .= '?'.$getString;
        }

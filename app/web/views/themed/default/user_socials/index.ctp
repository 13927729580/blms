	  ...
	  <div class="ds-post-options ds-gradient-bg">
		<span class="func" style="float:right;margin-right:10px;">
			<span class="tongbu"><span class="share_wz" style=""><?php echo $ld['simultaneously_published_to'] ?></span>
				...
				?>
				<a style="" class="share_icon"  onclick="checktoken('<?php echo $v['UserApp']['type']; ?>')" href="javascript:void(0);">
					<?php echo $html->image('/theme/default/img/'.$v['img'],array('id'=>$v['UserApp']['type']."_icon",'style'=>'width:16px;height:16px;'.($v['status']=='0'?'filter: Alpha(opacity=10);-moz-opacity:.1;opacity:0.3;':'')));  ?></a>
				<?php }} ?>
			</span>
		</span>
		<span class="kind_detail">
			...
			<?php 
				...
				echo "<div class='picks' id='[@F_".($k+1)."@]'><img src='".$webroot."theme/default/img/gif/F_".($k+1).".gif' title='".$v."' /></div>";
				}
			?>
					<div style="clear:both"></div></div>
			<div id="pictureFile" name="upfile">
				<div id="pictureFilebox" name="upfile" style="width:100%;">
					<span class="btn_close"><img src="<?php echo $webroot; ?>theme/default/img/btn_close_img.png"/></span>
					<div id="pictureFileName"  name="upfile" style="" ><input id="uppic" type="file" name="upfile" accept="image/png,image/gif,image/jpg" onchange="filesize(this)" /></div>
				</div>
			</div>
			</a>
		</span>
...
<div id="cddtab" class="am-tabs" data-am-tabs="{noSwipe: 1}">
  ...
  <div class="am-tabs-bd">
    <div class="am-tab-panel <?php if($pagetype=='all'||$pagetype=='diary'||$pagetype=='action'){echo 'am-active';}else{echo '';} ?>" id="tab2-1">
	  ...
	  <?php if(isset($log_set) && ($log_set==0) ||($log_set==1 && !empty($fan_like)) || $fan_like==1){?>
	  <ul class="am-comments-list am-comments-list-flip">
		...
		<li class="am-comment">
		  <a target="_blank" title="<?php echo $v['User']['name'];?>" href="<?php echo $html->url('/user_socials/index/'.$v['User']['id']); ?>">
		    <?php echo $html->image(isset($v['User']['img01'])&&$v['User']['img01']!=""?$v['User']['img01']:('theme/default/img/no_head.png'),array('alt'=>$v['User']['name'],'class'=>'am-comment-avatar','width'=>'48','height'=>'48'));  ?>
  		  </a>
		  <div class="am-comment-main">
		    ...
		    <div class="am-comment-bd">
			  ...
			  <figure data-am-widget="figure" class="am am-figure am-figure-default am-u-lg-12" data-am-figure="{  pureview: 'auto' }">
  				<?php echo $html->image($v['Blog']['img'],array('data-rel'=>$v['Blog']['img'],'width'=>'100%'));  ?>
			  </figure>
			  </div>
			  <?php } ?>
			  ...
			  <div class="comment_box_<?php echo $v['Blog']['id'];?>" style="display:none;">
				...
				<?php if(isset($comment_set) && ($comment_set==0) ||($comment_set==1 && !empty($fan_like)) || $fan_like==1){?>
				<div class="comments_list" style="<?php echo isset($_SESSION['User'])?'':'padding:0px;'; ?>"></div>
				<?php }else if($fan_like !=1 && $comment_set==2){ echo $ld['privacy_and_confidentiality'];	}else{echo $ld['without_permission'];}?>
			  </div>
			</div>
		  </div>
		</li>
	  ...
	  <?php }else if($fan_like !=1 && $log_set==2){ echo $ld['privacy_and_confidentiality'];}else{echo $ld['without_permission'];}?>
	  </div>
	  <!--所有end-->
	  <!--日志-->
    ...
    <div class="am-tab-panel <?php if($pagetype=='baobei'){echo 'am-active';}else{echo '';} ?>" id="tab2-2">
	  ...
	  <div style="display:block;" class="productslist like_list">
		...
		<?php if(isset($like_set) && ($like_set==0) ||($like_set==1 && !empty($fan_like)) || $fan_like==1){?>		
		...
		<?php }else if($fan_like !=1 && $like_set==2){ echo $ld['privacy_and_confidentiality'];	}else{echo $ld['without_permission'];}?>
	  </div>
			<div style="display:none" class="productslist buy_list">
				...
				<li class="item" id="item<?php echo $v['Product']['id'];?>">
					<div class="item_box">
						...
						<span class="dislike_icon" style="margin-left:10px;margin-top:-200px;position: absolute;cursor:pointer;"><?php echo $html->image('/theme/default/img/like_icon.png',array('id'=>$v['Product']['id'],'style'=>'width:15px;height:15px;'));  ?><span style="margin-left:7px;position: absolute;font-size: 11px;top: -2px;" id="<?php echo 'like_num'.$v['Product']['id'];?>" class="like_num"><?php echo $v['Product']['like_stat'];?></span></span>
...
// 	if ($("#input_detail").selectionStart || $("#input_detail").selectionStart == '0') { //obj是文本框对象
// 		console.log('111');
// 	    var startPos = $("#input_detail").selectionStart;
// 	    var endPos = $("#input_detail").selectionEnd;
// 	    $("#input_detail").value = $("#input_detail").value.substring(0,startPos) + "要插入的字符串" + $("#input_detail").value.substring(endPos,$("#input_detail").value.length);
// 	    // $("#input_detail").selectionStart = startPos + myValue.length;
// 	    // $("#input_detail").selectionEnd = startPos + myValue.length;
// 	} else {
// 		console.log('222');
// 	    $("#input_detail").val($("#input_detail").val()+"要插入的字符串");
// 	}
// 	// console.log($("#input_detail"))
// 	// if ($("#input_detail").selection) {
// 	//     pos = $("#input_detail").selection.createRange();
// 	//     pos.text = "要插入的字符串";
// 	// }
// })
	...
	var Url="<?php echo $server_host; ?>"+web_base+"/theme/default/img/gif/";//表情图片路径
	//表情数组
...
$(document).ready(function(){
	...
	$(".like_list").on("click",'.dislike_icon img',function(){
    	...
    	if(js_login_user_data==null){
    		...
    		if (confirm("真的要删除赞的宝贝吗?")){
    			//已登录
    			$.ajax({ 
    					url: web_base+"/user_socials/ajax_dislike",
    					type:"POST",
    					dataType:"html", 
    ...
    $(".WB_handle a").click(function(){
    	...
    	if(comment_box.css("display") == "none"){
    		...
    		$.ajax({ 
    			// url: "<?php echo $html->url('/user_socials/article_comment/')?>"+id[1],
    			url:web_base+"/user_socials/article_comment/"+id[1],
	    		dataType:"json",
	    		type:"GET",
	    		data: { 'parent_id': id[1]},
    ...
    $(".WB_detail .WB_info span").click(function(){
	    ...
	    if(js_login_user_data==null){
        	...
        	if (confirm(confirm_delete)){
    	    	...
    	    	window.location.href=web_base+"/user_socials/del_comment/"+id;
        	}
	    }
    });	

	    ...
	    <div class="am-tab-panel <?php if((isset($fanslist_type)&&$fanslist_type==2)||!isset($fanslist_type)){echo ' am-active';} ?>">
	      ...
	      <ul class="am-comments-list am-comments-list-flip">
	      	<?php foreach($fans_list as $k=>$v){?>
	      		<li class="am-comment">
					<a href="<?php echo $html->url('/user_socials/index/'.$v['UserFans']['fan_id']); ?>"><?php echo $html->image( isset($v['User']['img01'])&&$v['User']['img01']!=""?$v['User']['img01']:('/theme/default/img/no_head.png'),array('class'=>'am-comment-avatar','style'=>'width:48px;height:48px;','alt'=>$v['User']['name']));  ?></a>
	    ...
	    <div class="am-tab-panel <?php if(isset($fanslist_type)&&$fanslist_type==1){echo ' am-active';} ?>">
	    	<?php if(isset($focus_list)&&!empty($focus_list)){ ?>
	    		...
	    		<?php foreach($focus_list as $k=>$v){?>
	    			<li class="am-comment">
					<a href="<?php echo $html->url('/user_socials/index/'.$v['UserFans']['user_id']); ?>"><?php echo $html->image( isset($v['User']['img01'])&&$v['User']['img01']!=""?$v['User']['img01']:('/theme/default/img/no_head.png'),array('class'=>'am-comment-avatar','style'=>'width:48px;height:48px;','alt'=>$v['User']['name']));  ?></a>
					<div class="am-comment-main">
						<header class="am-comment-hd">
		      				<div class="am-comment-meta">
...
function mutual_concern(obj,id,type){
	...
	if(is_me==0){
		...
		$.ajax({ url: web_base+"/user_socials/fans_add/",
			dataType:"json",
			type:"POST",
			data: {'fans_id':id },
			success: function(data){
				// var url="<?php echo $html->url('/user_socials/fanslist/'.$id); ?>/"+type;
				var url=web_base+"/user_socials/fanslist/<?php echo $id;?>"+type;

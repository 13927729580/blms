	  ...
	  <div class="am-tabs-bd">
	    <div class="am-tab-panel am-active">
	      <?php if(isset($chat_list)&&sizeof($chat_list)>0){ ?>
			...
			<?php foreach($chat_list as $k=>$v){ //pr($v);?>

				<li class="am-comment">
					<a href="<?php echo $html->url('/user_socials/index/'.(isset($v['User']['id'])?$v['User']['id']:'')); ?>" title="<?php echo $v['User']['name'] ?>" target="_blank"><?php echo $html->image( isset($v['User']['img01'])&&$v['User']['img01']!=""?$v['User']['img01']:('/theme/default/img/no_head.png'),array('class'=>'am-comment-avatar','style'=>'width:48px;height:48px;','alt'=>$v['User']['name']));  ?></a>
							...
							<div id="userchat_list_<?php echo $v['UserChat']['id'];?>" style="display:none;">
								...
								<div class="chat_list_<?php echo $v['UserChat']['id'];?>">
									...
									<?php foreach($v['message_list'] as $vv){ ?>
										<li class="am-comment">
											<a href="<?php echo $html->url('/user_socials/index/'.(isset($vv['User']['id'])?$vv['User']['id']:'')); ?>" title="<?php echo $vv['User']['name'] ?>" target="_blank"><?php echo $html->image( isset($vv['User']['img01'])&&$vv['User']['img01']!=""?$vv['User']['img01']:('/theme/default/img/no_head.png'),array('class'=>'am-comment-avatar','style'=>'width:48px;height:48px;','alt'=>$vv['User']['name']));  ?></a>
											<div class="am-comment-main">
												<header class="am-comment-hd">
								      				<div class="am-comment-meta">
	    ...
	    <div class="am-tab-panel">
	      <?php if(isset($message)&&sizeof($message)>0){ ?>
			...
			<?php foreach($message as $k=>$v){ ?>
				<li class="am-comment">
					<a href="javascript:void(0);"><?php echo $html->image('/theme/default/img/no_head.png',array('class'=>'am-comment-avatar','style'=>'width:48px;height:48px;'));  ?></a>
					<div class="am-comment-main">
						<header class="am-comment-hd">
		      				<div class="am-comment-meta">
...
$(function(){
	...
	$(".comments_button button").click(function(){
		...
		$.ajax({ url: web_base+"/user_socials/private_comment/",
	    		dataType:"json",
	    		type:"POST",
	    		...
	    		success: function(data){
	    			if(data=="1"){
						...
						var show_html="<li class='am-comment'><a target='_blank' title='<?php echo $_SESSION['User']['User']['name'] ?>' href='<?php echo $html->url('/user_socials/index/'.($_SESSION['User']['User']['id'])); ?>'><img width='48' height='48' src='"+web_base+"<?php  echo $_SESSION['User']['User']['img01']!=""?$_SESSION['User']['User']['img01']:'/theme/default/img/no_head.png'; ?>' alt='<?php echo $_SESSION['User']['User']['name'] ?>' class='am-comment-avatar'></a><div class='am-comment-main'><header class='am-comment-hd'><div class='am-comment-meta'><a class='am-comment-author' href='<?php echo $html->url('/user_socials/index/'.($_SESSION['User']['User']['id'])); ?>'><?php echo $_SESSION['User']['User']['name'] ?></a>&nbsp;&nbsp;<time>"+time_str+"</time></div></header><div class='am-comment-bd'>"+content+"</div></div></li>";
	...
	$(".chat_history").click(function(){
		...
		if(id!=""){
			window.open(web_base+"/user_socials/chatroom/"+id,'_blank');
		}
	});
})
</script>
<div class="am-u-lg-3 am-u-md-4 am-u-sm-12">
	...
	<?php echo $html->image( isset($user['User']['img01'])&&$user['User']['img01']!=""?$user['User']['img01']:("/theme/default/img/no_head.png"),array('title'=>$user['User']['name'],'class'=>'am-u-sm-centered'));  ?>
		    <!-- 头像编辑链接浮动窗口 -->
...
$(document).ready(function(){
	...
	<?php if(isset($_SESSION['User']['User']['id']) && ($_SESSION['User']['User']['id']==$id)){?>
	$.ajax({ url: web_base+"/user_socials/get_message_count/",
		dataType:"json",
		type:"POST",
	...
	$(".del_my_focus").click(function(){
		...
		$.ajax({ url: web_base+"/user_socials/unfo/",
			dataType:"json",
			type:"POST",
			data: {'fans_id':unfans },
			success: function(data){

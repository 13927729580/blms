<style type="text/css">
.am-nav-tabs.am-nav-justify>li>a, .am-nav-tabs>li>a, .am-nav>li>a, .am-pagination .am-pagination-next a, .am-pagination .am-pagination-prev a{padding-left:10px}
</style>
...
<?php if($this->params['controller']=="categories" && $this->params['action']="view"){
  ...
  <?php if($code_infos[$sk]['type']=="module_category_list"){?>
    ...
    <div data-am-widget="titlebar" class="am-titlebar am-titlebar-default">
      <h2 class="am-titlebar-title am-color">
		<a href="<?php echo $svshow->seo_link_url(array('type'=>'PC','name'=>$v['CategoryProductI18n']['name'],'id'=>$v['CategoryProduct']['id']));?>"  style="font-weight:normal;font-size:15px;"><?php echo $v['CategoryProductI18n']['name']?></a>
      </h2>
	</div>
	<ul class="am-nav">
	  <!-- 2级分类 -->
	  <?php if(!empty($v['SubCategory'])){?>
		...
		<li style="margin:0 0 0 25px;" class="am-color ">
		  <a class="titles "  href="<?php echo $svshow->seo_link_url(array('type'=>'PC','name'=>$vvv['CategoryProductI18n']['name'],'id'=>$vvv['CategoryProduct']['id']));?>" >

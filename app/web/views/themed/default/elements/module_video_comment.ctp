		...
		<?php $i=0;if(!empty($sm['comment'])&&$sm['comment']!=1){foreach($sm['comment'] as $k=>$v){$i++; ?>
		  <li class="am-comment">
			<a target="_blank" title="<?php echo $v['User']['name'];?>" href="<?php echo $html->url('/user_socials/index/'.$v['Comment']['user_id']); ?>">
    		  <?php echo $html->image( isset($v['User']['img01'])&&v['User']['img01']!=""?$v['User']['img01']:('/theme/default/img/no_head.png'),array('class'=>'am-comment-avatar','style'=>'width:48px;height:48px;','alt'=>$v['User']['name']));  ?>
  			</a>
		      ...
		      <div class="am-comment-bd">
			    ...
			    <?php if(isset($v['Comment']['img']) && !empty($v['Comment']['img']) && file_exists(WWW_ROOT.$v['Comment']['img'])){?>
			  	<figure data-am-widget="figure" class="am am-figure am-figure-default am-u-lg-4 " data-am-figure="{pureview: 'auto'}">
			  		<?php echo $html->image($v['Comment']['img'],array('data-rel'=>$v['Comment']['img'],'style'=>'width:48px;height:48px;','alt'=>$vv['User']['name']));  ?>
					...
					<ul class="am-comments-list am-comments-list-flip">
					  ...
					  <li class="am-comment">
					    <a target="_blank" title="<?php echo $vv['User']['name'];?>" href="<?php echo $html->url('/user_socials/index/'.$vv['Comment']['user_id']); ?>">
						  <?php echo $html->image( isset($vv['User']['img01'])&&$vv['User']['img01']!=""?$vv['User']['img01']:('/theme/default/img/no_head.png'),array('class'=>'am-comment-avatar','style'=>'width:48px;height:48px;','alt'=>$vv['User']['name']));  ?>
					    </a>
						<div class="am-comment-main">
					      <header class="am-comment-hd">
					        <div class="am-comment-meta">
...
var Url=web_base+"/theme/default/img/gif/";//表情图片路径
//表情数组
...
function add_comment(){
	...
	$(".reply_to_user").click(function(){	
		...
		if(text==""){
			...
			$.ajax({ url: web_base+"/articles/reply_comment/",
	    		dataType:"json",
	    		type:"POST",

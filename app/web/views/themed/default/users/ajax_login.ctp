<?php echo $form->create('/users',array('action'=>'login','id'=>'ajax_login_form','name'=>'user_login', 'type'=>'POST','class'=>'am-form','onsubmit'=>'return(md5Login(this));return(false);'));?>
	...
	<div class="am-other-login" style="margin-bottom:15px;">
  	...
  	<div class="am-form-group">
		...
		<input name="user_name" id="user_names" class="login_user_names" type="email" chkRules="nnull:<?php echo $ld['login_name'].$ld['can_not_empty']; ?> ?>" placeholder="请输入邮箱/用户名/手机">
		<input type="hidden" class="login_user_type" name="login_type" value="" />
  	</div>
	<div class="am-form-group">
		<!-- <label><?php echo $ld['password'] ?></label> -->
		<input class="input1" type="password" placeholder="请输入密码" name="password" id="login_pwd" chkRules="nnull:<?php echo $ld['login_password_empty']?>;min6:<?php echo $ld['confirm_password_can_not_be_less_than_6_digits']?>;max16:<?php echo $ld['password_up_to_16']?>" onkeydown="if (event.keyCode==13){ javascript:login();}" />
	</div>
	<?php if(isset($configs['use_captcha'])&&$configs['use_captcha']=='1'){ ?>
	 <div class="am-form-group">
		<!-- <label><?php echo $ld['verify_code'] ?></label> -->
		<div style="height: 45px;" class="am-cf">
			<div class='am-form-icon am-form-feedback' style="float:left;width:58%;">
				<input type="hidden" id="ck_authnum" value="" />
				<input type="text" style=""  placeholder="请输入验证码" class="keywordauthen am-form-field" name="data[Users][authnum]" chkRules="authnum:验证码错误" id="ajax_login_authnums" /><span></span>
			</div>
			<div class="authentication" style="float:left;width:110px;line-height:37px;text-align:right;">
				<img id='authnum_ajax_login' align='absmiddle' src="<?php echo $webroot;?>securimages/index/?1234" alt="<?php echo $ld['not_clear']?>" onclick="change_captcha('authnum_ajax_login');" style="margin-left:0.8rem;margin-bottom:0.2rem;" />
				<a style="margin:0 0 0 5px;color:#65c5b3;" href="javascript:change_captcha('authnum_ajax_login');"><span class="am-icon-refresh"></span></a>
			</div>
		</div>
	</div>  
	<?php } ?>
  	<div class="am-form-group am-text-center" style="clear:both;margin-top:10px;">
    		<a href="javascript:void(0);" class="am-btn am-btn-primary am-btn-block reg_email" onclick="ajax_login()" style="width:100%;"><span><?php echo $ld['login'] ?></span></a>
  	</div>
	<div class="am-form-group">
		<div class="am-fl">
			<input type="checkbox" name="status" value="1" style="position:relative;top:2px;" class="checkl"/><span class="check"><?php echo $ld['auto_login'] ?></span>
		</div>
		<div class="am-fr">
			...
			$.ajax({
				 ...
				 success: function(data){
					...
					if(data.error_no==1){
						...
						$("#ajax_login_form .errors").css("height","50px");
						<?php if(isset($configs['use_captcha'])&&$configs['use_captcha']=='1'){ ?>
							change_captcha('authnum_ajax_login');

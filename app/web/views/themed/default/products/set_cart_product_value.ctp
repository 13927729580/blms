...
.customize_attrInfo .am-table > tbody > tr > td{border:none;}
</style>
...
<div class="customize_attrInfo" id="customize_attrInfo<?php echo $cart_pro_code; ?>">
  <table class="am-table am-form">
	<tr>
	  <td width="30%" style="white-space:nowrap;border-bottom:1px solid #ddd;border-top:1px solid #ddd">定制属性</td>
<!-- 	  <td width="20%" style="white-space:nowrap">默认值</td> -->
	  <td width="40%" style="white-space:nowrap;text-align:center;padding-left:10px;padding-right:10px;border-bottom:1px solid #ddd;border-top:1px solid #ddd">修改选项</td>
	</tr>
	...
	<tr>
	  ...
	  <?php if(sizeof($v['select_img'])>1){ ?>
		...
		<input type="hidden" name="CartProductValue[<?php echo $cart_pro_code; ?>][<?php echo $v['attr_id'] ?>]" value="<?php echo $v['default_value'] ?>" class="explain_code explain_<?php echo $v['attr_id'] ?>" />
...
<!-- 	  <td><?php echo $v['default_value'] ?></td> -->
	  <td style="padding-left:10px;padding-right:10px;">
	  	...
	  	<?php if(isset($v['select_value'])&&!empty($v['select_value'])&&$v['is_customize']==1){$customize_attr_input=true;
		...
		?>
		  <option value="<?php echo trim($slv); ?>" <?php echo $sel_txt; ?>><?php echo $attr_type_txt; ?>
		  </option>
		<?php } ?>
	    </select>
		...
		<?php foreach($v['select_value'] as $slk=>$slv){if(trim($slv)=="")continue;
		  $sel_txt="";
		  if($slv==$v['default_value']){$sel_txt=" selected='selected'";$select_value=$slv;}
		?>
		  <option value="<?php echo $v['select_price'][$slk] ?>" <?php echo $sel_txt; ?> ><?php echo $slk; ?></option>
		<?php } ?>
	    </select>
	  <?php }} ?>
	  	<?php if($customize_attr_input){ ?>
		<input type="hidden" name="CartProductValue[<?php echo $cart_pro_code; ?>][<?php echo $v['attr_id'] ?>]" class="explain_code explain_<?php echo $v['attr_id'] ?>" value="<?php echo $v['default_value'] ?>" />
		 <input type="hidden" name="AccessoryPrice[<?php echo $cart_pro_code; ?>][<?php echo $v['attr_id'] ?>]" value="<?php echo $v['select_price'][$select_value] ?>" />
		<?php }else{ ?>
		<input type="text" name="CartProductValue[<?php echo $cart_pro_code; ?>][<?php echo $v['attr_id'] ?>]" value="<?php echo $v['default_value'] ?>"  class="explain_code explain_<?php echo $v['attr_id'] ?>" />
		<?php } ?>
	...
	<tr>
	  ...
	  <td style="padding:6px 10px;"><textarea name="CartProductNote[<?php echo $cart_pro_code; ?>]" style="width:100%;resize:none;" rows="3" class="explain_textarea"></textarea></td>
	</tr>
  </table>
</div>
...
$(".attr_value_sel").change(function(){
	...
	var change_html = $(this).find("option:selected").html();
	var TR=$(this).parent().parent();
	console.log(change_value);
	console.log(change_html);
	TR.find("input[type=hidden][name*='CartProductValue']").val(change_html);
	TR.find("input[type=hidden][name*='AccessoryPrice']").val(change_value);
});
//图片属性修改

     ...
     <table cellpadding="0" cellspacing="0" class="am-table am-table-striped am-table-hover table-main user_style_list">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td width="8%"><a href="<?php echo $html->url('/products/'.$order_pro_data[$vvv['UserStyle']['id']]['Product']['id']); ?>"><?php echo $html->image($order_pro_data[$vvv['UserStyle']['id']]['Product']['img_thumb'],array('class'=>'pro_img'));  ?></a></td>
...
function set_style_default(obj,Id,attribute_code){
    ...
    $.ajax({ 
            // url:"<?php echo $html->url('/user_styles/set_style_default'); ?>",
            url:web_base+"/user_styles/set_style_default",
        	type:"POST",
        	dataType:"json",
...
function save_remark(){
    ...
    if(remark.length>0){
        $.ajax({ 
            // url:"<?php echo $html->url('/user_styles/update_remark'); ?>",
                    url:web_base+"/user_styles/update_remark",
                	type:"POST",
                	dataType:"json",

...
class RespondsController extends AppController
{
    ...
    public $uses = array('Product','Flash','Payment','ProductAlsobought','UserBalanceLog','UserPointLog','PaymentApiLog','User','UserAccount','MailTemplate','MailSendQueue','AccountInformation');
    ...
    public function return_code($code,$is_notify=0,$equipment_type="pc")
    {
        ...
        $alipay_amount=0;
        $trade_status = 0;//支付状态验证
        $verify_result=false;//请求来源验证
        $payment_api_log_id=0;//支付日志编号
        ...
        }else if($payment['Payment']['code']=='alipay'){
		$alipay_class_file=ROOT."/vendors/payments/alipay/alipay_notify.class.php";
		...
		$alipay_config['input_charset']= strtolower('utf-8');
		$alipay_config['cacert']    = ROOT.'/vendors/payments/alipay/cacert.pem';
		$alipay_config['transport']    = 'http';
		...
		$payment_api_log_id=$payment_api_log_id_arr[0];//支付记录Id
	        ...
	        if($is_notify==1){
	        	$verify_result = $response_payment->notify_verify();
	        }else{
        ...
        if ($payment_api_log['PaymentApiLog']['type'] == '2') {
            ...
            if (constant('Product') == 'AllInOne') {
                ...
                if (!empty($foo)) {
                		if($foo['Order']['service_type']=='virtual'){
                			$back_url = '/virtual_purchases/api_pay_callback/'.$payment_api_log['PaymentApiLog']['type_id'];
                		}else{
        ...
        if ($response['code'] == 0) {
            if ($payment_api_log['PaymentApiLog']['type'] == '2') {
                ...
                if (!empty($user_Info)) {
                    ...
                    $this->notify_balance($user_id,$add_money);
                }
                $msg = '支付成功';
            } else {
                //订单支付
                if (constant('Product') == 'AllInOne') {
                    $this->loadModel('Order');
                    $this->loadModel('OrderProduct');
                    $this->update_order($payment_api_log, $foo);
            ...
            if (constant('Product') == 'AllInOne'&&$payment_api_log['PaymentApiLog']['type']=='1') {
                $this->loadModel('Order');
                $this->loadModel('OrderProduct');
                $this->ex_pay_to($foo, $orderp_info['OrderProduct']['product_name']);
                $msg = $this->ld['your_order'].':'.$foo['Order']['order_code'].'&nbsp;'.$this->ld['order_total'].':'.sprintf($price_format, $payment_api_log['PaymentApiLog']['amount']).$this->ld['successful_to_pay'];
            }else if($payment_api_log['PaymentApiLog']['type']=='2'){
			$msg = '支付成功';
		}
        } else {
            	$msg = $this->ld['failure_to_pay'];
    ...
    */
	...
	public function weixin_notify(){
		...
		$out_trade_no_result=explode('_',$out_trade_no);
		$api_payment_id=isset($out_trade_no_result[0])?$out_trade_no_result[0]:0;
		$order_code=isset($out_trade_no_result[1])?$out_trade_no_result[1]:'';
		$orfo = $this->Order->find('first', array('conditions' => array('Order.order_code' => $order_code)));
		$payment_api_log = $this->PaymentApiLog->find('first', array('conditions' => array('PaymentApiLog.id' => $api_payment_id)));
	    	if (!empty($orfo)&&!empty($payment_api_log)) {
			$this->update_order($payment_api_log,$orfo);
	    	}
	    	die();
	}
	...
	$this->loadModel('OrderProduct');
	$this->loadModel('OrderAction');
	$need_pay = number_format($order_total['Order']['total'] - $order_total['Order']['point_fee'] - $order_total['Order']['discount'] - $order_total['Order']['coupon_fee'] - $order_total['Order']['user_balance']-$order_total['Order']['money_paid'], 2, '.', '') + 0;
	if($order_total['Order']['payment_status'] == '2'&&floatval($need_pay)==0){
		return;
	}
	$this->Order->order_notify('order_submission',$order_total['Order']['id'],$this);
        /* 改变订单状态 */
        ...
        if($order_total['Order']['service_type']=='virtual'){
			$order_total['Order']['shipping_status']='1';
			$order_total['Order']['shipping_time']=date('Y-m-d H:i:s');
			$order_total['Order']['shipping_id']=0;
			$order_total['Order']['shipping_name']='无需物流';
        }
        $this->Order->save($order_total);
            //已付款 未发货的商品冻结库存处理
            $this->frozen_quantity($order_total['Order']['id']);
            $this->OrderAction->saveAll(array('OrderAction' => array(
		            'order_id' => $order_total['Order']['id'],
		            'from_operator_id' => 0,
		            'user_id' => $order_total['Order']['user_id'],
		            'order_status' => 1,
		            'payment_status' => 2,
		            'shipping_status' => $order_total['Order']['shipping_status'],
		            'action_note' => $this->ld['successful_to_pay'],
            )));
            $user_info=$this->User->findbyid($order_total['Order']['user_id']);
		$payment_ids=array();
		if(intval($order_total['Order']['payment_id'])>0){
			$payment_ids[]=intval($order_total['Order']['payment_id']);
		}
		if(intval($order_total['Order']['sub_pay'])>0){
			$payment_ids[]=intval($order_total['Order']['sub_pay']);
		}
		...
		$receipt_account=isset($config_value['account'])?$config_value['account']:(isset($config_value['MCHID'])?$config_value['MCHID']:'');
		$account_info=array(
			'id'=>0,
			'account_type'=>0,
			'payer'=>isset($user_info['User']['first_name'])&&trim($user_info['User']['first_name'])!=''?$user_info['User']['first_name']:(isset($user_info['User']['name'])?$user_info['User']['name']:''),
			'payee'=>$payee,
			'receipt_account'=>$receipt_account,
			'payment_id'=>$payment_info['Payment']['id'],
			'transaction'=>$order_total['Order']['order_code'],
			'payment_amount'=>$payment_log['PaymentApiLog']['amount'],
			...
			'note'=>$this->ld['order'].$order_total['Order']['order_code']
             ...
             if($order_total['Order']['service_type']=='virtual'){
             	$order_item_detail=$this->OrderProduct->find('first',array('conditions'=>array('OrderProduct.order_id'=>$order_total['Order']['id']),'order'=>'OrderProduct.id', 'recursive' => '-1'));
             	if(!empty($order_item_detail)){
             		if($order_item_detail['OrderProduct']['item_type']=='activity'){
             			$this->loadModel('Activity');
             			$this->loadModel('ActivityUser');
             			$activity_data=$this->Activity->find('first',array('conditions'=>array('id'=>$order_item_detail['OrderProduct']['product_id'],'status'=>'1')));
             			if(!empty($activity_data)&&!empty($user_info)){
             				$activity_user_id=isset($user_info['User']['id'])?$user_info['User']['id']:0;
             				$activity_user_data=$this->ActivityUser->find('first',array('conditions'=>array('activity_id'=>$order_item_detail['OrderProduct']['product_id'],'user_id'=>$activity_user_id,'status'=>'1','payment_status'=>0)));
             				if(!empty($activity_user_data)){
             					$this->ActivityUser->save(array('id'=>$activity_user_data['ActivityUser']['id'],'payment_status'=>'1'));
             				}
             			}
             		}else if($order_item_detail['OrderProduct']['item_type']=='organization'){
             			$this->loadModel('Organization');
             			$OrganizationData=$this->Organization->find('first',array('conditions'=>array('id'=>$order_item_detail['OrderProduct']['product_id'],'status'=>'1')));
             			if(!empty($OrganizationData['Organization'])){
             				$this->Organization->save(array('id'=>$OrganizationData['Organization']['id'],'authentication_status'=>'1'));
             			}
             		}
             	}
             }
            if(isset($this->configs['use_point'])&&$this->configs['use_point']=='1'){
                	...
                	//下单积分赠送
                	if(isset($order_total['Order']['lease_type'])&&$order_total['Order']['lease_type']=='L'){//租赁订单
					...
					$this->UserPointLog->point_notify($point_log);
                		...
                		if(in_array($points_awarded_occasion,array('0','2'))&&$config_order_smallest <= $order_total['Order']['subtotal']&&$this->configs['out_order_points'] > 0){
					$user_info = $this->User->findbyid($order_total['Order']['user_id']);
					$old_point=$user_info['User']['point'];
        ...
        if(isset($this->configs['order_point_give_time'])&&$this->configs['order_point_give_time']=='0'){
        	//订单商品、订单支付金额赠送积分
        	$this->Order->give_order_point($order_total['Order']['id'],$this);
        }

...
class PagesController extends AppController
{
    ...
    public $uses = array('Flash','FlashImage','Article','Product','CategoryProduct','ProductsCategory','UserApp','Page','PageI18n','TopicProduct','Comment');
    public $components = array('RequestHandler','Cookie','Session','Captcha','Pagination');
    /**
     *主页.
     */
...
//		$user_log_data = $this->User->find("all",array("conditions"=>$condition,"limit"=>"5"));
    ...
    /*	$this->set("new_o",$this->Order->find("all",array('cache'=>$this->short,"limit"=>"5",'fields'=>array('Order.city','Order.shipping_name'),'order'=>array('Order.id desc'))));
        ...
        // 同步处理
        $syns = $this->UserApp->find('list', array('conditions' => array('UserApp.status' => 1,'UserApp.location'=>array(0,2)), 'fields' => array('UserApp.type')));
        $this->set('syns', $syns);
        //首页推荐，促销，最新商品
        $sale = false;
        $price = false;
...
//		$this->set('all_volumes',$all_volumes);
    ...
    function page_comment($page_id=0,$page=1){
    		Configure::write('debug', 1);
        	$this->layout = 'ajax';
        	
        	if(!(isset($this->configs['enable_page_comment'])&&$this->configs['enable_page_comment']=='1')){
        		die();
        	}
        	...
        	$conditions['Comment.type'] = 'SP';
        	$conditions['Comment.type_id'] = $page_id;
		...
		$parameters['route'] = array('controller' => 'pages','action' => 'page_comment/'.$page_id,'page' => $page,'limit' => $limit);
		//分页参数
    ...
    function ajax_page_comment_reply($comment_id=0){
    		Configure::write('debug', 1);
        	$this->layout = 'ajax';
        	if(!(isset($this->configs['enable_page_comment'])&&$this->configs['enable_page_comment']=='1')){
        		die();
    ...
    function ajax_add_page_comment(){
    		Configure::write('debug', 1);
        	$this->layout = 'ajax';
    ...
    public function ajax_header($page_id=0){
    ...
    public function closed(){
		...
		if(isset($this->configs['closed_reason']))$this->set('closed_reason', $this->configs['closed_reason']);
    }

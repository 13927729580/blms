...
class UserSocialsController extends AppController
{
    ...
    public $uses = array('User','UserFans','UserVisitors','UserAction','Blog','UserLike','Comment','UserChat','UserMessage','UserConfig','SynchroUser','OauthLog','BlockWord','Product','ProductI18n','Topic','TopicI18n','UserApp','SynchroUser','LanguageDictionary','UserFavorite');
    ...
    public function index($id = 0, $pagetype = 'all', $page = 1, $limit = 9)
    {
        ...
        if (!empty($user)) {
            ...
            $this->ur_heres[] = array('name' => ('用户中心'), 'url' => '/users/');
            $this->ur_heres[] = array('name' => ('日记'), 'url' => '');
        } else {
        ...
        if (isset($_SESSION['User']['User']['id'])) {
            ...
            foreach ($UserApp_list as $k => $v) {
            	  if(!isset($UserAppImg[$v['UserApp']['type']])){unset($UserApp_list[$k]);continue;}
        ...
        if (empty($user_privacyset)) {
            ...
            $user_privacyset[0]['UserConfig']['value'] = 0;
            $user_privacyset[1]['UserConfig']['code'] = 'mycomment';
            $user_privacyset[1]['UserConfig']['value'] = 0;
            $user_privacyset[2]['UserConfig']['code'] = 'mylog';
            $user_privacyset[2]['UserConfig']['value'] = 0;
        }
        ...
        $fan_like = $this->UserFans->find('count', array('conditions' => array('UserFans.user_id' => $id, 'UserFans.fan_id' => $user_id)));
        if ($user_id == $id) {
            $fan_like = 1;
            $log_set = 1;
            $comment_set = 1;
...
//    		}
    ...
    public function message_index($page = 1, $limit = 0)
    {
        ...
        $this->ur_heres[] = array('name' => '我的消息', 'url' => '');
...
/*		$parameters['get'] = array();
    ...
    public function ajax_like()
    {
        ...
        if (isset($_SESSION['User'])) {
            ...
            if (empty($like)) {
                $this->UserLike->deleteAll(array('UserLike.type'=>'P','UserLike.type_id'=>$type_id,'UserLike.action'=>'unlike'));
                ...
                if (isset($pro_info['Product'])) {
                    ...
                    $action_data['UserAction']['content'] = $_SESSION['User']['User']['name'].' '.$this->ld['like_the'].' '.$this->ld['product']."<a href='".$this->base."/products/".$pro_info['Product']['id']."'>".$pro_info['ProductI18n']['name'].'</a>';
    ...
    public function ajax_dislike()
    {
        ...
        if ($this->RequestHandler->isPost() && !empty($like)) {
            ...
            if (isset($pro_info['Product'])) {
                ...
                $action_data['UserAction']['content'] = $_SESSION['User']['User']['name'].' '.$this->ld['canceled'].' '.$this->ld['like_the'].' '.$this->ld['product']."<a href='".$this->base."/products/".$pro_info['Product']['id']."'>".$pro_info['ProductI18n']['name'].'</a>';
                $action_data['UserAction']['created'] = date('Y-m-d H:i:s');//用户创建时间
                $action_data['UserAction']['modified'] = date('Y-m-d H:i:s');//用户修改时间
                $this->UserAction->save($action_data['UserAction']);
            }
        ...
        if (isset($this->configs['show_product_like']) && $this->configs['show_product_like'] == '1') {
            ...
            $cond_pro['limit'] = $limit;
            $cond_pro['page'] = $page;
            $cond_pro['fields'] = array('UserLike.*','Product.id','Product.brand_id','Product.shop_price','Product.img_original','Product.img_detail');
            $like_list = $this->UserLike->find('all', $cond_pro);
		...
		foreach($brand_info as $v){
            ...
            $UserLike_data=$this->UserLike->find('list',array('fields'=>'type_id,id','conditions'=>array('UserLike.user_id'=>$userid,'UserLike.action'=>'like','UserLike.type'=>'P','UserLike.type_id'=>$product_ids)));
					$UserFavorite_data=$this->UserFavorite->find('list',array('fields'=>'type_id,id','conditions'=>array('UserFavorite.user_id'=>$userid,'UserFavorite.status'=>'1','UserFavorite.type'=>'P','UserFavorite.type_id'=>$product_ids)));
            $comment_num = $this->Comment->find('all', array('conditions' => array('Comment.type_id' => $product_ids, 'Comment.type' => 'P'), 'fields' => array('Comment.type_id', 'count(Comment.type_id) as Commentnum'), 'group' => 'Comment.type_id'));
            $like_num = $this->UserLike->find('all', array('conditions' => array('UserLike.type' => 'P', 'UserLike.type_id' => $product_ids), 'fields' => array('UserLike.type_id', 'count(UserLike.type_id) as num'), 'group' => 'UserLike.type_id'));
            foreach ($like_list as $k => $v) {
			$like_list[$k]['ProductI18n']['name'] = isset($product_I18N_list[$v['Product']['id']]) ? $product_I18N_list[$v['Product']['id']] : '';
			$like_list[$k]['UserLike']=isset($UserLike_data[$v['Product']['id']])?'1':'0';
			$like_list[$k]['UserFavorite']=isset($UserFavorite_data[$v['Product']['id']])?'1':'0';
			$like_list[$k]['Brand']=isset($brand_data[$v['Product']['brand_id']])?$brand_data[$v['Product']['brand_id']]:'';
                foreach ($like_num as $like_k => $like_v) {
                    if ($like_list[$k]['Product']['id'] == $like_v['UserLike']['type_id']) {
                        $like_list[$k]['Product']['like_num'] = $like_v[0]['num'];
                    }
    ...
    public function share_settings()
    {
        ...
        $this->set('user_list', $user_list);
        //同步授权分享
        $UserApp_list = $this->UserApp->find('all', array('fields' => array('UserApp.type'), 'conditions' => array('UserApp.status' => '1','UserApp.location'=>array(0,2))));
    ...
    public function get_unread_message(){
		...
		if(!empty($message_info)){
			$message_count = $this->UserMessage->find('all',array('conditions'=>array('UserMessage.to_id'=>$user_id,'UserMessage.is_read'=>0,'UserMessage.status'=>1)));
		}else{
			$message_count = 0;
		}
		die(json_encode($message_count));
    }
}

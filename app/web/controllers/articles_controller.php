...
class   ArticlesController extends AppController
{
    ...
    */
    ...
    public function view($id = '',$target_flag=0)
    {
	 ...
	 if (empty($article)) {
            ...
            echo '<meta http-equiv="Content-Type" content="text/html; charset=utf-8" /><script type="text/javascript">alert("'.$msg.'");	window.location.href="'.$this->webroot.'";</script>';
            die();
            $flag = 0;
            return;
        ...
        if($article['Article']['category_id']!=0){
        	$this->params['category_id'] = $article['Article']['category_id'];
        }
        $this->page_init($this->params);
        ...
        if (!empty($category_parent)){
            ...
            $i = 0;
            $cat_parent = $this->CategoryArticle->find('first', array('conditions' => array('CategoryArticle.id' => $catroot_category_id, 'CategoryArticleI18n.locale' => LOCALE), 'fields' => 'CategoryArticle.id,CategoryArticle.link,CategoryArticle.parent_id,CategoryArticle.article_template,CategoryArticleI18n.name'));
            $category_article_info=$cat_parent;
        ...
        if (trim($article['Article']['file_url'])!=""&&$target_flag==1) {
        		echo '<meta http-equiv="Content-Type" content="text/html; charset=utf-8" /><script type="text/javascript">window.location.href="'.$article['Article']['file_url'].'";</script>';
        		die();
        }
        ...
        /* 设置分类文章模板 */
        if (!empty($category_article_info['CategoryArticle']['article_template']) && $category_article_info['CategoryArticle']['article_template'] != '') {
            	$this->render['action']=$category_article_info['CategoryArticle']['article_template'];
        }
        /* 设置文章模板 */
    ...
    */
    ...
    public function category($id, $page = 1, $limit = 0, $order_field = 0, $order_type = 0, $showtype = 0, $type = 0, $keyword = '')
    {
        ...
        $this->params['page'] = $page;
        $this->params['locale'] = LOCALE;
        if(isset($_REQUEST['article_keywords'])&&trim($_REQUEST['article_keywords'])!=""){
        	$this->params['article_keywords'] = trim($_REQUEST['article_keywords']);
        	$keyword= trim($_REQUEST['article_keywords']);
        }
        ...
        if (isset($CategoryArticleInfo['CategoryArticle']['link']) && $CategoryArticleInfo['CategoryArticle']['link'] != '') {
            $this->redirect($CategoryArticleInfo['CategoryArticle']['link']);
        }
        $r_ur_heres=array();
        $tempurl = '/articles/category/'.$CategoryArticleInfo['CategoryArticle']['id'];
        $r_ur_heres[] = array('name' => $CategoryArticleInfo['CategoryArticleI18n']['name'], 'url' => $tempurl);
        //扩展分类
        //该分类上级分类
        $topcat = $this->CategoryArticle->find('first', array('conditions' => array('CategoryArticle.id' => $CategoryArticleInfo['CategoryArticle']['parent_id'])));
        if(!empty($topcat['CategoryArticle'])){
        	  $r_ur_heres[]=array('name' => $topcat['CategoryArticleI18n']['name'], 'url' =>'/articles/category/'.$topcat['CategoryArticle']['id']);
	        ...
	        if(!empty($topcat2)){
	        	$r_ur_heres[]=array('name' => $topcat2['CategoryArticleI18n']['name'], 'url' =>'/articles/category/'.$topcat2['CategoryArticle']['id']);
	        }
	        if ($topcat['CategoryArticle']['parent_id'] != ''&&isset($topcat2['CategoryArticle'])) {
	            ...
	            $cat_infos = $this->CategoryArticle->find('all', array('conditions' => array('CategoryArticle.parent_id' => $parent_id)));
	            if (!empty($cat_infos) && sizeof($cat_infos) > 0) {
	                $this->set('cat_infos', $cat_infos);
	                $this->set('cid', $id);
	            }
	        }
        }
        ...
        $sub_cat_infos=$this->CategoryArticle->find('all',array('conditions'=>array("CategoryArticle.parent_id"=>$id,"CategoryArticle.status"=>'1')));
        $this->set('sub_cat_infos', $sub_cat_infos);
        $category_ids=array();
        $category_ids[]=$id;
        foreach($sub_cat_infos as $v){
        	$category_ids[]=$v['CategoryArticle']['id'];
        }
        if(!empty($sub_cat_infos)){
        $sub_child_cat_ids=$this->CategoryArticle->find('list',array('conditions'=>array("CategoryArticle.parent_id"=>$category_ids,"CategoryArticle.status"=>'1')));
        	$category_ids=array_merge($category_ids,$sub_child_cat_ids);
        }
        $this->set('detail', $CategoryArticleInfo['CategoryArticleI18n']['detail']);
        $this->set('meta_description', $CategoryArticleInfo['CategoryArticleI18n']['meta_description']);
        $this->set('meta_keywords', $CategoryArticleInfo['CategoryArticleI18n']['meta_keywords']);
        if($page>1){
        	$this->pageTitle = $CategoryArticleInfo['CategoryArticleI18n']['name'].' - '.sprintf($this->ld['page'], $page).' - '.$this->configs['shop_title'];//
        }else{
        	$this->pageTitle = $CategoryArticleInfo['CategoryArticleI18n']['name'].' - '.$this->configs['shop_title'];//
        }
        if (empty($CategoryArticleInfo)) {
 ...
 //       $limit = isset($this->configs['products_category_page_size']) ? $this->configs['products_category_page_size'] : ((!empty($limit)) ? $limit : 20);
        ...
        //扩展分类
        $kuozhan = $this->ArticleCategory->find('list', array('conditions' => array('category_id' => $category_ids), 'fields' => 'article_id'));

        $conditions['OR']['AND']['Article.category_id'] = $category_ids;
        ...
        if ($keyword != '') {
            ...
            $conditions['OR']['ArticleI18n.subtitle like'] = "%$keyword%";
            $conditions['OR']['ArticleI18n.content like'] = "%$keyword%";
        }
        //pr($conditions);
        //分页start
        //get参数
        ...
        $pages = $this->Pagination->init($conditions, $parameters, $options); // Added
        //分页end
        $options = array();
        $options['conditions'] = $conditions;
        //pr($conditions);
        //$options['order'] = $order_field.' '.$order_type;
        $options['order'] = 'Article.recommand,orderby asc,Article.created desc';
        $options['limit'] = $limit;
        $options['page'] = $page;
        $this->set('id', $id);
        $articles_flash = $this->Flash->find("all",array("conditions"=>array("Flash.page"=>'AC', "Flash.page_id"=>$id)));
        ...
        if (!empty($articles)) {
            ...
            $article_like_info=$UserLike->find('all',array('conditions'=>array('UserLike.type'=>'A','UserLike.type_id'=>$article_ids,'UserLike.action'=>'like'),'fields'=>array("UserLike.type_id","count(UserLike.id) as like_num"),"group"=>'type_id'));
            	...
            	$this->set('article_like',$article_like_data);
            	if(isset($_SESSION['User']['User'])){
            		...
            		$this->set('article_user_like',$article_user_like);
            	}
            }
        }
	  ...
	  if (class_exists('Activity')) {
			...
			$conditions['Activity.type']='AC';
			$conditions['Activity.type_id']=$category_ids;
			$activity_list = $Activity->find('all', array('conditions' => $conditions,'order' => 'Activity.start_date desc'));
			$this->set('activity_list',$activity_list);
	  }
	
        /* 设置文章模板布局 */
        if (!empty($CategoryArticleInfo['CategoryArticle']['layout'])) {
            $this->layout = $CategoryArticleInfo['CategoryArticle']['layout'];
        } else {
            $this->layout = 'default';
        }
        /* 设置文章模板 */
        if (!empty($CategoryArticleInfo['CategoryArticle']['template']) && $CategoryArticleInfo['CategoryArticle']['template'] != 'default') {
            $this->render($CategoryArticleInfo['CategoryArticle']['template'], $this->layout);
        }
        $this->page_init($this->params);
    }
...
//		}
    ...
    function ajax_article_like(){
		...
		$article_like = $this->UserLike->find('first', array('conditions' => array('user_id' => $userid, 'type' => 'A', 'type_id' => $type_id, 'action' => 'like')));
		if (empty($article_like)) {
			$this->UserLike->deleteAll(array('UserLike.type'=>'A','UserLike.type_id'=>$type_id,'UserLike.action'=>'unlike'));
			$article_like['UserLike']['id'] = '0';//用户id
			$article_like['UserLike']['user_id'] = $userid;//用户id
			$article_like['UserLike']['type'] = 'A';
			$article_like['UserLike']['type_id'] = $type_id;
			$article_like['UserLike']['action'] = 'like';
			$this->UserLike->save(array('UserLike' => $article_like['UserLike']));
			$result['code'] = '1';
		}
		$like_num = $this->UserLike->find('count', array('conditions' => array('type' => 'A', 'type_id' => $type_id, 'action' => 'like')));
		$result['like_num'] = $like_num;
		die(json_encode($result));
    }
    
    function ajax_article_dislike(){
    		Configure::write('debug', 0);
		$this->layout = 'ajax';
		$result['code'] = '0';
		$this->loadModel('UserLike');
		...
		$userid = !empty($_POST['user_id']) ? $_POST['user_id'] : '';
        	...
        	$article_like = $this->UserLike->find('first', array('conditions' => array('user_id' => $userid, 'type' => 'A', 'type_id' => $type_id, 'action' => 'like')));
        	if(!empty($article_like)){
        		$article_like_data['id'] = $article_like['UserLike']['id'];
			$article_like_data['action'] = 'unlike';
			$this->UserLike->save(array('UserLike' => $article_like_data));
			$result['code'] = '1';
			$like_num = $this->UserLike->find('count', array('conditions' => array('type' => 'A', 'type_id' => $type_id, 'action' => 'like')));
			$result['like_num'] = $like_num;
        	}
    ...
    function ajax_article_favorite(){
    		//登录验证
		$this->checkSessionUser();
		Configure::write('debug', 0);
		$this->layout = 'ajax';
		...
		$userid = $_SESSION['User']['User']['id'];
		$type_id = isset($_POST['type_id']) ? $_POST['type_id'] : '0';
		$article_favorite = $this->UserFavorite->find('first', array('conditions' => array('user_id' => $userid, 'type' => 'a', 'type_id' => $type_id)));
		if (empty($article_like)||isset($article_favorite['UserFavorite'])&&$article_favorite['UserFavorite']['status']=='0') {
			$article_favorite['UserFavorite']['id'] = isset($article_favorite['UserFavorite'])?$article_favorite['UserFavorite']['id']:0;//用户id
			$article_favorite['UserFavorite']['user_id'] = $userid;//用户id
			$article_favorite['UserFavorite']['type'] = 'a';
			$article_favorite['UserFavorite']['type_id'] = $type_id;
			$article_favorite['UserFavorite']['status'] = '1';
			$this->UserFavorite->save(array('UserFavorite' => $article_favorite['UserFavorite']));
			$result['code'] = '1';
		}
		$favorite_num = $this->UserFavorite->find('count', array('conditions' => array('type' => 'a', 'type_id' => $type_id, 'status' => '1')));
		$result['favorite_num'] = $favorite_num;
		die(json_encode($result));
    }
    
    function ajax_article_disfavorite(){
    		Configure::write('debug', 0);
		$this->layout = 'ajax';
		$result['code'] = '0';
		$this->loadModel('UserFavorite');
		...
		$userid = !empty($_POST['user_id']) ? $_POST['user_id'] : '';
        	$type_id = !empty($_POST['type_id']) ? $_POST['type_id'] : '';
        	$article_favorite = $this->UserFavorite->find('first', array('conditions' => array('user_id' => $userid, 'type' => 'a', 'type_id' => $type_id, 'status' => '1')));
        	if(!empty($article_favorite)){
        		$article_favorite_data['id'] = $article_favorite['UserFavorite']['id'];
			$article_favorite_data['status'] = '0';
			$this->UserFavorite->save(array('UserFavorite' => $article_favorite_data));
			$result['code'] = '1';
			$favorite_num = $this->UserFavorite->find('count', array('conditions' => array('type' => 'a', 'type_id' => $type_id, 'status' => '1')));
			$result['favorite_num'] = $favorite_num;
        	}
    ...
    function article_comment($article_id=0,$page=1){
    		Configure::write('debug', 1);
        	$this->layout = 'ajax';
        	...
        	$conditions['Comment.type_id'] = $article_id;
		...
		$parameters['get'] = array();
		//地址路由参数（和control,action的参数对应）
		$parameters['route'] = array('controller' => 'articles','action' => 'article_comment/'.$article_id,'page' => $page,'limit' => $limit);
		//分页参数
    ...
    function ajax_article_comment_reply($comment_id=0){
    		Configure::write('debug', 1);
        	$this->layout = 'ajax';
    ...
    function ajax_add_article_comment(){
    		Configure::write('debug', 1);
        	$this->layout = 'ajax';

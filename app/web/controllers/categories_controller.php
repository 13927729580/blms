...
class CategoriesController extends AppController
{
     ...
     */
    public function view($id, $page = 1, $limit = 10, $order_field = 0, $order_type = 0, $showtype = 0, $type = 0, $brand_id = 0, $min_price = -1, $max_price = -1, $filters_attr_price = 0, $filters = '', $keyword = '')
    {
    	 if(isset($_REQUEST)){
        ...
        if ($min_price == -1) {
            $min_price = isset($_REQUEST['min_price']) ? $_REQUEST['min_price'] : -1;
        }
        if ($max_price == -1) {
            $max_price = isset($_REQUEST['max_price']) ? $_REQUEST['max_price'] : -1;
        }
	...
	$filters = $this->clean_xss($filters);
        ...
        $p_ids =$this->ProductsCategory->find('list', array('fields' => 'ProductsCategory.product_id', 'conditions' => array('ProductsCategory.category_id' => $id)));
        $page_conditions['and']['or']['Product.category_id'] = $id;
        $page_conditions['and']['or']['Product.id'] = $p_ids;
        $page_conditions['and']['Product.status'] = 1;
        $page_conditions['and']['Product.forsale'] = 1;
        $page_conditions['and']['Product.alone'] = 1;
...
$products_list = $this->Product->find('all', array('conditions' => array('Product.id' => $p_ids), 'page' => $page, 'limit' => $limit, 'fields' => 'Product.id,ProductI18n.name,Product.img_detail,Product.img_thumb,Product.img_detail,ProductI18n.description,ProductI18n.seller_note'));
        ...
        while (!empty($this->CategoryProduct->allinfo['P']['assoc'][$root_category_id]['CategoryProduct']['parent_id']) && $i++ < 10) {
            ...
            if(!isset($this->CategoryProduct->allinfo['P']['assoc'][$root_category_id])){break;}
...
//        	pr($filter_condition);die;
      ...
      //  pr($filter_condition);
        ...
        $category_count = isset($this->CategoryProduct->allinfo['P']['subids'][$id])?count($this->CategoryProduct->allinfo['P']['subids'][$id]):0;

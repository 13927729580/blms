...
*/
	...
	public function check_order(){
		...
		$result['code']='0';
		$result['data']='no';
		$order_code=isset($_REQUEST['order_id'])?$_REQUEST['order_id']:'';
		$order_info = $this->Order->find('first', array('conditions' => array('Order.order_code' => $order_code,'Order.payment_status' =>'2','Order.status' =>'1')));
		if (!empty($order_info)) {
			...
			if(floatval($need_pay)<=0){
				$order_items=array();
				$order_item_types=array();
				if(isset($order_info['OrderProduct'])){
					foreach($order_info['OrderProduct'] as $v){
						$order_items[]=$v['product_name'];
						$order_item_types[]=$v['item_type'];
					}
				}
				$result['code']='1';
				$result['data']=array(
					...
					'service_type'=>$order_info['Order']['service_type'],
					'item_type'=>implode('|',$order_item_types),
					'order_item_detail'=>implode(',',$order_items)
				);
			}
		}
    ...
    public function balance_deposit2()
    {
	    ...
	    try {
	        ...
	        if($payment['Payment']['code']=="weixinpay"){
				...
				$this->PaymentApiLog->deleteAll(array("PaymentApiLog.type_id"=>$orderid,"PaymentApiLog.is_paid"=>'0',"PaymentApiLog.created <="=>date("Y-m-d H:i:s",strtotime("-1 days"))));
				...
				$payment_api_id=$this->PaymentApiLog->id;
		                ...
		                if (isset($_SERVER['HTTP_USER_AGENT']) && strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') !== false) {
		                    ...
		                    if (empty($_GET['code'])){
		                        $request_url=$this->server_host.$_SERVER['REQUEST_URI'];
		                    ...
		                    if(!empty($openid)){
		                    	...
		                    	//自定义订单号，此处仅作举例
		                    	$timeStamp = time();
		                    	$out_trade_no = $payment_api_id.'_'.$order_code;
		                    	$unifiedOrder->setParameter("out_trade_no","$out_trade_no");//商户订单号 
		                    	$unifiedOrder->setParameter("total_fee",$amt);//总金额
		                    	$unifiedOrder->setParameter("notify_url",$this->server_host.$this->webroot.'responds/weixin_notify');//通知地址
					...
					$input->SetOut_trade_no($payment_api_id.'_'.$order_code);
					...
					$input->SetNotify_url($this->server_host.$this->webroot.'responds/weixin_notify');
				...
				$host = isset($_SERVER['HTTP_X_FORWARDED_HOST']) ? $_SERVER['HTTP_X_FORWARDED_HOST'] : (isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : '');
					...
					$alipay_parameter=array(
						...
						"payment_type"	=> '1',
						"notify_url"	=> $this->server_host.$this->webroot.'responds/return_code/'.$payment['Payment']['code'].'/1/wap',
						"return_url"	=>  $this->server_host.$this->webroot.'responds/return_code/'.$payment['Payment']['code'].'/0/wap',
						"out_trade_no"	=> $out_trade_no,
						"subject"	=> '['.$payment_api_log['order_id'].']'.(isset($orfo['OrderProduct'][0]['product_name'])?' - '.$orfo['OrderProduct'][0]['product_name']:''),
					...
					$html_text = $alipaySubmit->buildRequestForm($alipay_parameter,"get", "支付",$is_wechat);
					...
					$out_trade_no=$this->PaymentApiLog->id."_".$order_code;
						...
						"payment_type"	=> '1',//支付类型
						"notify_url"	=> $this->server_host.$this->webroot.'responds/return_code/'.$payment['Payment']['code'].'/1/pc',
						"return_url"	=> $this->server_host.$this->webroot.'responds/return_code/'.$payment['Payment']['code'].'/0/pc',
						"out_trade_no"	=> $out_trade_no,
						"subject"	=> '['.$payment_api_log['order_id'].']'.(isset($orfo['OrderProduct'][0]['product_name'])?' - '.$orfo['OrderProduct'][0]['product_name']:''),
						"total_fee"	=> $amount_money,
						"body"	=> '',
					...
					$html_text = $balance_payment->buildRequestForm($alipay_parameter,"get", "支付",$is_wechat);
					$pay_form_txt=$html_text;
	        		}else{
					...
					if ($payment['Payment']['is_online'] == 1) {   //在线支付增加api日志
						...
						$payment_config['cancel_return'] = $this->server_host.$this->webroot;
						$payment_config['return_url'] = $this->server_host.$this->webroot.'responds/return_code/'.$payment['Payment']['code'].'/0/pc';
						$payment_config['notify_url'] = $this->server_host.$this->webroot.'responds/return_code/'.$payment['Payment']['code'].'/1/pc';
						$payment_config['payerName'] = '['.$payment_api_log['order_id'].']'.(isset($orfo['OrderProduct'][0]['product_name'])?' - '.$orfo['OrderProduct'][0]['product_name']:'');
    ...
    public function balance_deposit()
    {
        ...
        try {
            ...
            $balance_payment = new $payment['Payment']['code']();
            if ($payment['Payment']['is_online'] == 1) {   //在线支付增加api日志
            	$this->PaymentApiLog->deleteAll(array("PaymentApiLog.type_id"=>$account_id,"PaymentApiLog.order_id"=>$order_id,"PaymentApiLog.is_paid"=>'0',"PaymentApiLog.created <="=>date("Y-m-d H:i:s",strtotime("-1 days"))));
                $payment_api_log = array(
                    'payment_code' => $payment['Payment']['code'],
                    'type' => 1,
                    'type_id' => $account_id,
...
//				$ids=$this->PaymentApiLog->find('first',array('conditions'=>array('PaymentApiLog.type_id'=>$orderid),'fields'=>array('PaymentApiLog.id')));
                ...
                $host = isset($_SERVER['HTTP_X_FORWARDED_HOST']) ? $_SERVER['HTTP_X_FORWARDED_HOST'] : (isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : '');
                $payment_config['cancel_return'] = $this->server_host.$this->webroot;
                $payment_config['return_url'] = $this->server_host.$this->webroot.'responds/return_code/'.$payment['Payment']['code'];

...
class AppController extends Controller
{
    ...
    public function beforeFilter()
    {
        ...
        //系统模块
	$this->System->modules();
	$this->system_modules=$this->System->modules;
	$this->SystemList=array_keys($this->system_modules);
	$this->set('system_modules', $this->system_modules);
	$this->set('SystemList', $this->SystemList);
...
//        $this->set('app_infos', $app_infos);
        ...
        */
        ...
        //高级系统资源库 内定
        $this->system_resources = $this->SystemResource->resource_formated(false, LOCALE);
    ...
    //  $conditions = array('Topic.start_time <='=>DateTime,'Topic.end_time >='=>DateTime);
        ...
        /*
            微信同步登录参数配置
        */
        $wechat_info = $this->UserApp->find('first', array('conditions' => array('UserApp.type' => 'Wechat', 'UserApp.status' => 1,'UserApp.location'=>array(0,2))));
        ...
        */
        ...
        //分享访问赠送积分
	if(isset($this->configs['user_share_points'])&&intval($this->configs['user_share_points'])>0){
		$this->loadModel('ShareAffiliateLog');
		$this->ShareAffiliateLog->share_affiliate_log($this);
	}
	
	if(isset($_REQUEST['affiliate_from'])&&$_REQUEST['affiliate_from']!=''){
		$this->loadModel('AffiliateAccessLog');
		$this->AffiliateAccessLog->access_log($this);
	}
    }
    ...
    public function checkSessionUser()
    {
	        if (!isset($_SESSION['User'])) {
				...
				$this->redirect('/users/login');
	        		...
	        		$user_total=$this->User->find('count',array('conditions'=>array('User.id'=>$user_id)));
	        		if(empty($user_total))$this->redirect('/users/logout');
	        }
    }
    ...
    public function set_locale()
    {
        ...
        if (!defined('LOCALE')) {
            ...
            //todo 跳转到默认语言首页
            echo '这个语言未开通 '.(defined('LOCALE')?LOCALE:'');
            die();
        }
        $this->locale = LOCALE;
        //$_SESSION['lo']=LOCALE;

...
class ProductsController extends AppController
{
    ...
    public function view($id = '', $page = 1)
    {
        ...
        //租赁
        if($product["Product"]["lease_status"]==1){
        	$pro_lease = $this->ProductLease->find('first', array('conditions' => array('ProductLease.product_code' => $product['Product']['code'],'ProductLease.status'=>1)));
			$this->set('pro_lease', $pro_lease);
        }
...
//        }
        ...
        if (isset($_SESSION['User'])) {
            ...
            $user_pro_like = $this->UserLike->find('first', array('conditions' => array('UserLike.type' => 'P', 'UserLike.type_id' => $id, 'UserLike.action' => 'like', 'UserLike.user_id' => $user_id)));
            if (empty($user_pro_like)) {
                $like_flag = true;
            } else {
                $like_flag = false;
            }
        ...
        $favorite_num=$this->UserFavorite->find('count',array('conditions' => array('type' => 'p', 'type_id' => $id)));
        $this->set('favorite_num', $favorite_num);
        //是否可以评论
        $can_comment = false;
        if (!empty($product)) {
            //记录商品浏览历史
            ...
            if (!in_array($product['Product']['id'], $pro_view_log)) {
                ...
                setcookie('pro_view_log', $pro_view_log_txt, time() + 60 * 60 * 24*30, '/');
            }
        ...
        //同步授权分享
        $UserApp_list = $this->UserApp->find('all', array('fields' => array('UserApp.type'), 'conditions' => array('UserApp.status' => '1', 'UserApp.type !=' => 'Wechat','UserApp.location'=>array(0,2))));
    ...
    //	if($product['Product']['product_type_id'] > 0){
        ...
        if(isset($product['Product']['brand_id'])&&!empty($product['Product']['brand_id'])){
        	$this->params['brand_id'] = $product['Product']['brand_id'];
        }
     ...
     *@param $showtype
     */
    public function advancedsearch($keyword = '', $page = 1, $limit = 12, $order_field = 0, $order_type = 0, $showtype = 0, $type = 0, $brand_id = 0, $min_price = 0, $max_price = 0, $flag = 0, $category = ''){
    	 if(isset($this->configs['visitors_no_visit_product'])&&$this->configs['visitors_no_visit_product']=='1'){
    	 	 if(!isset($_SESSION['User'])){
     			$this->flash($this->ld['time_out_relogin'],'/users/login');
     			return;
      	}
     	 }
       ...
       if (isset($_GET['keyword']) && $_GET['keyword'] != '') {
        ...
        if ($keyword == 'All'||$keyword == 'All Product') {
            $keyword = '';
        }
        $strkeyword = $keyword;
        if ($strkeyword == '') {
        ...
        if(isset($_GET['category_id']) && intval($_GET['category_id'])>0){
        	$direct_sub_category_ids=isset($category_infos['direct_subids'])?$category_infos['direct_subids']:array();
        	$category_id=intval($_GET['category_id']);
        	$category_ids=array($category_id);
        	$sub_cat_ids=isset($direct_sub_category_ids[$category_id])?$direct_sub_category_ids[$category_id]:array();
        	$category_ids=array_merge($category_ids,$sub_cat_ids);
        	foreach($sub_cat_ids as $v){
        		$sub_cat_ids=isset($direct_sub_category_ids[$v])?$direct_sub_category_ids[$v]:array();
        		$category_ids=array_merge($category_ids,$sub_cat_ids);
        	}
            	$conditions['AND']['Product.category_id'] =$category_ids;
        }
        if(isset($_GET['type'])&&trim($_GET['type'])!='')$type=UrlDecode($_GET['type']);
            ...
            foreach ($pro as $k => $v) {
                ...
                if(!empty($v['ProductAttribute'])){
                	foreach($v['ProductAttribute'] as $vv){
                		$product_attributes[$vv['product_id']][$vv['attribute_id']]=$vv['attribute_value'];
                	}
                }
            }
		...
		$attr_id_infos=array();$pat = array();
		$pubile_attr_options=array();
		if(!empty($pubile_attr_info)){
			foreach($pubile_attr_info as $v){
				$attr_id_infos[$v['Attribute']['code']]=$v['Attribute']['id'];
				$pat[$v['Attribute']['id']] = $v['AttributeI18n']['name'];
				if(!empty($v['AttributeOption'])){
					foreach($v['AttributeOption'] as $vv){
						$pubile_attr_options[$vv['attribute_id']][$vv['option_value']]=$vv['option_name'];
					}
				}
			}
		}
            ...
            $this->set('pubile_attr_options',$pubile_attr_options);
            $this->layout = 'default_search';
        if (isset($_POST['flag']) && $_POST['flag'] == 1&&!empty($pro)) {
		$allproduct = array();
		Configure::write('debug', 1);
		$allproduct = array();
		$export_fields=array($this->ld['product_description'],$this->ld['product_code'],$this->ld['brand'],$this->ld['stock']);
		if(!empty($pat))$export_fields=array_merge($export_fields,$pat);
		$allproduct[]=$export_fields;
		foreach($pro as $v){
			$product_tmp=array();
			$product_tmp[]=$v['ProductI18n']['name'];
			$product_tmp[]=$v['Product']['code'];
			$product_tmp[]=isset($brand_names[$v['Product']['brand_id']])?$brand_names[$v['Product']['brand_id']]:'';
			$product_tmp[]=$v['Product']['quantity'];
			if(!empty($pat)){
				$product_id=$v['Product']['id'];
				foreach($pat as $kk=>$vv){
					$attribute_value=isset($product_attributes[$product_id][$kk])?$product_attributes[$product_id][$kk]:'';
					$attribute_name=isset($pubile_attr_options[$kk][$attribute_value])?$pubile_attr_options[$kk][$attribute_value]:'';
					$product_tmp[]=$attribute_name;
				}
			}
			$allproduct[]=$product_tmp;
		}
            //把一个数组输出成文件
    ...
    public function ajax_add_message()
    {
		...
		if (isset($_POST)) {
            ...
            if(!empty($result)){
	            ...
	            $result_mail = $this->Notify->send_email($mail_send_queue, $this->configs);
    ...
    */
    ...
    public function add_comment()
    {
        ...
        if ($this->RequestHandler->isPost()) {
            ...
            if ($comment_captcha) {
                if (!isset($this->data['Comment']['authnum'])) {
            ...
            if (isset($pro_info['Product'])) {
                ...
                $action_data['UserAction']['content'] = $_SESSION['User']['User']['name']."评论了商品 <a href='".$this->base."/products/".$pro_info['Product']['id']."'>".(isset($pro_info['ProductI18n']['name']) ? $pro_info['ProductI18n']['name'] : $type).'</a>'.(($this->data['Comment']['content'] != '') ? ' - '.$this->data['Comment']['content'] : '');
    ...
    public function uploadPic()
    {
        ...
        if ($this->RequestHandler->isPost()) {
            ...
            if (!in_array($info['extension'], $types)) {
                ...
                $img_url = $this->server_host.'files/upload_logo/'.$img_name;
    ...
    public function upload_add($product_id)
    {
        if ($this->RequestHandler->isPost()) {
            ...
            if (empty($product_id)) {
                $msg = '请选择商品';
                echo '<meta http-equiv="Content-Type" content="text/html; charset=utf-8" /><script type="text/javascript">alert("'.$msg.'");	window.location.href="'.$this->base.'/products/view_canvas/"</script>';
                die();
            }
            ...
            if (isset($files['img_file_url']['tmp_name']) && !empty($files['img_file_url']['tmp_name'])) {
                ...
                $product['file_url'] = $this->server_host.'files/'.date('Ymd').$file_name;
    ...
    public function get_url($product_id)
    {
        ...
        $product['file_url'] = $this->server_host.'/files/'.$file_name;
        $this->Product->save(array('Product' => $product));
        $result['flag'] = 1;
    ...
    public function add_weibo_pic($status = '新内容', $pic = '', $qq_list)
    {
        ...
        $t_client_id = $this->UserApp->find('first', array('conditions' => array('UserApp.type' => 'QQWeibo','UserApp.location'=>array(0,2))));

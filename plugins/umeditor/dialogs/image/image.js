...
//                }
    ...
    var NetWork = {
		...
		/*Image Space*/
		$(".edui-image-space", me.dialog).on("click", function () {
			$ele.attr('id','EduiImageSearchTxt');
			var pageurl=window.location.href;
			if(pageurl.indexOf("/open_elements")>0||pageurl.indexOf("/operators/view")>0){
				select_img('EduiImageSearchTxt','OpenElementDescription');
			}else{
				select_img('EduiImageSearchTxt');
			}
		}).hover(function () {
    ...
    UM.registerWidget('image', {
        tpl: "<link rel=\"stylesheet\" type=\"text/css\" href=\"<%=image_url%>image.css\">" +
            ...
            "<td><div class=\"edui-image-searchAdd\"><%=lang_btn_add%></div></td><td><div class=\"edui-image-space\"><%=lang_image_space%></div></td></tr></table>" +
            "</div>" +
